...
- Rocket Soccer's Unity UI is fixed at 16:9. The iframe/card remains vertical for the Tip Tap feed, while the wrapper centers the complete interactive 16:9 canvas and uses a muted live canvas stream as a full-card background. Tip Tap's expanded control rail stays below the interactive game. Do not stretch Unity back to the card's portrait aspect ratio; that clips the source game's right and bottom controls.
