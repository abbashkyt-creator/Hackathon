/*
 * Fail-closed local PokiSDK shim for the Master Chess build.
...
(function () {
  ...
  window.PokiSDK = {
    ...
    commercialBreak: (cb) => adBreak('interstitial', 'master-chess-source-commercial').then((shown) => { if (shown) cb?.(); }),
    rewardedBreak: (cb) => adBreak('rewarded', 'master-chess-source-rewarded').then((shown) => { if (shown) cb?.(); }),
