/*
 * Fail-closed local PokiSDK shim for the Basketball Stars build.
...
(function () {
  ...
  window.PokiSDK = {
    ...
    commercialBreak: (cb) => adBreak('interstitial', 'basketball-stars-source-commercial').then((shown) => { if (shown) cb?.(); }),
    rewardedBreak: (cb) => adBreak('rewarded', 'basketball-stars-source-rewarded').then((shown) => { if (shown) cb?.(); }),
