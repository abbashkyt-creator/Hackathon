/*
 * Fail-closed local PokiSDK shim for the Stickman Hook build.
...
(function () {
  ...
  window.PokiSDK = {
    ...
    commercialBreak: (cb) => adBreak('interstitial', 'stickman-hook-source-commercial').then((shown) => { if (shown) cb?.(); }),
    rewardedBreak: (cb) => adBreak('rewarded', 'stickman-hook-source-rewarded').then((shown) => { if (shown) cb?.(); }),
