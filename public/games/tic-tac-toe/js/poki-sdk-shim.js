/*
 * Fail-closed local PokiSDK shim for the TicTacToe build (Cocos Creator 3.8).
 * Resolves init immediately; routes ad breaks to the Tip Tap owned-ads client;
 * no-ops lifecycle. No remote SDK or analytics.
 */
(function () {
  'use strict';
  ...
  window.PokiSDK = {
    ...
    commercialBreak: (cb) => adBreak('interstitial', 'tic-tac-toe-source-commercial').then((shown) => { if (shown) cb?.(); }),
    rewardedBreak: (cb) => adBreak('rewarded', 'tic-tac-toe-source-rewarded').then((shown) => { if (shown) cb?.(); }),
