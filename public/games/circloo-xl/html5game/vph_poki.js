...
function poki_game_loaded() {
    var gmdiv = document.getElementById("gm4html5_div_id");
    gmdiv.style.visibility = "visible";

    if (window.PokiSDK_loadState != 2) {
        window.PokiSDK_loadState = 2;
        if (PokiSDK)
            PokiSDK.gameLoadingFinished();
    }

    window.removeLoadingTextP();
}

function poki_measure(category, action, label) {
    if (PokiSDK) {
        PokiSDK.measure(category, action, label);
    }

    console.log("Poki measure: " + category + ", " + action + ", " + label);
}
