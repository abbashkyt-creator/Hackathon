 ...
 * and equipping dragon skins.
 *
 * HOW SKIN CONFIG WORKS
 *   Skins are defined in CONFIG.dragonSkins (config.js).  Each entry has:
 *     id            – unique string key
 *     nameKey       – i18n key for the localized display name
 *     texture       – path to the full dragon texture used in gameplay
 ...
 *   player.setDragonTexture(texturePath) – a method we add to Player.js.
 *
 * HOW OWNERSHIP PERSISTENCE WORKS
 *   Owned skin ids and the equipped skin id are stored together with the coin
 *   count in localStorage under the key 'dragonPlayerData'.
 *   DragonSkinShop.loadPlayerData() / savePlayerData() handle serialization.
 ...
 *   two-column landscape card.  The dragon preview sits in a <canvas> element
...
const STORAGE_KEY = 'dragonPlayerData';
...
function defaultPlayerData() {
    const ownedByDefault = (CONFIG.dragonSkins ?? [])
        .filter((s) => s.unlockedByDefault)
        .map((s) => s.id);
    const firstId = CONFIG.dragonSkins?.[0]?.id ?? 'classic';
    return {
        coins: 0,
        ownedDragonSkins: ownedByDefault.length > 0 ? ownedByDefault : [firstId],
        equippedDragonSkinId: firstId
    };
}
...
function injectStyles() {
    if (document.getElementById('dragon-skin-shop-styles')) return;
    const style = document.createElement('style');
    style.id = 'dragon-skin-shop-styles';
    style.textContent = `
        .dss-screen {
            position: fixed;
            inset: 0;
...
export class DragonSkinShop {
     ...
     * @param {(id: string, texturePath: string) => void} options.onEquip – apply texture to dragon
    ...
    constructor({ getCoinCount, setCoinCount, onEquip, onWatchAd, onHide } = {}) {
        ...
        this._data = saved ? {
            ...
            ownedDragonSkins: Array.isArray(saved.ownedDragonSkins) ? saved.ownedDragonSkins : defaults.ownedDragonSkins,
            equippedDragonSkinId: saved.equippedDragonSkinId ?? defaults.equippedDragonSkinId,
        } : defaults;
        // Sync coin count from the live game value on open (handled in show()).

        this.skins = CONFIG.dragonSkins ?? [];
        this._selectedIndex = Math.max(0, this.skins.findIndex(
            (s) => s.id === this._data.equippedDragonSkinId
        ));

        // THREE.js mini-renderer for the dragon preview.
        this._previewRenderer = null;
        this._previewScene = null;
        this._previewCamera = null;
        this._previewMixer = null;
        this._previewDragonModel = null;
        ...
        this._previewDragonAssetPromise = null;
        this._previewDragonAsset = null;
    ...
    _buildThumbs() {
        ...
        this.skins.forEach((skin, i) => {
            ...
            if (this._data.ownedDragonSkins.includes(skin.id)) {
    ...
    _refreshUI() {
        ...
        const isOwned = this._data.ownedDragonSkins.includes(skin.id);
        const isEquipped = this._data.equippedDragonSkinId === skin.id;
    ...
    _handleAction() {
        ...
        this._data.ownedDragonSkins.push(skin.id);
        this.setCoinCount(this._data.coins);
        this._saveData();
        this._equipSkin(skin.id);
        console.log('Dragon skin purchased', skin.id);
    }
    ...
    async _handleAdUnlock() {
        ...
        try {
            ...
            if (rewarded) {
                ...
                this._saveData();
                this._equipSkin(skin.id);
                console.log('Dragon skin unlocked via ad', skin.id);
            }
        } finally {
            this._adInProgress = false;
    ...
    _equipSkin(skinId) {
        ...
        if (!skin) return;
        this._data.equippedDragonSkinId = skinId;
        this._saveData();
        this.onEquip(skinId, skin.texture);
        this._buildThumbs();
        this._refreshUI();
        console.log('Dragon skin equipped', skinId);
    }
    ...
    preloadPreviewAssets() {
        void this._ensurePreviewDragonAsset();
        const skin = this.skins[this._selectedIndex];
        if (skin?.texture) {
    ...
    hasAffordableLockedSkin(currentCoins = this._data.coins) {
        ...
        const owned = new Set(this._data.ownedDragonSkins || []);
    ...
    _initPreviewRenderer() {
        ...
        this._loadPreviewDragon();
        this._previewClock.start();
        this._tickPreview();
    }

    _ensurePreviewDragonAsset() {
        if (this._previewDragonAsset) {
            return Promise.resolve(this._previewDragonAsset);
        }
        if (this._previewDragonAssetPromise) {
            return this._previewDragonAssetPromise;
        }

        const loader = createGLTFLoader();
        this._previewDragonAssetPromise = loaderLoadWithRetry(loader, './gfx/mesh/dragon/dragon.glb')
            .then((gltf) => {
                this._previewDragonAsset = gltf;
                return gltf;
            })
            .catch((err) => {
                this._previewDragonAssetPromise = null;
    ...
    _loadPreviewTextureAsync(texturePath) {
        ...
        this._previewTexturePromises[texturePath] = new Promise((resolve) => {
            this._previewTextureLoader.load(
                ...
                undefined,
                (err) => {
                    console.warn('[DragonSkinShop] Texture load error', texturePath, err);
    ...
    async _loadPreviewDragon() {
        let gltf;
        try {
            gltf = await this._ensurePreviewDragonAsset();
        } catch (err) {
            console.warn('[DragonSkinShop] Preview dragon load error', err);
            return;
        }

        console.log('[DragonSkinShop] Dragon loaded for preview');
        ...
        this._previewDragonModel = model;
        ...
        if (gltf.animations?.length) {
            ...
            const idleClip =
                ...
                gltf.animations[0];
            console.log('[DragonSkinShop] Playing animation:', idleClip?.name);
            if (idleClip) {
    ...
    _applyTextureToPreviewModel(texturePath) {
        ...
        if (!tex || !this._previewDragonModel) return;
        this._previewDragonModel.traverse((child) => {
            if (!child.isMesh) return;
            const mats = Array.isArray(child.material) ? child.material : [child.material];
            for (const m of mats) {
                if (!m) continue;
    ...
    _tickPreview() {
        ...
        if (this._previewDragonModel) {
            this._previewDragonModel.rotation.y += delta * 0.35;
        }
    ...
    getEquippedSkinId() {
        return this._data.equippedDragonSkinId;
    }
