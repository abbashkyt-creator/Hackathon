...
const TOP_BAR_ASSETS = {
    ...
    flameBackground: './gfx/UI/flame.webp',
    flameFill: './gfx/UI/flame_fill.webp',
...
export class TopBarUI {
    constructor(options = {}) {
        ...
        this.textures = {
            ...
            flameBackground: this.loadTexture(TOP_BAR_ASSETS.flameBackground),
            flameFill: this.loadTexture(TOP_BAR_ASSETS.flameFill),
        ...
        this.flameIndicator = this.createProgressIndicator(
            this.textures.flameBackground,
            this.textures.flameFill,
            'vertical',
            2010
        );
        ...
        this.coinDisplay = this.createCoinDisplay(2035);
        ...
        // Red dot showing dragon position on the minimap.
    ...
    setFlameProgress(value) {
        this.setIndicatorProgress(this.flameIndicator, value);
    }
    ...
    layout(width, height) {
        ...
        let flameWidth = verticalHeight * this.getTextureAspect(this.textures.flameBackground, 1);
        ...
        let groupWidth = healthWidth + flameWidth + energyWidth + coinWidth + (groupGap * 3)
        ...
        if (groupWidth > availableGroupWidth) {
            ...
            groupWidth = healthWidth + flameWidth + energyWidth + coinWidth + (groupGap * 3)
        ...
        this.setIndicatorSize(this.flameIndicator, flameWidth, verticalHeight);
        ...
        this.flameIndicator.root.position.set(cursorX + (flameWidth * 0.5), rowCenterY, 0);
        cursorX += flameWidth + groupGap;
    ...
    // Maps the dragon's world position onto the minimap and positions the red dot.
    ...
    // The dot is clamped to minimap bounds in case the dragon flies outside the grid.
    updateMinimapPlayerDot(dragonX, dragonY, level) {
        ...
        let rx = (dragonX - originX) / gridWorldWidth;
        rx = THREE.MathUtils.clamp(rx, 0, 1);

        let mappedRy;
        const dragonWorldY = dragonY;
        if (dragonWorldY <= worldGridTop) {
            // Below or at grid top — map into [gridTop .. gridBottom] image slice
            const t = THREE.MathUtils.clamp((dragonWorldY - worldGridBottom) / Math.max(worldGridTop - worldGridBottom, 0.0001), 0, 1);
            ...
            const t = THREE.MathUtils.clamp((dragonWorldY - worldGridTop) / aboveCeilingRange, 0, 1);
    ...
    updateMinimapFighterJetDots(objects, level) {
        if (!this._fighterJetDotPool) {
            this._fighterJetDotPool = [];
            this._fighterJetDotTexture = this._createDotTexture(6);
        }

        const needed = objects ? objects.length : 0;

        while (this._fighterJetDotPool.length < needed) {
            const sprite = new THREE.Sprite(
                new THREE.SpriteMaterial({
                    map: this._fighterJetDotTexture,
                    color: 0x888888,
                    transparent: true,
                    depthTest: false,
                    depthWrite: false,
            ...
            sprite.visible = false;
            this.root.add(sprite);
            this._fighterJetDotPool.push(sprite);
        }

        const b = this.minimapScreenBounds;
        const dotSize = Math.max(b.height * 0.035, 2);
        const dotScale = dotSize * 2;
        const poolLen = this._fighterJetDotPool.length;

        for (let i = 0; i < needed; i++) {
            const sprite = this._fighterJetDotPool[i];
            const obj    = objects[i];
