...
const SIDE_FLAME_ASSETS = {
    background: 'gfx/UI/sidebutton.webp',
    flameIcon: 'gfx/UI/booster_flame.webp',
    timerGreen: 'gfx/UI/timer_green.webp',
    timerRed: 'gfx/UI/timer_red.webp',
    ad: 'gfx/UI/ad.webp'
};

export const SIDE_FLAME_BOOST_ASSET_URLS = Object.values(SIDE_FLAME_ASSETS);
...
function createAdditiveSpriteMaterial(texture) {
    return new THREE.SpriteMaterial({
        map: texture,
        transparent: true,
        opacity: 0,
...
export class SideFlameBoostButton {
    constructor({ durationSeconds = 120, onPress, domElement = null, loadingManager = null } = {}) {
        ...
        // Match Joystick.js: UI size is derived from the screen short side over CONFIG.VIEW_HEIGHT.
        this.uiShortSideUnits = CONFIG.VIEW_HEIGHT;

        this.textureLoader = new THREE.TextureLoader(this.loadingManager);
        this.textures = {
            background: this.loadTexture(SIDE_FLAME_ASSETS.background),
            flameIcon: this.loadTexture(SIDE_FLAME_ASSETS.flameIcon),
            timerGreen: this.loadTexture(SIDE_FLAME_ASSETS.timerGreen),
            timerRed: this.loadTexture(SIDE_FLAME_ASSETS.timerRed),
            ad: this.loadTexture(SIDE_FLAME_ASSETS.ad)
        };
        ...
        this.iconSprite = new THREE.Sprite(createSpriteMaterial(this.textures.flameIcon, 1));
        ...
        this.glowSprite = new THREE.Sprite(createAdditiveSpriteMaterial(this.textures.background));
        this.glowSprite.renderOrder = 2540;
    ...
    update({ isActive, isPending, remainingSeconds, durationSeconds, isVisible = true, isUnlocked = false, highlight = false, dt = 0 } = {}) {
        ...
        const dtSafe = Math.max(0, Number.isFinite(dt) ? dt : 0);
        if (highlight) {
            this.highlightPhase = (this.highlightPhase + dtSafe * Math.PI * 4) % (Math.PI * 2);
        } else {
            this.highlightPhase = 0;
        }
        ...
        this.glowSprite.visible = highlight;
        this.glowSprite.material.opacity = highlight ? 0 + 0.5 * (0.5 + 0.5 * Math.sin(this.highlightPhase)) : 0;
    ...
    layout(width, height) {
        ...
        const centerY = topBarMetrics.rowCenterY - (topBarMetrics.rowHeight * 0.5) - missionGap - (buttonHeight * 0.5);
        ...
        this.glowSprite.scale.set(buttonWidth * 1.3, buttonHeight * 1.3, 1);
        this.iconSprite.position.set(-buttonWidth * 0.06, buttonHeight * 0.02, 0);
        const iconWidth = buttonWidth;
        const iconHeight = iconWidth / this.getTextureAspect(this.textures.flameIcon, 1);
