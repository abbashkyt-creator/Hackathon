...
export class TankObject extends VehicleObject {
    ...
    getTankCombatConfig() {
        ...
        return {
            ...
            bulletDamageToDragon: Number.isFinite(combat.bulletDamageToDragon) ? Math.max(0, combat.bulletDamageToDragon) : 15,
    ...
    getTargetWorldPosition(dragonTarget, out = new THREE.Vector3()) {
        const hitCircle = dragonTarget?.getWorldCollisionCircle?.();
        if (
            hitCircle &&
            ...
            // and collision outcomes stay aligned (especially while the dragon is airborne).
            out.set(hitCircle.centerX, hitCircle.centerY, this.container.position.z);
            return out;
        }

        if (dragonTarget?.getMouthWorldPosition) {
            return dragonTarget.getMouthWorldPosition(out);
        }

        if (dragonTarget?.getWorldPosition) {
            return dragonTarget.getWorldPosition(out);
        }
    ...
    canTrackAndFire(dragonTarget, targetWorld) {
        if (!dragonTarget || !targetWorld || !this.cannonPivot || !this.cannonNode) {
            return false;
        }
    ...
    getBullet() {
        ...
        return {
            ...
            damageToDragon: this.tankCombat.bulletDamageToDragon
        };
    }
    ...
    fireBullet() {
        ...
        bullet.damageToDragon = this.tankCombat.bulletDamageToDragon;
    ...
    getActiveBulletsForCollision() {
        return this.activeBullets
            ...
            .map((bullet) => ({
                ...
                damageToDragon: bullet.damageToDragon
            }));
    }
    ...
    updateTankCombat(delta, dragonTarget) {
        ...
        const targetWorld = this.getTargetWorldPosition(dragonTarget, TMP_TARGET_WORLD);
        const inRange = this.canTrackAndFire(dragonTarget, targetWorld);
        this.fireCooldown = Math.max(0, this.fireCooldown - delta);
    ...
    update(delta, level, dragonTarget = null) {
        ...
        this.updateTankCombat(delta, dragonTarget);
