...
export class LevelRenderer {
    constructor(scene, level, options = {}) {
        ...
        // The dragon model has real 3D depth and a forward visual Z offset, so front object
        ...
        this.dragonWorldZ = Number.isFinite(CONFIG?.spawnPosition?.z) ? CONFIG.spawnPosition.z : 0;
        this.postGameplayLayerSpacing = 0.5;
        this.gameplayLayerIndex = this.findGameplayLayerIndex();
        this.dragonRenderOrder = this.getLayerRenderOrder(this.gameplayLayerIndex) + 0.5;
/*        
        this.materials = {
...
*/
    ...
    getLayerRenderOrder(layerIndex) {
        ...
        // the dragon split: layers after "Gameplay" get nudged forward so the dragon can sit
    ...
    getDragonRenderOrder() {
        return this.dragonRenderOrder;
    }

    // Returns renderOrder and layerDepth (Z) for objects that should appear in front of all
    // foreground tile layers but still be occluded by the depth buffer (so the dragon,
    ...
    createLayerMaterial(options = {}) {
        ...
        const material = new THREE.MeshBasicMaterial({
            ...
            // transparent object-layer sprites can sit either in front of or behind the dragon.
            depthTest: true,
            depthWrite: false
        });
    ...
    getLayerDepth(layerIndex) {
        ...
        // Keep the dragon in a fixed world-Z slot, then start the first layer after Gameplay
        ...
        return this.dragonWorldZ + this.layerZSpacing + (postGameplayIndex * this.postGameplayLayerSpacing);
    }
