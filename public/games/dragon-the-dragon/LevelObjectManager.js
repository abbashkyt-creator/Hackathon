...
export class LevelObjectManager {
    ...
    constructor(scene, level, options = {}) {
        ...
        this.lastRingPassDragonPoint = null;
    ...
    async loadFromLevel() {
        ...
        // Collect all objects to load first so we can batch them in parallel.
        const pending = [];
        for (const layer of this.level.objectLayers || []) {
            ...
            for (const object of layer.objects || []) {
                ...
                pending.push({ type, object, levelObject });
            }
        }

        // Load in parallel batches. Each load() only touches its own instance so
        // concurrent calls are safe. Batching avoids overwhelming the browser with
        // too many simultaneous network requests and GLB parse tasks at once.
        const BATCH_SIZE = 6;
        for (let i = 0; i < pending.length; i += BATCH_SIZE) {
            const batch = pending.slice(i, i + BATCH_SIZE);
            const results = await Promise.allSettled(
                batch.map(({ levelObject }) => levelObject.load())
            );
            for (let j = 0; j < batch.length; j++) {
                if (results[j].status === 'fulfilled') {
                    loadedObjects.push(batch[j].levelObject);
                } else {
                    const err = results[j].reason;
                    const { type, object } = batch[j];
    ...
    updateRingPassProgress() {
        const dragonCircle = this.dragonTarget?.getWorldCollisionCircle?.();
        if (
            !dragonCircle ||
            !Number.isFinite(dragonCircle.centerX) ||
            !Number.isFinite(dragonCircle.centerY)
        ) {
            this.lastRingPassDragonPoint = null;
            return;
        }
        ...
        const currentPoint = {
            x: dragonCircle.centerX,
            y: dragonCircle.centerY
        };
        const previousPoint = this.lastRingPassDragonPoint || currentPoint;
        ...
        this.lastRingPassDragonPoint = currentPoint;
    }
    ...
    triggerDragonFaintCrashExplosion(position, faintConfig = {}) {
        if (!position || !Number.isFinite(position.x) || !Number.isFinite(position.y)) {
            return;
        }
    ...
    applyDragonFireDamage(player, delta) {
        if (!player || !Number.isFinite(delta) || delta <= 0) {
            return;
        }

        // Grid was rebuilt at end of previous frame's update() — no need to rebuild again here.
        this.applyDragonFireballDamage(player);
        this.applyDragonContinuousFlameDamage(player, delta);
    }

    applyBossFireDamage(bossObject, player, delta) {
        if (!bossObject || !player || player.isDead?.() === true) {
            return;
        }
        ...
        // ── Fireballs ──────────────────────────────────────────────────────────
        const fireballDamage = Math.max(0, bossObject.getBossFireballDamage?.() ?? 0);
        if (fireballDamage > 0) {
            const fireballs = bossObject.getBossActiveFireballsForDamage?.() || [];
            const dragonCircle = player.getWorldCollisionCircle?.();
            if (dragonCircle) {
                for (const fireball of fireballs) {
                    if (!Number.isFinite(fireball?.id) ||
                        !Number.isFinite(fireball?.x) ||
                        !Number.isFinite(fireball?.y)) {
                        continue;
                    }
                    const fireballRadius = Math.max(
                        0,
                        Number.isFinite(fireball.radius) ? fireball.radius : 0
                    );
                    const fdx = fireball.x - dragonCircle.centerX;
                    const fdy = fireball.y - dragonCircle.centerY;
                    const distSq = fdx * fdx + fdy * fdy;
                    const hitDist = dragonCircle.radius + fireballRadius;
                    if (distSq > hitDist * hitDist) {
                        continue;
                    }

                    player.applyDamage?.(fireballDamage, 'fireball');

                    if (bossObject.shouldBossDespawnFireballOnHit?.()) {
                        ...
                        bossObject.consumeBossFireballById?.(fireball.id, {
                            impactPosition: {
                                x: fireball.x,
                                y: fireball.y,
                                z: Number.isFinite(fireball.z) ? fireball.z : 0
                            },
                            impactNormal
                        });
                    }
        ...
        // ── Continuous flame ───────────────────────────────────────────────────
        const flameDps = Math.max(0, bossObject.getBossFlameDamagePerSecond?.() ?? 0);
        if (flameDps > 0) {
            const flameRect = bossObject.getBossContinuousFlameDamageRect?.();
            if (flameRect) {
                const dragonCircle = player.getWorldCollisionCircle?.();
                if (dragonCircle) {
                    const playerRect = {
                        centerX: dragonCircle.centerX,
                        centerY: dragonCircle.centerY,
                        halfWidth: dragonCircle.radius,
                        halfHeight: dragonCircle.radius,
                        angle: 0
                    };
                    if (rectIntersectsRect(flameRect, playerRect)) {
                        player.applyDamage?.(flameDps * delta, 'fire');
                    }
                }
            }
        }
    ...
    applyTankBulletDamage(player) {
        ...
        const dragonCircle = player.getWorldCollisionCircle?.();
        const fireballEquivalentDamage = Number.isFinite(player.getFireballDamage?.())
            ? Math.max(player.getFireballDamage(), 0)
            : 0;
        if (!dragonCircle && fireballEquivalentDamage <= 0) {
            return;
        }
        ...
        for (const object of this.objects) {
            ...
            const isCarriedTank = object.state === 'carried';
            ...
            for (const bullet of bullets) {
                ...
                if (isCarriedTank && fireballEquivalentDamage > 0) {
                    let closestTarget = null;
                    let closestDistance = Number.POSITIVE_INFINITY;
                    const bulletPoint = { x: bullet.x, y: bullet.y };

                    for (const target of this.objects) {
                        if (!target || target === object || target.isDestroyed || target.markedForRemoval) {
                            continue;
                        }
                        ...
                        if (isPointInsideObject(bulletPoint, target)) {
                            continue;
                        }

                        const distanceToRect = pointToObjectDistance(bulletPoint, target);
                        if (distanceToRect > bulletRadius || distanceToRect >= closestDistance) {
                            continue;
                        }

                        closestDistance = distanceToRect;
                        closestTarget = target;
                    }

                    if (closestTarget) {
                        closestTarget.applyDamage(fireballEquivalentDamage, 'tankBulletObject');
                ...
                const distanceToDragon = pointToCircleDistance(
                    { x: bullet.x, y: bullet.y },
                    dragonCircle
                );
                if (distanceToDragon > bulletRadius) {
                    continue;
                }

                const damage = Math.max(
                    0,
                    Number.isFinite(bullet.damageToDragon) ? bullet.damageToDragon : 0
                );
                if (damage <= 0) {
                    object.consumeBulletById?.(bullet.id);
                    continue;
    ...
    applyDragonFireballDamage(player) {
        const fireballDamage = Number.isFinite(player.getFireballDamage?.())
            ? Math.max(player.getFireballDamage(), 0)
            : 0;
        if (fireballDamage <= 0) {
            return;
        ...
        const fireballs = player.getActiveFireballsForDamage?.() || [];
        if (!fireballs.length) {
            this.fireballHitRegistry.clear();
            return;
        }
        const activeFireballIds = this._activeFireballIds;
        activeFireballIds.clear();
        for (let i = 0; i < fireballs.length; i++) {
            const id = fireballs[i]?.id;
            if (Number.isFinite(id)) activeFireballIds.add(id);
        }
        for (const trackedFireballId of this.fireballHitRegistry.keys()) {
            if (!activeFireballIds.has(trackedFireballId)) {
                this.fireballHitRegistry.delete(trackedFireballId);
            }
        }

        for (const fireball of fireballs) {
            if (!Number.isFinite(fireball?.x) || !Number.isFinite(fireball?.y) || !Number.isFinite(fireball?.id)) {
                continue;
            }
            ...
            const fireballPoint = this._scratchFireballPoint;
            fireballPoint.x = fireball.x;
            fireballPoint.y = fireball.y;
            let fireballConsumed = false;
            const fireballCandidates = this.queryObjectSpatialCandidates(
                fireball.x - fireballRadius,
                fireball.y - fireballRadius,
                fireball.x + fireballRadius,
                fireball.y + fireballRadius
            );

            for (const object of fireballCandidates) {
                if (!object || object.isDestroyed || object.markedForRemoval) {
                    continue;
                }

                const missiles = object.getActiveMissilesForCollision?.() || [];
                if (!missiles.length) {
                    continue;
                }

                for (const missile of missiles) {
                    if (!missile?.active || !Number.isFinite(missile?.position?.x) || !Number.isFinite(missile?.position?.y)) {
                        continue;
                    }

                    const missileRadius = Math.max(Number.isFinite(missile.hitRadius) ? missile.hitRadius : 0.45, 0.25);
                    const distanceToMissile = Math.hypot(
                        fireballPoint.x - missile.position.x,
                        fireballPoint.y - missile.position.y
                    );
                    if (distanceToMissile > fireballRadius + missileRadius) {
                        continue;
                    }

                    missile.destroyMissile?.('hitByDragonFire');
                    this.audioManager?.play?.('fireHit', { volume: 0.8, cooldown: 0.06 });
                    player.consumeFireballById?.(fireball.id, {
                        impactPosition: {
                            x: missile.position.x,
                            y: missile.position.y,
                            z: Number.isFinite(missile.position.z) ? missile.position.z : 0
                        },
                        impactNormal: null
                    });
                    this.fireballHitRegistry.delete(fireball.id);
                    fireballConsumed = true;
                    break;
                }
            ...
            let closestTarget = null;
            let closestDistance = Number.POSITIVE_INFINITY;
            let hitTargetsForFireball = this.fireballHitRegistry.get(fireball.id);
            if (!hitTargetsForFireball) {
                hitTargetsForFireball = new Set();
                this.fireballHitRegistry.set(fireball.id, hitTargetsForFireball);

                if (Number.isFinite(fireball.spawnX) && Number.isFinite(fireball.spawnY)) {
                    const fireballSpawnPoint = { x: fireball.spawnX, y: fireball.spawnY };
                    const spawnCandidates = this.queryObjectSpatialCandidates(
                        fireball.spawnX,
                        fireball.spawnY,
                        fireball.spawnX,
                        fireball.spawnY
                    );
                    for (const target of spawnCandidates) {
                        if (!target || target.isDestroyed || target.markedForRemoval || target.id == null) {
                            continue;
                        }
                        if (target.config?.indestructible) {
                            continue;
                        }

                        const targetRect = target.getWorldCollisionRect?.() || target.getExplosionDamageRect?.();
                        if (!targetRect || !isPointInsideObject(fireballSpawnPoint, target)) {
                            continue;
                        }

                        hitTargetsForFireball.add(target.id);
                    }
                }
            }

            for (const target of fireballCandidates) {
                if (!target || target.isDestroyed || target.markedForRemoval) {
                    continue;
                }
                if (target.config?.indestructible) {
                    continue;
                }
                if (target.id != null && hitTargetsForFireball.has(target.id)) {
                    continue;
                }
                ...
                if (isPointInsideObject(fireballPoint, target)) {
                    continue;
                }

                const distanceToRect = pointToObjectDistance(fireballPoint, target);
                if (distanceToRect > fireballRadius || distanceToRect >= closestDistance) {
                    continue;
                }
            ...
            closestTarget.applyDamage(fireballDamage, 'fireball');
            this.playObjectHitSound(closestTarget, { volume: 0.85, cooldown: 0.06 });
            if (closestTarget.id != null) {
                hitTargetsForFireball.add(closestTarget.id);
            }
            if (player.shouldDespawnFireballOnHit?.()) {
                const targetRect = closestTarget.getWorldCollisionRect?.() || closestTarget.getExplosionDamageRect?.();
                let impactNormal = null;
                if (targetRect) {
                    const normalX = fireball.x - targetRect.centerX;
                    const normalY = fireball.y - targetRect.centerY;
    ...
    applyDragonContinuousFlameDamage(player, delta) {
        const flameDamagePerSecond = Number.isFinite(player.getFlameDamagePerSecond?.())
            ? Math.max(player.getFlameDamagePerSecond(), 0)
            : 0;
        if (flameDamagePerSecond <= 0) {
            return;
        }

        const flameRect = player.getContinuousFlameDamageRect?.();
        if (!flameRect) {
            return;
        }

        const frameDamage = flameDamagePerSecond * delta;
        if (frameDamage <= 0) {
            return;
        }
        const flameSourcePoint = (
            Number.isFinite(flameRect.sourceX) &&
            Number.isFinite(flameRect.sourceY)
        )
            ? { x: flameRect.sourceX, y: flameRect.sourceY }
            : null;
        const flameCandidates = this.queryObjectSpatialCandidates(
            flameRect.centerX - flameRect.halfWidth,
            flameRect.centerY - flameRect.halfHeight,
            flameRect.centerX + flameRect.halfWidth,
            flameRect.centerY + flameRect.halfHeight
        );

        for (const object of flameCandidates) {
            if (!object || object.isDestroyed || object.markedForRemoval) {
                continue;
            }
            if (object.config?.indestructible) {
                continue;
            }
            ...
            for (const missile of missiles) {
                ...
                const missileRect = {
                    centerX: missile.position.x,
                    centerY: missile.position.y,
                    halfWidth: Math.max(Number.isFinite(missile.hitRadius) ? missile.hitRadius : 0.45, 0.25),
                    halfHeight: Math.max(Number.isFinite(missile.hitRadius) ? missile.hitRadius : 0.45, 0.25),
                    angle: 0
                };

                if (flameSourcePoint && isPointInsideRect(flameSourcePoint, missileRect)) {
                    continue;
                }

                if (!rectIntersectsRect(flameRect, missileRect)) {
                    continue;
                }

                missile.destroyMissile?.('hitByDragonFire');
                this.audioManager?.play?.('fireHit', { volume: 0.65, cooldown: 0.12 });
            }

            const objectRect = object.getWorldCollisionRect?.() || object.getExplosionDamageRect?.();
            if (!objectRect) {
                continue;
            }

            if (flameSourcePoint && isPointInsideObject(flameSourcePoint, object)) {
                continue;
            }

            if (!rectIntersectsObject(flameRect, object)) {
                continue;
            }

            object.applyDamage(frameDamage, 'fire');
        }
    }
    ...
    // Dragon Fury — single-shot radial inferno blast. Damages every enemy within `radius`
    ...
    detonateInferno(originX, originY, options = {}) {
        ...
        for (let i = 0, n = objs.length; i < n; i++) {
            ...
            if (obj === this.dragonTarget) continue;
    ...
    setDragonTarget(target) {
        this.dragonTarget = target || null;
    }
    ...
    update(delta) {
        ...
        const dragonPos = this.dragonTarget?.position;
        if (dragonPos) this.physicsWorld.setFocalPoint(dragonPos.x, dragonPos.y);
        ...
        if (!CONFIG.disableLevelObjectUpdate) {
            ...
            for (let i = 0, n = objs.length; i < n; i++) {
                ...
                if (!object.isOffScreen || object.alwaysUpdate) {
                    object.update(delta, this.level, this.dragonTarget, objs);
        ...
        this.applyTankBulletDamage(this.dragonTarget);
        ...
        if (this.level?.setDynamicCollisionEdges) {
            const dynamicEdges = [];
            const dPos = this.dragonTarget?.position;
            const objs = this.objects;
            for (let i = 0, n = objs.length; i < n; i++) {
                const object = objs[i];
        ...
        const dragon = this.dragonTarget;
        const groundEdge = dragon?.groundContact?.edge;
        if (groundEdge?._object && dragon?.onGround && !dragon?.carriedBy) {
            const platform = groundEdge._object;
            if (Number.isFinite(platform.frameDeltaX) || Number.isFinite(platform.frameDeltaY)) {
                dragon.position.x += platform.frameDeltaX ?? 0;
                dragon.position.y += platform.frameDeltaY ?? 0;

                // If the platform carried the dragon outside level bounds, slide them off.
                const lvl = this.level;
                if (lvl) {
                    const levelLeft  = lvl.worldOriginX ?? 0;
                    const levelRight = levelLeft + (lvl.width ?? 0) * (lvl.tileWidth ?? 1);
                    if (dragon.position.x < levelLeft || dragon.position.x > levelRight) {
                        dragon.position.x = Math.max(levelLeft, Math.min(levelRight, dragon.position.x));
                        dragon.setAirborneState?.();
                    }
                }
            }
        }
        ...
        // Feed dragon position/velocity before the physics update so pushLevelObjectsFromDragon
        // runs every substep rather than once per frame — reliable on slow/mobile devices.
        this.physicsWorld.setDragonState(
            this.dragonTarget?.position ?? null,
            this.dragonTarget?.velocity ?? null
        );
    ...
    updateChopperAudioLoop(delta = 0) {
        const targetPosition = this.dragonTarget?.position || null;
        const minDistance = 8;
    ...
    findNearestPickupableObject(position, radius, dragon) {
        let nearestObject = null;
        let nearestDistance = Number.POSITIVE_INFINITY;

        for (const object of this.queryNearbyObjects(position, radius)) {
            if (!object.canBePickedUpBy(dragon)) {
                continue;
            }
    ...
    getInteractionAnchorWorldPosition(object, dragon, useGrabPoints, scratch) {
        if (useGrabPoints && typeof object.getGrabPointWorldPosition === 'function' && typeof dragon?.selectDragGrabPoint === 'function') {
            const grabPointName = dragon.selectDragGrabPoint(object);
    ...
    findNearestInteractableObject(position, radius, dragon, useGrabPoints = false) {
        let nearestObject = null;
        let nearestDistance = Number.POSITIVE_INFINITY;
        ...
        for (const object of this.queryNearbyObjects(position, radius)) {
            ...
            if (!dragon?.canUsePickupDropButton?.(object)) continue;

            const anchorPos = this.getInteractionAnchorWorldPosition(object, dragon, useGrabPoints, this._scratchVec3);
