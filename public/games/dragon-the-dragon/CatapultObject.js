 ...
 *     tracks dragon proximity to drive the pull, and handles launch/release.
 *
 * Interaction model:
 *   1. Dragon flies above the catapult carrying an accepted object and drops it.
 *      The basket detects overlap and adopts the object as its loaded projectile.
 *   2. Dragon flies close to the loaded basket. While the dragon is inside
 *      `grabActivationRadius`, the basket position follows the dragon's position
 *      (clamped to `maxPullDistance` from rest).
 *   3. When the dragon moves outside the grab radius (or the basket reaches
 *      max stretch and the dragon keeps moving), the basket "releases" —
...
export class CatapultObject extends LevelObject {
    constructor(options) {
        ...
        // Constraint object pushed onto dragonTarget.positionConstraints while pulling.
        this._dragonConstraint = null;
        this._constrainedDragon = null;
    ...
    _registerDragonConstraint(dragon) {
        if (!dragon?.positionConstraints || this._dragonConstraint) return;
        this._constrainedDragon = dragon;
        this._dragonConstraint = {
            cx: this._restPosition.x,
            cy: this._restPosition.y,
            radius: this._catapultCfg.maxPullDistance
        };
        dragon.positionConstraints.push(this._dragonConstraint);
    }

    _unregisterDragonConstraint(dragon) {
        if (!this._dragonConstraint) return;
        const target = dragon ?? this._constrainedDragon;
        const arr = target?.positionConstraints;
        if (arr) {
            const idx = arr.indexOf(this._dragonConstraint);
    ...
    // by the dragon — caught mid-fall). Excluded: CARRIED/GRABBED/DRAGGED, and
    ...
    _tryDetectLoad(allObjects) {
        ...
        for (const obj of allObjects) {
            ...
            // Skip anything currently held by the dragon.
            if (
    ...
    // starts from the slingshot, not from wherever the dragon dropped it.
    ...
    update(delta, level, dragonTarget, allObjects) {
        ...
        // the dragon is close enough that a drop is plausible. This avoids iterating
        // all level objects every frame when the catapult is on-screen but unused.
        if (!this._loadedObject) {
            const dragonPos = dragonTarget?.position;
            const scanRadius = this._catapultCfg.maxPullDistance + this._catapultCfg.basketRadius + 4;
            const dragonNear = dragonPos &&
                Math.hypot(dragonPos.x - restPos.x, dragonPos.y - restPos.y) < scanRadius;
            if (dragonNear) {
                this._tryDetectLoad(allObjects);
            }
        }

        // 2) Grab is driven by the dragon's existing pickup/drag system.
        // While the dragon is holding the loaded object, the basket follows the
        // object's position (clamped to maxPullDistance from rest). When the
        // dragon releases the object, we fire it using the pull vector.
        if (this._loadedObject) {
            const obj = this._loadedObject;
            const isHeldByDragon = (
                obj.state === LEVEL_OBJECT_STATES.CARRIED ||
                obj.state === LEVEL_OBJECT_STATES.GRABBED ||
                obj.state === LEVEL_OBJECT_STATES.DRAGGED
            );
            ...
            if (isHeldByDragon) {
                ...
                this._registerDragonConstraint(dragonTarget);

                // Carried objects are re-parented to the dragon's carry socket, so
                ...
                // Dragon just dropped the loaded object — fire the catapult.
                this._unregisterDragonConstraint(dragonTarget);
                this._launch();
            } else {
    ...
    dispose() {
        this.trajectoryBounds = null;
        this._unregisterDragonConstraint(null);
        if (this._loadedObject) {
            this._releaseLoadedObject(0, 0);
        }
        if (this._basketNode) {
