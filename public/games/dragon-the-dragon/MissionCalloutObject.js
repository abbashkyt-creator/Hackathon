...
const ZONE_SHOW_RADIUS = 30;   // world units — zone appears when dragon is within this distance
 ...
 * behind the dragon and all foreground layers automatically.
 */
export class MissionCalloutObject extends LevelObject {
    ...
    update(delta, level, dragonTarget, allObjects) {
        ...
        // Zone visibility — show when callout is visible and dragon is nearby.
        if (this._zoneMesh) {
            let zoneVisible = false;
            if (visible && dragonTarget?.position) {
                const dx = dragonTarget.position.x - this.container.position.x;
                const dy = dragonTarget.position.y - this.container.position.y;
