...
// Keep dragon-specific tuning centralized so visuals and animation mapping stay easy to tweak.
export const DRAGON_MODEL_SETTINGS = {
    path: './gfx/mesh/dragon/dragon.glb',
    texturePath: './gfx/textures/dragon/dragon_texture.webp',
    // Render the dragon unlit so the texture colors stay exact and lighting does not darken it.
    useUnlitMonotoneMaterial: true,
    monotoneColor: '#ffffff',
    // Fixed dragon model scale in world space so every device uses the exact same size.
    modelScale: 8,
    // Fixed local base offset for the imported model.
    // Ground alignment is controlled explicitly with fitOffset.y + CONFIG.dragonGroundOffsetY.
    baseOffsetY: 0,
    // Manual world-space visual offsets. Y is intentionally a simple fixed value.
    // Keep the body in front of the ground layers even when the dragon turns on the Y axis.
...
const DRAGON_FIRE_SETTINGS = {
    // Plug your fire sprite texture path here (provided by user request).
    flameTextureUrl: './gfx/fire.webp',
    longPressThreshold: 0.18,
    fallbackMouthOffset: { x: 2.1, y: 1.15, z: 1.65 },
    // Degrees added to the bone-derived fire direction (positive = upward rotation).
    // Use this to compensate for the head's natural downward tilt at rest so that
    // idle/hover fire shoots forward instead of slightly downward.
    fireAngleOffsetDeg: 0,
    configOverrides: {
        maxParticles: 280,
        mouth: {
            // This rig's jaw axis is reversed for additive overlay; invert so fire opens wider.
            invert: true
        },
        // Local offset from jaw pivot for fire spawn origin (easy tuning knobs).
        // Increase X to push fire farther out from the mouth.
        // Increase Y to push fire higher relative to the jaw orientation.
        jawSpawnOffset: { x: -5, y: 0.0, z: 0 },
        burst: {
            duration: 0.18,
            immediateFlameCount: 14,
            immediateEmberCount: 8,
...
export const PLAYER_PRELOAD_ASSET_URLS = [
    DRAGON_MODEL_SETTINGS.path,
    DRAGON_MODEL_SETTINGS.texturePath,
    DRAGON_FIRE_SETTINGS.flameTextureUrl
];

// Kick off dragon asset fetches the moment this module is imported — before init(),
// before cloud sync, before level JSON. The browser caches the responses so when
// the GLTF/texture loaders request the same URLs they hit cache instead of the network.
// The 'no-store' case is intentionally ignored: if the server sends no-store headers
// this won't help, but it won't hurt either.
for (const url of PLAYER_PRELOAD_ASSET_URLS) {
...
export class Player {
    constructor(scene, ground, joystick, options = {}) {
        ...
        this.dragonFlightTurnPivot = new THREE.Group();

        this.dragonTiltPivot = new THREE.Group();
        this.dragonTiltPivot.position.y = -PLAYER_RADIUS;

        this.dragonTurnPivot = new THREE.Group();
        this.dragonTurnPivot.position.y = PLAYER_RADIUS;

        this.dragonCollisionAnchor = new THREE.Group();
        this.dragonCollisionAnchor.name = 'DragonCollisionAnchor';

        this.dragonFacingPivot = new THREE.Group();
        this.dragonFacingPivot.visible = false;

        this.dragonFitRoot = new THREE.Group();
        this.dragonFitRoot.rotation.set(
            DRAGON_MODEL_SETTINGS.extraRotation.x,
            DRAGON_MODEL_SETTINGS.extraRotation.y,
            DRAGON_MODEL_SETTINGS.extraRotation.z
        );

        this.dragonFacingPivot.add(this.dragonFitRoot);
        this.dragonTurnPivot.add(this.dragonCollisionAnchor);
        this.dragonTurnPivot.add(this.dragonFacingPivot);
        this.dragonTiltPivot.add(this.dragonTurnPivot);
        this.dragonFlightTurnPivot.add(this.dragonTiltPivot);
        this.mesh.add(this.dragonFlightTurnPivot);
...
//        const spawnY = Number.isFinite(spawnConfig.y) ? spawnConfig.y : 0;
        ...
        this.maxLiftWeight = Number.isFinite(CONFIG.DRAGON_CARRY?.maxLiftWeight)
            ? CONFIG.DRAGON_CARRY.maxLiftWeight
            : 0;
        this.maxDragWeight = Number.isFinite(CONFIG.DRAGON_DRAG?.maxDragWeight)
            ? CONFIG.DRAGON_DRAG.maxDragWeight
            : 0;
        ...
        this.firePressDuration = 0;
        this.fireLongPressTriggered = false;
        ...
        this.rewardedFlameBoostActive = false;
        this.flameDepletedCount = 0;
        this.rewardedSpeedBoostActive = false;
        this.energyDepletedCount = 0;
        this.maxFlameValue = Math.max(
            0,
            Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.maxFlameValue)
                ? CONFIG.DRAGON_FIRE_COMBAT.maxFlameValue
                : 100
        );
        this.flameDrainValue = Math.max(
            0,
            Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.flameDrainValue)
                ? CONFIG.DRAGON_FIRE_COMBAT.flameDrainValue
                : 25
        );
        this.flameFillSpeed = Math.max(
            0,
            Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.flameFillSpeed)
                ? CONFIG.DRAGON_FIRE_COMBAT.flameFillSpeed
                : 15
        );
        this.flameEmptyDuration = Math.max(
            0,
            Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.flameEmptyDuration)
                ? CONFIG.DRAGON_FIRE_COMBAT.flameEmptyDuration
                : 0
        );
        this.flameEmptyTimer = 0;
        this.currentFlameValue = this.maxFlameValue;
        this.lastFlameDrainAmount = 0;
        this.maxEnergyValue = Math.max(
            0,
            Number.isFinite(CONFIG.DRAGON_ENERGY_BOOST?.maxEnergyValue)
                ? CONFIG.DRAGON_ENERGY_BOOST.maxEnergyValue
                : 100
        );
        this.energyDrainValue = Math.max(
            0,
            Number.isFinite(CONFIG.DRAGON_ENERGY_BOOST?.energyDrainValue)
                ? CONFIG.DRAGON_ENERGY_BOOST.energyDrainValue
                : 25
        );
        this.energyFillSpeed = Math.max(
            0,
            Number.isFinite(CONFIG.DRAGON_ENERGY_BOOST?.energyFillSpeed)
                ? CONFIG.DRAGON_ENERGY_BOOST.energyFillSpeed
                : 15
        );
        this.energyEmptyDuration = Math.max(
            0,
            Number.isFinite(CONFIG.DRAGON_ENERGY_BOOST?.energyEmptyDuration)
                ? CONFIG.DRAGON_ENERGY_BOOST.energyEmptyDuration
                : 0
        );
        this.energyEmptyTimer = 0;
        this.energySpeedMultiplier = Math.max(
            1,
            Number.isFinite(CONFIG.DRAGON_ENERGY_BOOST?.energySpeedMultiplier)
                ? CONFIG.DRAGON_ENERGY_BOOST.energySpeedMultiplier
                : 1.6
        );
        ...
        this.maxHealthValue = Math.max(
            0,
            Number.isFinite(CONFIG.DRAGON_HEALTH?.maxHealthValue)
                ? CONFIG.DRAGON_HEALTH.maxHealthValue
                : 100
        );
        this.healthFillSpeed = Math.max(
            0,
            Number.isFinite(CONFIG.DRAGON_HEALTH?.healthFillSpeed)
                ? CONFIG.DRAGON_HEALTH.healthFillSpeed
                : 0
        );
        this.currentHealthValue = this.maxHealthValue;
        this.lastMissileDamageTime = -Infinity;
        this.gameOverReady = false;
        // Dragon Fury ultimate charge (0..1): fills while breathing flame plus a slow passive
        ...
        this.debugMatterDragonAnchorLines = new Map();
    ...
    setupDebugHitRect() {
        ...
        this.debugHitRectFill.name = 'DragonCollisionShapeDebugFill';
        ...
        this.debugHitRectLine.name = 'DragonCollisionShapeDebugOutline';
        ...
        this.debugCarryRectWorldLine.name = 'DragonCarryRectWorldDebugOutline';
        ...
        this.debugCarryRectReprojectedLine.name = 'DragonCarryRectReprojectedDebugOutline';
        ...
        this.debugCarryRectConnectorLine.name = 'DragonCarryRectComparisonConnectors';
    ...
    getCarryRectDebugComparisonWorldPolygons() {
        ...
        const transform = this.getDragonCollisionTransform();
        ...
        const reprojectedRectWorldPoints = orderedRectWorldPoints
            .map((point) => this.transformWorldPointToDragonLocal(point, transform))
            .filter(Boolean)
            .map((point) => this.transformDragonLocalPointToWorld(point, transform))
            .filter(Boolean);
    ...
    updateDebugMatterDragonAnchors() {
        if (CONFIG.LEVEL_OBJECTS?.debugRenderMatterPhysics !== true) {
            this.disposeDebugMatterDragonAnchors();
            return;
        }
        ...
        for (const marker of markerSpecs) {
            ...
            let line = this.debugMatterDragonAnchorLines.get(marker.key);
            ...
            if (!line) {
                ...
                line.name = `DragonMatterAnchorDebug:${marker.key}`;
                line.renderOrder = 1000015;
                line.frustumCulled = false;
                this.scene.add(line);
                this.debugMatterDragonAnchorLines.set(marker.key, line);
            } else {
        ...
        for (const [key, line] of [...this.debugMatterDragonAnchorLines.entries()]) {
            ...
            this.debugMatterDragonAnchorLines.delete(key);
        }
    }

    disposeDebugMatterDragonAnchors() {
        for (const line of this.debugMatterDragonAnchorLines.values()) {
            line.geometry?.dispose?.();
            line.material?.dispose?.();
            line.removeFromParent?.();
        }
        this.debugMatterDragonAnchorLines.clear();
    }

    updateDebugHitRect() {
        this.updateDebugMatterDragonAnchors();
    ...
    setupDebugCollisionOverlay() {
        ...
        this.debugCollisionOverlay.name = 'DragonCollisionOverlayDebug';
    ...
    setRenderOrder(renderOrder) {
        ...
        this.dragonFire?.setRenderOrder(renderOrder);
    ...
    loadDragon() {
        loaderLoadWithRetry(this.loader, DRAGON_MODEL_SETTINGS.path)
            .then((gltf) => this.handleDragonLoaded(gltf))
            .catch((error) => {
                const detail = error instanceof Error
                    ? error.message
                    : (error?.message || error?.url || error?.filename || String(error));
                console.error(`[Player] Failed to load dragon.glb: ${detail}`, error);
            });
    }

    handleDragonLoaded(gltf) {
        this.dragonModel = gltf.scene;
        this.applyDragonTexture(this.dragonModel);
        this.prepareDragonModel(this.dragonModel);
        this.applyInitialDragonNodeVisibility();
        this.collectDragonMaterials();
        this.setupAnimations(gltf.animations);
        this.setupCarrySocket();

        this.dragonFacingPivot.visible = true;
        this.updateFacingDirection(0);
        this.updateGroundAlignment(0);
        this.setupDragonFireBreath();

//        console.info('[Player] Dragon model loaded:', DRAGON_MODEL_SETTINGS.path);
    }

    findDragonNodesByName(name) {
        if (!this.dragonModel || typeof name !== 'string' || !name.trim()) {
            return [];
        }
    ...
    setDragonNodeVisible(name, visible) {
        const matches = this.findDragonNodesByName(name);
        if (!matches.length) {
            console.warn(`[Player] Dragon model node not found: "${name}"`);
            return false;
        }
    ...
    setDragonNodeVisibility(nodeVisibility = {}) {
        ...
        for (const [name, visible] of Object.entries(nodeVisibility)) {
            changed = this.setDragonNodeVisible(name, visible === true) || changed;
        }
        return changed;
    }

    setModelNodeVisibility(nodeVisibility = {}) {
        return this.setDragonNodeVisibility(nodeVisibility);
    }
    ...
    showModelNodes(nodeNames = []) {
        ...
        for (const name of nodeNames) {
            changed = this.setDragonNodeVisible(name, true) || changed;
        }
        return changed;
    }
    ...
    hideModelNodes(nodeNames = []) {
        ...
        let changed = false;
        for (const name of nodeNames) {
            changed = this.setDragonNodeVisible(name, false) || changed;
        }
        return changed;
    }

    applyInitialDragonNodeVisibility() {
        const hiddenNodes = CONFIG.DRAGON_MODEL?.hiddenNodesOnLoad;
    ...
    setupDragonFireBreath() {
        const fallbackMouthOffset = new THREE.Vector3(
            DRAGON_FIRE_SETTINGS.fallbackMouthOffset.x,
            DRAGON_FIRE_SETTINGS.fallbackMouthOffset.y,
            DRAGON_FIRE_SETTINGS.fallbackMouthOffset.z
        );
        const mouthObject = this.findDragonMouthObject();
        this.mouthObject = mouthObject;
        this.setupMouthSocket(mouthObject);

        this._neckBone = this.findDragonBone('neck1');
        this._appliedNeckRaiseOffset = 0;
        this._appliedNeckRaiseQuat = null;
        this._dragHeadRaiseT = 0;
        ...
        this.dragonFire = new DragonFireBreath(
            ...
            DRAGON_FIRE_SETTINGS.flameTextureUrl,
            {
                renderOrder: this.visualRenderOrder,
                mouthOffset: fallbackMouthOffset,
                combat: {
                    fireballHitRadius: Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.fireballHitRadius)
                        ? CONFIG.DRAGON_FIRE_COMBAT.fireballHitRadius
                        : 0.9,
                    fireballImpactParticleCount: Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.fireballImpactParticleCount)
                        ? CONFIG.DRAGON_FIRE_COMBAT.fireballImpactParticleCount
                        : 8,
                    fireballImpactDuration: Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.fireballImpactDuration)
                        ? CONFIG.DRAGON_FIRE_COMBAT.fireballImpactDuration
                        : 0.25,
                    fireballImpactScale: Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.fireballImpactScale)
                        ? CONFIG.DRAGON_FIRE_COMBAT.fireballImpactScale
                        : 0.5,
                    fireballImpactSpeedMin: Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.fireballImpactSpeedMin)
                        ? CONFIG.DRAGON_FIRE_COMBAT.fireballImpactSpeedMin
                        : 4.5,
                    fireballImpactSpeedMax: Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.fireballImpactSpeedMax)
                        ? CONFIG.DRAGON_FIRE_COMBAT.fireballImpactSpeedMax
                        : 10.5,
                    flameHitboxLength: Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.flameHitboxLength)
                        ? CONFIG.DRAGON_FIRE_COMBAT.flameHitboxLength
                        : 9,
                    flameHitboxWidth: Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.flameHitboxWidth)
                        ? CONFIG.DRAGON_FIRE_COMBAT.flameHitboxWidth
                        : 2.4,
                    flameHitboxForwardOffset: Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.flameHitboxForwardOffset)
                        ? CONFIG.DRAGON_FIRE_COMBAT.flameHitboxForwardOffset
                        : 0.8,
                    flameSurfaceEpsilon: Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.flameSurfaceEpsilon)
                        ? CONFIG.DRAGON_FIRE_COMBAT.flameSurfaceEpsilon
                        : 0.35,
                    clipFlameToStaticPolygons: CONFIG.DRAGON_FIRE_COMBAT?.clipFlameToStaticPolygons !== false,
                    blockFireInsideStaticPolygons: CONFIG.DRAGON_FIRE_COMBAT?.blockFireInsideStaticPolygons !== false
                },
                ...DRAGON_FIRE_SETTINGS.configOverrides
            },
            this.loadingManager
        );
        this.dragonFire.setTerrainProvider(this.ground || null);
        this.dragonFire.setFacingDirection(this.lastFacingDirection);
        this.dragonFire.setRenderOrder(this.visualRenderOrder);
        this.dragonFire.fireAngleOffsetRad = THREE.MathUtils.degToRad(DRAGON_FIRE_SETTINGS.fireAngleOffsetDeg ?? 0);

        if (mouthObject) {
//            console.info('[Player] Fire breath mouth socket:', mouthObject.name || '(unnamed object)');
        } else {
//            console.info('[Player] Fire breath using fallback mouth offset:', fallbackMouthOffset.toArray());
        }
    }
    ...
    setupMouthSocket(mouthObject) {
        ...
        this.mouthSocket.name = 'DragonMouthSocket';
    ...
    setupCarrySocket() {
        ...
        const footBone = this.findDragonBone('grab');
        this.carrySocket = new THREE.Group();
        this.carrySocket.name = 'DragonCarrySocket';

        // Carrying is anchored to the authored grab bone so phase 1 pickups follow the real
        // dragon animation without requiring any changes to the imported dragon rig or GLBs.
        if (footBone) {
...
//            console.info('[Player] Carry socket attached to dragon bone: grab');
            return;
        }

        this.dragonFitRoot.add(this.carrySocket);
    ...
    updateCarrySocketScaleCompensation() {
        ...
        // The carry socket sits under the scaled dragon rig. Counter-scaling the socket keeps
    ...
    // Swaps the dragon's texture at runtime (called by DragonSkinShop on equip).
    setDragonTexture(texturePath) {
        DRAGON_MODEL_SETTINGS.texturePath = texturePath;
        if (!this.dragonModel) return; // model not loaded yet — texturePath is set above so handleDragonLoaded will pick it up
        this.applyDragonTexture(this.dragonModel);
    }

    applyDragonTexture(dragonModel) {
...
//                console.info('[Player] Dragon texture applied:', DRAGON_MODEL_SETTINGS.texturePath);
            },
            undefined,
            (error) => {
                console.warn('[Player] Failed to load dragon texture, keeping original materials.', error);
            }
        );
    }

    prepareDragonModel(dragonModel) {
        this.dragonFitRoot.clear();
        this.dragonFitRoot.rotation.set(
            DRAGON_MODEL_SETTINGS.extraRotation.x,
            DRAGON_MODEL_SETTINGS.extraRotation.y,
            DRAGON_MODEL_SETTINGS.extraRotation.z
        ...
        dragonModel.traverse((child) => {
            ...
            const nextMaterials = sourceMaterials.map((material) => {
                ...
                if (!DRAGON_MODEL_SETTINGS.useUnlitMonotoneMaterial) {
                    // Let the dragon self-occlude correctly so wings and body parts do not
                ...
                const unlitMaterial = new THREE.MeshBasicMaterial({
                    color: new THREE.Color(DRAGON_MODEL_SETTINGS.monotoneColor),
        ...
        this.dragonFitRoot.add(dragonModel);

        // Use one explicit model scale instead of runtime auto-fit scaling.
        dragonModel.scale.setScalar(DRAGON_MODEL_SETTINGS.modelScale);
        this.dragonFitRoot.updateMatrixWorld(true);

        const fittedBounds = this.getStaticModelBounds(this.dragonFitRoot);
        const fittedCenter = new THREE.Vector3();
        fittedBounds.getCenter(fittedCenter);

        this.dragonFitRoot.position.set(
            DRAGON_MODEL_SETTINGS.fitOffset.x - fittedCenter.x,
            DRAGON_MODEL_SETTINGS.baseOffsetY + DRAGON_MODEL_SETTINGS.fitOffset.y + CONFIG.dragonGroundOffsetY,
            DRAGON_MODEL_SETTINGS.fitOffset.z - fittedCenter.z
        );
        this.dragonFitRoot.updateMatrixWorld(true);
    }

    collectDragonMaterials() {
        this.dragonMaterialStates = [];
        if (!this.dragonModel) {
            return;
        }
        ...
        this.dragonModel.traverse((child) => {
            ...
            for (const material of materials) {
                ...
                this.dragonMaterialStates.push({
                    material,
...
//        console.info('[Player] Dragon clips found:', clipNames);

        if (!clips.length) {
            return;
        }

        this.animationMixer = new THREE.AnimationMixer(this.dragonModel);
...
//        console.info('[Player] Chosen dragon clips per state:', chosenClips);
        this.playLoopAnimation(this.getPreferredLoopState());
    }

    ensureRequiredAnimationClips() {
        if (!this.animationActions.drag) {
            throw new Error('[Player] Missing required dragon animation clip: drag-loop (state "drag").');
        }
        if (!this.animationActions.dragIdle) {
            throw new Error('[Player] Missing required dragon animation clip: drag_idle-loop (state "dragIdle").');
        }
    }
    ...
    buildStateClipMap(clips) {
        ...
        for (const state of Object.keys(DRAGON_MODEL_SETTINGS.clipOverrides)) {
    ...
    findClipForState(state, clipEntries) {
        const overrideName = DRAGON_MODEL_SETTINGS.clipOverrides[state];
        if (overrideName) {
        ...
        const hints = DRAGON_MODEL_SETTINGS.clipHints[state] || [];
    ...
    getCombinedInput() {
        ...
        if (this.isAutoInteractionActive()) {
            ...
            // the dragon away while grab or the mouth is being aligned to an object anchor.
            out.x = 0; out.y = 0;
            return out;
        }
    ...
    canUseFlame() {
        ...
        if (this.rewardedFlameBoostActive === true) {
            return true;
        }
        return this.flameEmptyTimer <= 0 && this.currentFlameValue > 0.0001;
    }

    setRewardedFlameBoostActive(isActive) {
        const nextActive = Boolean(isActive);
        if (this.rewardedFlameBoostActive === nextActive) {
            return;
        }

        this.rewardedFlameBoostActive = nextActive;
        if (nextActive) {
            this.currentFlameValue = this.maxFlameValue;
        } else if (this.currentFlameValue <= 0.0001 && this.isFlameActive()) {
            this.stopFlame();
        }
    }

    isFlameActive() {
        return this.dragonFire?.isContinuousFireActive?.() === true;
    }

    startFlame() {
        if (!this.dragonFire || !this.canUseFlame()) {
            return false;
        }

        const wasActive = this.isFlameActive();
        this.dragonFire.startContinuousFire();
        const isActive = this.isFlameActive();
    ...
    updateFuryCharge(dt) {
        ...
        if (this.lastFlameDrainAmount > 0 && this.flameDrainValue > 0) {
            gain += (this.lastFlameDrainAmount / this.flameDrainValue) * (fury.chargePerFlameSecond ?? 0);
        }
    ...
    consumeFlameResource(amount) {
        ...
        const previousValue = this.currentFlameValue;
        this.currentFlameValue = Math.max(0, previousValue - amount);
        const drainedAmount = Math.max(0, previousValue - this.currentFlameValue);
        this.lastFlameDrainAmount += drainedAmount;
    ...
    updateFlameResource(delta) {
        ...
        this.lastFlameDrainAmount = 0;
        const maxFlame = Math.max(this.maxFlameValue, 0);
        if (maxFlame <= 0) {
        ...
        if (this.rewardedFlameBoostActive) {
            this.currentFlameValue = maxFlame;
        } else if (this.isFlameActive()) {
            // Continuous flame is the only attack that consumes this resource.
            this.consumeFlameResource(this.flameDrainValue * delta);
            if (this.currentFlameValue <= 0) {
                this.currentFlameValue = 0;
                this.flameDepletedCount += 1;
                this.flameEmptyTimer = this.flameEmptyDuration;
                this.stopFlame();
            }
        } else {
            // Hold off refill while the empty-penalty timer is still counting down.
            if (this.flameEmptyTimer > 0) {
                this.flameEmptyTimer = Math.max(0, this.flameEmptyTimer - delta);
            } else {
                // Automatic refill when not actively breathing continuous flame.
                this.currentFlameValue += this.flameFillSpeed * delta;
            }
        }

        this.currentFlameValue = THREE.MathUtils.clamp(this.currentFlameValue, 0, maxFlame);
    }

    getFlameDepletedCount() {
        return Math.max(0, Number.isFinite(this.flameDepletedCount) ? this.flameDepletedCount : 0);
    }
    ...
    isTryingFlameWhileEmpty() {
        if (this.rewardedFlameBoostActive) return false;
        return this.isFlameButtonDown() && !this.canUseFlame();
    }

    isTryingSpeedWhileEmpty() {
        if (this.rewardedSpeedBoostActive) return false;
        return this.isEnergyBoostButtonDown() &&
            this.hasEnergyBoostMovementIntent() &&
            !this.canUseEnergyBoost();
    }
    ...
    getFlameProgress() {
        if (this.maxFlameValue <= 0) {
            return 0;
        }

        // UI progress is normalized from current value over max value.
        return THREE.MathUtils.clamp(this.currentFlameValue / this.maxFlameValue, 0, 1);
    }
    ...
    hasEnergyBoostVelocity() {
        // Drain/boost only while the dragon is actually moving in any direction.
    ...
    applyDamage(amount, type = 'generic', options = {}) {
        ...
        if (type === 'missile') {
            ...
            const cooldownSeconds = Math.max(
                0,
                CONFIG.DRAGON_HEALTH?.missileDamageCooldownSeconds ?? 0.35
            );
        ...
        this.spawnDragonHitImpactEffect(options.impactPosition, options.projectileDirection);
        this.triggerDragonHitFlash();
        this.audioManager?.play?.('dragonHit', { volume: 0.85, cooldown: 0.08 });
    ...
    triggerDragonHitFlash() {
        if (!this.dragonMaterialStates.length) {
            return;
        }
        ...
        this.updateDragonHitFlash(0);
    }

    updateDragonHitFlash(delta) {
        if (!this.hitFlashActive) {
            return;
        }

        const flashConfig = CONFIG.DRAGON_HIT_FLASH || {};
        ...
        for (const state of this.dragonMaterialStates) {
            ...
            if (state.baseEmissive && material.emissive) {
                ...
                // MeshBasicMaterial has no emissive channel. Since the dragon's normal color is
        ...
        if (progress >= 1) {
            this.restoreDragonHitFlashMaterials();
        }
    }

    restoreDragonHitFlashMaterials() {
        for (const state of this.dragonMaterialStates) {
            const material = state.material;
            if (!material) {
                continue;
    ...
    spawnDragonHitImpactEffect(impactPosition, projectileDirection) {
        if (!this.dragonFire?.spawnFireballImpactEffect) {
            return false;
        }

        const direction = this.normalizeHitPushDirection(projectileDirection);
        const impactConfig = CONFIG.DRAGON_HIT_IMPACT || {};
        const fallbackOffset = Math.max(impactConfig.dragonHitImpactFallbackOffset ?? 0.85, 0);
        ...
        if (
            ...
            resolvedPosition.set(
                impactPosition.x,
                impactPosition.y,
                Number.isFinite(impactPosition.z) ? impactPosition.z : this.position.z
            );
        } else {
            // Projectiles should usually provide the collision point. If they do not, place the
            // spark on the side of the dragon facing the incoming projectile direction.
            resolvedPosition.copy(this.position);
            if (direction) {
                resolvedPosition.x -= direction.x * fallbackOffset;
                resolvedPosition.y -= direction.y * fallbackOffset;
            }
        }

        const outwardNormal = direction
            ? { x: -direction.x, y: -direction.y }
            : null;

        // This reuses the fireball impact pool, but with smaller/faster settings so dragon hits
        // read as contact sparks instead of vehicle destruction blasts.
        this.dragonFire.spawnFireballImpactEffect(resolvedPosition, outwardNormal, {
            scale: impactConfig.dragonHitImpactScale ?? 0.5,
            duration: impactConfig.dragonHitImpactDuration ?? 0.25,
            particleCount: impactConfig.dragonHitImpactParticleCount ?? 8
        });
        return true;
    }

    computeHitPushDistance(damage) {
        const pushConfig = CONFIG.DRAGON_HIT_PUSH || {};
    ...
    getHitPushStepCount() {
        return Math.max(1, Math.floor(CONFIG.DRAGON_HIT_PUSH?.hitPushStepCount ?? 6));
    }
    ...
    resolveHitPushAgainstCollision(offset) {
        ...
        // solids. Each accepted step leaves the dragon in a valid placement.
        for (let step = 0; step < steps; step += 1) {
            ...
            if (!resolvedPosition || !this.isValidDragonPosition(resolvedPosition, {
    ...
    isValidDragonPosition(position, options = {}) {
    ...
    resetTimelineVisualPivots() {
        this.dragonFlightTurnPivot.rotation.set(0, 0, 0);
        this.dragonTiltPivot.rotation.set(0, 0, 0);
        this.dragonTurnPivot.rotation.set(0, 0, 0);
        this.dragonFacingPivot.rotation.set(0, 0, 0);
    }
    ...
    onTimelineTransformUpdated(target = this.mesh) {
        ...
        // so dragonFire.setFacingDirection (called every frame) stays correct.
        const leftYaw = DRAGON_MODEL_SETTINGS.facingYaw.left;
        const rightYaw = DRAGON_MODEL_SETTINGS.facingYaw.right;
    ...
    setTimelineAnimationControlled(isControlled) {
        ...
        // Preserve the facing direction the timeline left the dragon in.
        ...
        this.lastFacingDirection = distToLeft <= distToRight ? -1 : 1;
        this.dragonFacingPivot.rotation.y = this.lastFacingDirection > 0 ? rightYaw : leftYaw;
    ...
    resetForLevel(level) {
        ...
        this.fireLongPressTriggered = false;
        this.stopFlame();
        this.dragonFire?.clearTransientEffects?.();
    ...
    playDeadGroundAnimation() {
        ...
        this.transitionAction = null;
        this.queuedLoopState = null;
        this.playAnimation(resolvedState, DRAGON_MODEL_SETTINGS.fadeDuration, true);
    ...
    getDragonCollisionAnchorWorldPosition(target = new THREE.Vector3()) {
        const anchor = this.dragonCollisionAnchor || this.mesh;
    ...
    getWorldCollisionCircle() {
        ...
        const worldPosition = this.getDragonCollisionAnchorWorldPosition(new THREE.Vector3());
        return {
    ...
    getDragonCollisionTransform2(carriedRect) 
    {        
        const facingSign = this.lastFacingDirection >= 0 ? 1 : -1;     
...
/*    
    getDragonCollisionTransform2() {
        const facingSign = this.lastFacingDirection >= 0 ? 1 : -1;
        const socket = this.carrySocket;

        if (!socket) {
            return this.getDragonCollisionTransform();
        }
        ...
        // Result uses the same sign convention as getDragonCollisionTransform:
...
*/
    transformDragonLocalPointToWorld(localPoint, transform = this.getDragonCollisionTransform()) {
    ...
    transformDragonLocalVectorToWorld(localPoint, transform = this.getDragonCollisionTransform()) {
        if (!localPoint || !transform) {
            return null;
        }
    ...
    transformWorldPointToDragonLocal(worldPoint, transform = this.getDragonCollisionTransform()) {
       ...
       const transform = this.getDragonCollisionTransform();
        ...
        for (let index = 0; index < localPolygon.length; index += 1) {
            ...
            offsets.push(circleAnchorOffset.clone().add(
                this.transformDragonLocalVectorToWorld(point, transform)
            ));

            const nextPoint = localPolygon[(index + 1) % localPolygon.length];
            offsets.push(circleAnchorOffset.clone().add(
                this.transformDragonLocalVectorToWorld(point.clone().lerp(nextPoint, 0.25), transform)
            ));
            offsets.push(circleAnchorOffset.clone().add(
                this.transformDragonLocalVectorToWorld(point.clone().lerp(nextPoint, 0.5), transform)
            ));
            offsets.push(circleAnchorOffset.clone().add(
                this.transformDragonLocalVectorToWorld(point.clone().lerp(nextPoint, 0.75), transform)
            ));
        }

        offsets.push(circleAnchorOffset.clone().add(
            this.transformDragonLocalVectorToWorld(
                new THREE.Vector2(
    ...
    orderRectPointsForCarryPolygon(worldPoints, transform = this.getDragonCollisionTransform()) {
        ...
        const localPoints = worldPoints
            .map((point) => ({
                worldPoint: point,
                localPoint: this.transformWorldPointToDragonLocal(point, transform)
            }))
    ...
    buildCarriedFlightCollisionLocalPolygon() {
        ...
        const transform = this.getDragonCollisionTransform2(carriedRect);
        const rectWorldPoints = this.getRectWorldPoints(carriedRect, false);
        const orderedRectWorldPoints = this.orderRectPointsForCarryPolygon(rectWorldPoints, transform);
        const {
            maxCornerDrift,
        ...
        this.maybeLogCarryFlightCollisionDebug({
            ...
            // world->dragon-local conversion for the carried rect corners.
//              orderedRectLocalPoints: orderedRectWorldPoints.map((point) => this.formatDebugVector2(point)),
    ...
    canUseFireButton() {
        return !this.isInputLocked() && !this.isDraggingObject() && !this.isInWater;
    }
    ...
    getBackMostVisualZ() {
        ...
        this.dragonModel.updateWorldMatrix(true, true);
        this.dragLayerBounds.setFromObject(this.dragonModel);
    ...
    getCarryFlapSpeedMultiplier() {
        ...
        const weightFactor = Number.isFinite(CONFIG.DRAGON_CARRY?.flapSpeedWeightFactor)
            ? CONFIG.DRAGON_CARRY.flapSpeedWeightFactor
            : 0;
    ...
    isCarryHoverOnlyActive() {
        ...
        const threshold = CONFIG.DRAGON_CARRY?.freeFlyCarryWeightThreshold ?? 10;
    ...
    isAirbornePickupEligible() {
        ...
        // beginAutoPickup will force the dragon into hover to complete the approach.
    ...
    canPickupObject(levelObject = null, options = {}) {
        ...
        // Don't allow pickup if the dragon is more than 1 tile below the object's collision bottom.
        ...
        // grab point is below the dragon's circle bottom — prevents grabbing the block underfoot.
    ...
    getCarryHoverSpeedMultiplier() {
        return Number.isFinite(CONFIG.DRAGON_CARRY?.carryHoverSpeedMultiplier)
            ? CONFIG.DRAGON_CARRY.carryHoverSpeedMultiplier
            : 1;
    }
    ...
    canDragObject(levelObject = null, options = {}) {
        ...
        // When grounded on a dynamic walkable object, reject grab nodes below the dragon's
        ...
        // moving — beginAutoDrag will walk the dragon back to the correct grab position).
        if (options.allowDuringAuto || options.skipMouthSideCheck) {
            return true;
        }

        // Require the dragon to be within 1.5 tiles vertically of the grab node.
        const grabPointName = this.selectDragGrabPoint(levelObject);
        if (!this._scratchVec3B) this._scratchVec3B = new THREE.Vector3();
        const grabWorld = levelObject.getPhysicsAnchorWorldPosition?.(grabPointName, this._scratchVec3B) ||
            levelObject.getGrabPointWorldPosition?.(grabPointName, this._scratchVec3B);
    ...
    isMouthOnFacingSideOfDragGrabPoint(levelObject) {
        const dragonFacing = this.lastFacingDirection >= 0 ? 1 : -1;
        const grabPointName = this.selectDragGrabPoint(levelObject);
    ...
    getPickupAlignSpeed() {
        return Math.max(
            0.001,
            Number.isFinite(CONFIG.DRAGON_CARRY?.pickupAlignSpeed)
                ? CONFIG.DRAGON_CARRY.pickupAlignSpeed
                : 18
        );
    }
    ...
    getPickupAlignTolerance() {
        return Math.max(
            0.001,
            Number.isFinite(CONFIG.DRAGON_CARRY?.pickupAlignTolerance)
                ? CONFIG.DRAGON_CARRY.pickupAlignTolerance
                : 0.25
        );
    }
    ...
    getPickupMaxDuration() {
        return Math.max(
            0.001,
            Number.isFinite(CONFIG.DRAGON_CARRY?.pickupMaxDuration)
                ? CONFIG.DRAGON_CARRY.pickupMaxDuration
                : 1
        );
    }
    ...
    getDragAlignSpeed() {
        return Math.max(
            0.001,
            Number.isFinite(CONFIG.DRAGON_DRAG?.alignSpeed)
                ? CONFIG.DRAGON_DRAG.alignSpeed
                : 14
        );
    }
    ...
    getDragAlignTolerance() {
        return Math.max(
            0.001,
            Number.isFinite(CONFIG.DRAGON_DRAG?.alignTolerance)
                ? CONFIG.DRAGON_DRAG.alignTolerance
                : 0.35
        );
    }
    ...
    getDragAlignMaxDuration() {
        return Math.max(
            0.001,
            Number.isFinite(CONFIG.DRAGON_DRAG?.alignMaxDuration)
                ? CONFIG.DRAGON_DRAG.alignMaxDuration
                : 1
        );
    }
    ...
    getDragGrabOvershootDistance() {
        return Math.max(
            0,
            Number.isFinite(CONFIG.DRAGON_DRAG?.grabOvershootDistance)
                ? CONFIG.DRAGON_DRAG.grabOvershootDistance
                : 0
        );
    }
    ...
    getDragMovementMultiplierForWeight(weight) {
        const backwardSpeedMultiplier = THREE.MathUtils.clamp(
            Number.isFinite(CONFIG.DRAGON_DRAG?.backwardSpeedMultiplier)
                ? CONFIG.DRAGON_DRAG.backwardSpeedMultiplier
                : 0.8,
            0,
            1
        );
        const maxDragWeight = Math.max(
            0.001,
            Number.isFinite(CONFIG.DRAGON_DRAG?.maxDragWeight)
                ? CONFIG.DRAGON_DRAG.maxDragWeight
                : this.maxDragWeight || 260
        );
        const minMultiplier = THREE.MathUtils.clamp(
            Number.isFinite(CONFIG.DRAGON_DRAG?.minMovementMultiplier)
                ? CONFIG.DRAGON_DRAG.minMovementMultiplier
                : 0.12,
            0,
            1
        );
    ...
    getDragAnimationSpeedMultiplier() {
        ...
        const dragConfig = CONFIG.DRAGON_DRAG || {};
    ...
    getDragBackwardMaxWalkSpeed() {
        const baseSpeed = Math.max(
            0,
            Number.isFinite(CONFIG.DRAGON_DRAG?.backwardMaxWalkSpeed)
                ? CONFIG.DRAGON_DRAG.backwardMaxWalkSpeed
    ...
    getMouthWorldPosition(target = new THREE.Vector3()) {
        ...
        target.copy(DRAGON_FIRE_SETTINGS.fallbackMouthOffset);
    ...
    getFireballDamage() {
        return Math.max(
            0,
            Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.fireballDamage)
                ? CONFIG.DRAGON_FIRE_COMBAT.fireballDamage
                : 0
        );
    }
    ...
    getFireballCooldown() {
        return Math.max(
            0,
            Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.fireballCooldown)
                ? CONFIG.DRAGON_FIRE_COMBAT.fireballCooldown
                : 0
        );
    }

    updateFireballCooldown(dt) {
        if (this.fireballCooldownRemaining <= 0) {
            return;
        }

        this.fireballCooldownRemaining = Math.max(0, this.fireballCooldownRemaining - Math.max(0, dt));
    }

    canShootFireball() {
        return this.fireballCooldownRemaining <= 0;
    }

    resetFireballCooldown() {
        this.fireballCooldownRemaining = this.getFireballCooldown();
    }

    getFlameDamagePerSecond() {
        return Math.max(
            0,
            Number.isFinite(CONFIG.DRAGON_FIRE_COMBAT?.flameDamagePerSecond)
                ? CONFIG.DRAGON_FIRE_COMBAT.flameDamagePerSecond
                : 0
        );
    }
    ...
    getActiveFireballsForDamage() {
        if (!this.dragonFire?.getActiveFireballsForCollision) {
            return [];
        }
    ...
    selectDragGrabPoint(levelObject) {
        ...
        let selectedX = dragonFacing > 0 ? Infinity : -Infinity;
        let selectedTieDistanceSq = Infinity;

        // A walking dragon can only reach the near side of the object. Facing right means the
        // dragon approaches from the left, so only the leftmost world-space anchor is reachable.
        ...
        for (const name of ['grab_front', 'grab_back']) {
            ...
            const isBetterSide = dragonFacing > 0
        ...
        // Pick the end that puts the dragon's mouth at the near side of the vehicle:
        // same facing grabs the back; opposite facing grabs the front.
        return dragonFacing === objectFacing ? 'grab_back' : 'grab_front';
    }
    ...
    completeAutoDrag() {
        ...
        if (grabWorld) {
            ...
            const maxHeightOffset = Number.isFinite(CONFIG.DRAGON_DRAG?.maxGrabHeightOffset)
                ? CONFIG.DRAGON_DRAG.maxGrabHeightOffset
                : tileHeight;
    ...
    updateAutoPickupAlignment(dt) {
        ...
        if (this.autoPickupElapsed >= this.getPickupMaxDuration()) {
            ...
            // dragging the dragon indefinitely through the level.
            this.cancelAutoPickup();
            return;
        }
    ...
    updateAutoDragAlignment(dt) {
        ...
        // collision, but allow reverse walking here: overshot grabs need the dragon to step
    ...
    _updateDragTakeoffHold(input, dt) {
        const dragCfg = CONFIG.DRAGON_DRAG || {};
    ...
    updateCarriedObjectFacing() {
        ...
        if (this._carryLastFacingDirection !== this.lastFacingDirection) {
            ...
            // inverted relative to the dragon, keep it inverted after each turn.
            obj.onCarryFacingFlipped?.(this.lastFacingDirection * relation);
            obj.carryTurnYOffset = 0;
        } else if (this.isTurning) {
            // Mirror the dragon's live turn rotation onto the carried object so it
            // rotates in sync. The relation only affects the committed facing on turn
            // completion — the rotation arc itself always follows the dragon's direction.
    ...
    updatePulledLevelObjectVelocity(frameStartX, dt) {
        ...
        if (!this.isMouthDraggingObject()) {
            const maxLen = Number.isFinite(CONFIG.DRAGON_DRAG?.maxJointLength) ? CONFIG.DRAGON_DRAG.maxJointLength : 8;
    ...
    tryPickUpObject(levelObject, options = {}) {
        ...
        if (this.canLiftObject(levelObject)) {
            ...
            this._carryLastFacingDirection = this.lastFacingDirection;
            // Track whether the object started aligned with the dragon (+1) or inverted (-1).
    ...
    updateFireInput(dt) {
        ...
        const fireIsDown = this.isFireInputDown();

        if (fireIsDown) {
            if (!this.fireButtonWasDown) {
                this.fireButtonWasDown = true;
                this.firePressDuration = 0;
                this.fireLongPressTriggered = false;

                // Immediate short-attack feel on initial tap.
                if (this.canShootFireball()) {
                    this.dragonFire.startSmallBurst();
                }
            } else {
                this.firePressDuration += dt;

                if (
                    !this.fireLongPressTriggered &&
                    this.firePressDuration >= DRAGON_FIRE_SETTINGS.longPressThreshold
                ) {
                    this.fireLongPressTriggered = this.startFlame();
                }
            }
            return;
        }
        ...
        if (this.fireLongPressTriggered) {
            this.stopFlame();
        } else if (this.canShootFireball()) {
            // Fireballs stay available even with an empty flame meter.
            const spawnedFireball = this.dragonFire.spawnFireball(this.velocity) === true;
            if (spawnedFireball) {
                this.audioManager?.play?.('fire', { volume: 0.75, detune: (Math.random() * 80) - 40 });
                this.resetFireballCooldown();
            }
        }
    ...
    update(dt) {
        ...
        if (this.isReviving) {
            ...
            this.mesh.position.copy(this.position);
            this.updateDebugHitRect();
            this.updateDragonHitFlash(dt);
            if (this.dragonFire) {
                this.dragonFire.setFlyingState(false);
                this.dragonFire.setFacingDirection(this.getImmediateFireFacingDirection());
                this.dragonFire.setAimDirectionOverride(this.getImmediateFireAimDirection());
            ...
            if (this.dragonFire) {
                ...
                this.dragonFire.updateAnchorPostMixer();
            }
        ...
        if (this.isDeadState) {
            ...
            this.updateDebugHitRect();
            if (this.dragonFire) {
                this.dragonFire.setFlyingState(!this.onGround && this.airMode === 'fly');
                this.dragonFire.setFacingDirection(this.getImmediateFireFacingDirection());
                this.dragonFire.setAimDirectionOverride(this.getImmediateFireAimDirection());
            }
            this.updateDragonHitFlash(dt);
            if (this.dragonFire) {
                this.dragonFire.update(dt);
                this.resolveBreakableTilesFromFire();
            }
            this.updateAnimationState(0, 0, wasOnGround);
            if (this.animationMixer) {
                this.animationMixer.update(dt);
        ...
        // The dragon can enter water from outside but never self-exits — water exit
        ...
        if (this.grabbedObject) {
            ...
            // in the level so the dragon can only flap in place until the player releases it.
        ...
        // Undo neck raise before mixer overwrites — mirrors DragonFireBreath.beforeAnimationUpdate.
        ...
        if (this.dragonFire) {
            // Apply the manual jaw override after mixer updates so it layers on top of all clips.
            this.dragonFire.updateMouth(dt);
            // Resolve flame direction from bone poses now that the mixer has settled.
            this.dragonFire.updateAnchorPostMixer();
        }
    ...
    setupUnderwaterTrail() {
        ...
        this.underwaterTrailGroup.name = 'DragonUnderwaterTrail';
...
//            particle.scale.setScalar(3);
    ...
    isDeepWaterSwim() {
        ...
        const deepTilt = CONFIG.DRAGON_WATER?.deepTiltDistance ?? 4;
    ...
    updateWaterMovement(input, dt) {
        ...
        if (this.waterState === 'swim' && this.isDeepWaterSwim()) {
            ...
            if (this.isFlightTurning) {
                ...
                const hasStoppedH = Math.abs(this.velocity.x) <= DRAGON_MODEL_SETTINGS.animationSpeedDeadZone;
            ...
            if (inputSign !== 0) {
                ...
                if (shoreGround && shoreGround.surfaceHeight >= waterSurfaceY - surfaceSnap) {
                    // Move to shore position first so the dragon is outside the water polygon
    ...
    getWaterAnimationState() {
        ...
        switch (this.waterState) {
            ...
            case 'swim': {
                ...
                const moving = Math.sqrt(vx * vx + vy * vy) > DRAGON_MODEL_SETTINGS.animationSpeedDeadZone;
    ...
    canStayGroundedOnSurface(currentGroundInfo, candidateGroundInfo) {
        ...
        // distant lower surface means the dragon leaves the ground instead of snapping downward.
    ...
    getCollisionStepSize() {
        const configuredStep = CONFIG.DRAGON_MOVEMENT?.maxStepSize;
        ...
        const ratio = Number.isFinite(CONFIG.DRAGON_MOVEMENT?.maxStepSizeTileRatio)
            ? CONFIG.DRAGON_MOVEMENT.maxStepSizeTileRatio
            : 0.4;
    ...
    getMaxMovementSteps() {
        return Math.max(
            1,
            Math.floor(
                Number.isFinite(CONFIG.DRAGON_MOVEMENT?.maxMovementSteps)
                    ? CONFIG.DRAGON_MOVEMENT.maxMovementSteps
                    : 10
            )
        );
    }
    ...
    getGroundFollowDropMax() {
        ...
        // dragon sticks to steep terrain instead of launching into the air.
    ...
    resolveBreakableTilesFromFire() {
        if (!this.dragonFire?.forEachDamagePoint || !this.ground?.breakTileAtCell) {
            return;
        }

        this.dragonFire.forEachDamagePoint((x, y) => {
            const { col, row } = this.ground.worldToCell(x, y);
            const tile = this.ground.getTileAtCell(col, row);
            if (tile?.breakable) {
                this.ground.breakTileAtCell(col, row);
            }
        });
    }

    getCeilingFaintConfig() {
        return CONFIG.DRAGON_CEILING_FAINT || {};
    }
    ...
    triggerFaintCrashExplosion(impactPoint) {
        ...
        this.levelObjectManager?.triggerDragonFaintCrashExplosion?.(impactPoint, this.getCeilingFaintConfig());
        this.logDebugChange(
            'ceilingFaint',
            'crash',
    ...
    updateGroundedMovement(input, dt, options = {}) {
        if (this.isTurning) {
            ...
            if (Math.abs(this.velocity.x) < DRAGON_MODEL_SETTINGS.animationSpeedDeadZone) {
                this.velocity.x = 0;
            }
            this.velocity.y = 0;
            this.onGround = true;
...
*/
        if (!hasHorizontalInput && Math.abs(this.velocity.x) < DRAGON_MODEL_SETTINGS.animationSpeedDeadZone) {
            this.velocity.x = 0;
        }
        if (isRequestingReverse && Math.abs(this.velocity.x) < DRAGON_MODEL_SETTINGS.animationSpeedDeadZone) {
            this.velocity.x = 0;
        }
    ...
    updateAirMode(input, inputLength, isAirReverseRequested) {
        if (this.isCarryHoverOnlyActive()) {
            // Carrying keeps the dragon in the responsive hover control family so pickup / carry
        ...
        if (this.shouldLockHoverNearGround()) {
            // Near the surface, keep hover stable so the dragon does not accidentally pop into
        ...
        // During an airborne side reversal, force a stable hover phase until the dragon has slowed
    ...
    getCarryWeightedFlightMaxSpeed(baseSpeed) {
        ...
        const maxSpeedAtMaxLiftWeight = Number.isFinite(CONFIG.DRAGON_CARRY?.flightMaxSpeedAtMaxLiftWeight)
            ? CONFIG.DRAGON_CARRY.flightMaxSpeedAtMaxLiftWeight
            : baseSpeed;
    ...
    getFlightBaseRotationY(facing) {
        ...
        // orientation and rotate 180deg on Y when the dragon should be viewed from the other side.
    ...
    applyFlightTurn(dt, input) {
        ...
        const hasStoppedHorizontally = Math.abs(this.velocity.x) <= DRAGON_MODEL_SETTINGS.animationSpeedDeadZone;
    ...
    getGroundTargetVelocity(inputX) {
        ...
        if (!this.isMouthDraggingObject() && forwardInput > 0) {
            const divisor = Number.isFinite(CONFIG.DRAGON_DRAG?.pushSpeedDivisor) ? CONFIG.DRAGON_DRAG.pushSpeedDivisor : 1.8;
    ...
    updateTurnState(dt, inputX) {
        ...
        if (this.isGrabStruggleActive()) {
            // Too-heavy grab keeps the dragon anchored to the object; keep facing stable so
        ...
        if (this.isPullDraggingObject()) {
            ...
            // the dragon should keep facing the object while pulling it with the mouth.
            this.cancelTurn();
            this.debugState.delete('turnWaitForStop');
            return;
        }
        ...
        const movementSign = this.getSignWithDeadZone(this.velocity.x, DRAGON_MODEL_SETTINGS.animationSpeedDeadZone);
        ...
        // smoothly rotates from start facing to target facing (dragonTurnPivot is suppressed).
        if (isSurfaceWater) {
        ...
        // Always rotate the dragon forward toward the camera during side switching.
    ...
    updateFacingDirection(inputX) {
        if (this.isAutoDragActive() || this.isAutoPickupActive()) {
            this.dragonFacingPivot.rotation.y = this.lastFacingDirection > 0
                ? DRAGON_MODEL_SETTINGS.facingYaw.right
                : DRAGON_MODEL_SETTINGS.facingYaw.left;
            return;
        }
        ...
        if (this.isPullDraggingObject()) {
            ...
            this.dragonFacingPivot.rotation.y = dragFacing > 0
                ? DRAGON_MODEL_SETTINGS.facingYaw.right
                : DRAGON_MODEL_SETTINGS.facingYaw.left;
            return;
        }
        ...
        if (this.isTurning) {
            if (this.isInWater) {
                ...
                this.dragonFacingPivot.rotation.y = DRAGON_MODEL_SETTINGS.facingYaw.right;
            } else {
                this.dragonFacingPivot.rotation.y = this.turnStartFacing > 0
                    ? DRAGON_MODEL_SETTINGS.facingYaw.right
                    : DRAGON_MODEL_SETTINGS.facingYaw.left;
            }
            return;
        ...
        const velocitySign = this.getSignWithDeadZone(this.velocity.x, DRAGON_MODEL_SETTINGS.animationSpeedDeadZone);
        const inputSign = this.getSignWithDeadZone(inputX, 0.05);
    ...
    getImmediateFireAimDirection() {
        ...
        if (!this.onGround && this.airMode === 'hover' && !this.isFlightTurning) {
            ...
            const offset = THREE.MathUtils.degToRad(DRAGON_FIRE_SETTINGS.fireAngleOffsetDeg ?? 0);
            const aimAngle = hoverPitch + offset;
    ...
    updateGroundAlignment(dt) {
        ...
        const alignmentSpeed = this.onGround
            ? (Number.isFinite(CONFIG.groundRotationSpeed)
                ...
                : DRAGON_MODEL_SETTINGS.tiltLerpSpeed)
            : (this.isDeepWaterSwim() || this.waterState === 'swimDive')
                ? (CONFIG.DRAGON_WATER.swimTiltRotationSpeed ?? 3)
        ...
        this.dragonFlightTurnPivot.rotation.y = flightTurnRotationY;
        this.dragonTiltPivot.rotation.z = this.currentGroundTilt;
        // In water the flight pivot owns Y-facing; suppress dragonTurnPivot to avoid double-rotation.
        this.dragonTurnPivot.rotation.y = this.isInWater ? 0 : this.currentTurnRotation;
...
//            console.info('[Player] Grounded -> airborne');
            // Release non-mouth ground drag when the dragon becomes airborne.
    ...
    getGroundAnimationState() {
        ...
        const movementBand = absSpeed <= DRAGON_MODEL_SETTINGS.animationSpeedDeadZone
            ? 'idle'
...
//        console.info(`[Player] Locomotion animation chosen: ${movementBand} at speed ${absSpeed.toFixed(2)}`);
    ...
    playLoopAnimation(state) {
        ...
        this.playAnimation(resolvedState, DRAGON_MODEL_SETTINGS.fadeDuration, false);
    }
    ...
    playTimelineAnimation(state, options = {}) {
        ...
        if (directClipAction) {
            ...
            this.playResolvedAnimationAction(
                directClipAction,
                resolvedState,
                DRAGON_MODEL_SETTINGS.fadeDuration,
                !loop
            );
        }
    }
    ...
    resolveStateWithFallback(state) {
        const fallbackOrder = DRAGON_MODEL_SETTINGS.fallbackOrder[state] || [state];
    ...
    playAnimation(state, fadeDuration = DRAGON_MODEL_SETTINGS.fadeDuration, loopOnce = false) {
    ...
    playResolvedAnimationAction(nextAction, stateLabel, fadeDuration = DRAGON_MODEL_SETTINGS.fadeDuration, loopOnce = false) {
        if (!nextAction) {
            return;
        }
...
//        console.info(`[Player] Animation transition: ${this.activeAnimationState ?? 'none'} -> ${stateLabel}`);
    ...
    tryStartTakeoff(input) {
        ...
        this.cancelTurn();
        this.audioManager?.play?.('dragonLiftoff', { volume: 0.85 });
        this.audioManager?.stopLoop?.('gallop');
    ...
    completeTurn() {
        ...
        if (!this.onGround && !this.isInWater) {
            ...
            if (this.isCarryHoverOnlyActive()) {
                ...
                // Once the dragon climbs above the hover-lock distance, normal air transitions
            ...
            if (!this.isCarryHoverOnlyActive() && this.airMode === 'fly' && shouldResumeFlightImmediately) {
                ...
                // otherwise the dragon can visually keep the previous side while movement resumes.
                this.flightFacingRotationY = this.getFlightBaseRotationY(this.lastFacingDirection);
                this.flightTurnVisualRotation = this.flightFacingRotationY;
            } else {
                this.flightFacingRotationY = 0;
    ...
    updateLocomotionAnimationSpeed(dt) {
        ...
        if (this.isInWater) {
            const waterMaxSpeed = (CONFIG.DRAGON_WATER?.waterMaxSpeed ?? 4) * this.getCurrentSpeedMultiplier();
            ...
            const idleBase = CONFIG.DRAGON_WATER?.swimIdleBaseSpeed ?? 0.6;
        ...
        if (this.onGround && !this.isTurning) {
            ...
            if (this.isPullDraggingObject()) {
                if (heavyDragStandstillAnimationSpeed > 0 && absGroundSpeed <= DRAGON_MODEL_SETTINGS.animationSpeedDeadZone) {
                    // Overweight pull attempts are movement-locked. Drive drag-loop from the
                    // equivalent max-drag-weight pull speed so the dragon visibly slides in place.
                ...
                // Dragging moves opposite the dragon's facing direction. Play the ground cycle
    ...
    getAirborneAnimationReference() {
        ...
        if (this.isThinAirFlapBoostActive()) {
            ...
            // current Y input is neutral and the dragon is simply trying to hold altitude.
    ...
    getAirborneAnimationInputMultiplier(inputX = this.currentInput.x, inputY = this.currentInput.y) {
        ...
        // In air mode, the current stick direction chooses how hard the dragon appears to flap:
        ...
        const struggleBoost = Number.isFinite(CONFIG.DRAGON_CARRY?.struggleFlapInputBoost)
            ? CONFIG.DRAGON_CARRY.struggleFlapInputBoost
            : 1.2;
    ...
    getCycleSyncedTimeScale(state, absGroundSpeed, cycleDistance) {
        ...
        return THREE.MathUtils.clamp(
            rawTimeScale,
            0,
            DRAGON_MODEL_SETTINGS.locomotionTimeScaleMax
        );
    }
    ...
    syncGroundCycleAnimationPhase(state, cycleDistance, direction = 1, traveledDistance = this.groundTravelDistance) {
        ...
        // so the dragon pose stays consistent across devices while grounded.
    ...
    getNormalizedAirborneTimeScale(airSpeed) {
        ...
        return THREE.MathUtils.clamp(normalizedSpeed * 2.2, 0.35, DRAGON_MODEL_SETTINGS.locomotionTimeScaleMax);
    }
    ...
    getAirborneStateTimeScale(state, baseScale) {
        ...
        if (this.isGrabStruggleActive()) {
            const struggleMax = Number.isFinite(CONFIG.DRAGON_CARRY?.struggleFlapMaxTimeScaleAtMaxLiftWeight)
                ? Math.max(CONFIG.DRAGON_CARRY.struggleFlapMaxTimeScaleAtMaxLiftWeight, 0)
                : baseMaxTimeScale;
    ...
    getVisualTiltTarget() {
        ...
        if (angleDifference > THREE.MathUtils.degToRad(18)) {
            ...
            // close enough again, otherwise the dragon appears to "jump" in pitch.
            assistBlend = Math.min(assistBlend, 0.12);
        }
