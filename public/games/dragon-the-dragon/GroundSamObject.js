...
export class GroundSamObject extends TankObject {
    ...
    getMissileConfig() {
        ...
        return {
            ...
            damageToDragon: Math.max(0, Number(this.config.missileDamageToDragon ?? missileConfig.damageToDragon ?? 25)),
    ...
    getDragonTargetWorld(dragonTarget, out = TMP_TARGET_WORLD) {
    ...
    updateMissileCombat(delta, dragonTarget) {
        if (
        ...
        const target = this.getDragonTargetWorld(dragonTarget, TMP_TARGET_WORLD);
        if (!target) {
            return;
        }
    ...
    updateTankCombat(delta, dragonTarget) {
        ...
        this.updateMissileCombat(delta, dragonTarget);
    }

    update(delta, level, dragonTarget = null) {
        super.update(delta, level, dragonTarget);
        if (this.missileLauncher?.hasActiveWork?.()) {
            this.missileLauncher.update(delta, dragonTarget);
        }
