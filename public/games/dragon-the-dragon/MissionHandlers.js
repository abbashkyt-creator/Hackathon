...
// as DragonFireBreath so it stays in front of the same level layers.
class RaceTrail {
...
export class RaceMission extends MissionTypeHandler {
    start(mission, game) {
        ...
        if (game?.scene && this._smoothedPath?.length >= 2) {
            ...
            const trailRenderOrder = game.player?.dragonFire?.renderOrder ??
                game.player?.visualRenderOrder ??
                game.levelRenderer?.getDragonRenderOrder?.() ??
                6.5;
    ...
    buildSmoothedRacePath(game, mission) {
        ...
        const waypoints = [];
        const dragonPos = game?.player?.position;
        if (dragonPos) waypoints.push({ x: dragonPos.x, y: dragonPos.y });
    ...
    update(deltaTime, mission, game) {
        ...
        this._trail?.setRenderOrder(
            game?.player?.dragonFire?.renderOrder ??
            game?.player?.visualRenderOrder ??
            game?.levelRenderer?.getDragonRenderOrder?.()
        );
        this._trail?.setRenderDepth(
            game?.player?.dragonFire?.spawnPosition?.z ??
