...
import { SideFlameBoostButton } from './SideFlameBoostButton.js';
import { SideSpeedBoostButton } from './SideSpeedBoostButton.js';
import { AudioManager } from './AudioManager.js';
import { SettingsDialog } from './SettingsDialog.js';
import { DragonSkinShop, loadPlayerData, savePlayerData } from './DragonSkinShop.js';
...
const SOUND_EFFECTS = {
    ...
    plane: 'sfx/plane.ogg',
    dragonHit: 'sfx/dragon_hit.ogg',
    dragonLiftoff: 'sfx/dragon_liftoff.ogg',
    ...
    flameStart: 'sfx/flame_start.ogg',
    flameLoop: 'sfx/flame_loop.ogg',
    flameEnd: 'sfx/flame_end.ogg',
    ...
    pickupFire: 'sfx/fire_pickup.ogg',
...
function loadSavedQualityMode() {
    ...
    const parsed = loadLocalJson('dragonSettings', null);
...
function getPerformanceQualityProfile(mode = 'auto', slowDeviceDetected = false) {
    ...
    return {
        ...
        dragonFire: {
            terrainChecksEnabled: configuredProfile.dragonFire?.terrainChecksEnabled !== false,
            flameRateMultiplier: configuredProfile.dragonFire?.flameRateMultiplier ?? 1,
            emberRateMultiplier: configuredProfile.dragonFire?.emberRateMultiplier ?? 1,
            smokeRateMultiplier: configuredProfile.dragonFire?.smokeRateMultiplier ?? 1,
            fireballTrailRateMultiplier: configuredProfile.dragonFire?.fireballTrailRateMultiplier ?? 1,
            fireballImpactParticleMultiplier: configuredProfile.dragonFire?.fireballImpactParticleMultiplier ?? 1,
            pointLightEnabled: configuredProfile.dragonFire?.pointLightEnabled !== false
        },
        burnableScenery: {
...
const POKI_LOADING_FINISHED_STORAGE_KEY = 'dragon:pokiGameLoadingFinished';
...
class Game {
    constructor() {
        ...
        // --- Dragon Fury ultimate (Inferno Shockwave) ---
        ...
        this.sideFlameBoostButton = null;
        this.flameBoostRewardInProgress = false;
        this.sideFlameBoostUnlocked = false;
        this.rewardedFlameBoostRemaining = 0;
    ...
    setupLighting() {
/*        
        // Basic scene meshes use unlit materials, so this only affects the imported dragon model.
...
*/        
    ...
    applyPerformanceModeOverrides() {
        ...
        const dragonFire = this.player?.dragonFire ?? null;
        const desiredTerrainChecksEnabled = profile.dragonFire?.terrainChecksEnabled !== false;
        ...
        if (
            ...
            (!dragonFire || dragonFire.terrainChecksEnabled === desiredTerrainChecksEnabled)
        ) {
            this.burnableSceneryManager?.setPerformanceProfile?.(profile.burnableScenery);
            dragonFire?.setPerformanceProfile?.(profile.dragonFire);
            this.parallaxBackground?.setBackgroundLayersVisible?.(desiredParallaxVisible);
            return;
        }

        this.performanceModeState.appliedSlowDeviceMode = slowDevice;
        dragonFire?.setTerrainChecksEnabled?.(desiredTerrainChecksEnabled);
        dragonFire?.setPerformanceProfile?.(profile.dragonFire);
        this.burnableSceneryManager?.setPerformanceProfile?.(profile.burnableScenery);
        this.parallaxBackground?.setBackgroundLayersVisible?.(desiredParallaxVisible);
    }
    ...
    async runCameraPreview(waypoints, { continuous = false, speed = null, holdSeconds = null, zoomFactor = null } = {}) {
        ...
        // Snap camera back to dragon, restore zoom, and exit cinematic mode.
        const exitPreview = () => {
            if (this.player) {
        ...
        if (continuous) {
            // Build full path: start (dragon) → all waypoints → end (dragon).
            const dragonPos = this.player
                ? { x: this.player.position.x, y: this.player.position.y }
                : waypoints[0];
            const path = [dragonPos, ...waypoints, dragonPos];
    ...
    updateCameraFollow(dt = null) {
        ...
        // centered dragon -> minZoom, max look-ahead -> maxZoom.
        ...
        // the dragon opposite on screen (e.g. moving right -> dragon shifts left in viewport).
        const bounds = this.getCameraWorldBounds();
        if (!bounds) {
            return;
        }
        ...
        // Do not clamp camera Y to the authored level top/bottom. Otherwise the dragon gets
    ...
    onResize() {
        ...
        this.sideFlameBoostButton?.layout(viewport.width, viewport.height);
    ...
    getRewardedFlameBoostDuration() {
        return Math.max(
            0,
            Number.isFinite(CONFIG.REWARDED_FLAME_BOOST?.durationSeconds)
                ? CONFIG.REWARDED_FLAME_BOOST.durationSeconds
                : 120
        );
    }
    ...
    isAdActive() {
        return this.pokiGameplayGate.isStopped(PokiStopReasons.AD_COMMERCIAL) ||
            this.pokiGameplayGate.isStopped(PokiStopReasons.AD_REWARDED);
    }
    ...
    openSettingsDialogFromUi() {
        ...
        if (this.pokiGameplayGate.isStopped(PokiStopReasons.TIMELINE_SEQUENCE)) {
            return false;
        }
    ...
    handleGlobalKeyDown(event) {
        // While an ad is running, swallow all keyboard input. Otherwise a
        // background dialog could still receive keys (e.g. ESC closing a dialog
        // behind the ad would resume gameplay underneath the running ad).
        if (this.isAdActive()) {
            event.preventDefault?.();
    ...
    handleGlobalUiInput() {
        // No UI input while an ad is running — the same reasoning as
        // handleGlobalKeyDown: don't let a back/accept press close a dialog
        // behind the ad and resume gameplay underneath it.
        if (this.isAdActive()) {
            return;
        }
    ...
    async init() {
        ...
        this.burnableSceneryManager = new BurnableSceneryManager(this.scene, this.level, {
            ...
            levelUrl: this.currentLevelUrl,
            renderOrder: this.levelRenderer.getDragonRenderOrder() - 0.25,
        ...
        this.levelObjectManager.setDragonTarget(this.player);
        ...
        this.player.setRenderOrder(this.levelRenderer.getDragonRenderOrder());
        ...
        this.skinShop = new DragonSkinShop({
            ...
            onEquip: (skinId, texturePath) => {
                this.player?.setDragonTexture?.(texturePath);
            },
        ...
        if (equippedTexture) this.player?.setDragonTexture?.(equippedTexture);
        ...
        this.sideFlameBoostButton = new SideFlameBoostButton({
            durationSeconds: this.getRewardedFlameBoostDuration(),
            onPress: () => this.activateRewardedFlameBoost(),
        ...
        this.topBarUI.setFlameProgress(this.player.getFlameProgress());
        ...
        await Promise.all([levelObjectsReady, burnableSceneryReady]);
    ...
    async rebuildForMissionLevel(level, levelValue, levelUrl) {
        ...
        this.pickupDropButtonEnabled = false;
        this.rewardedFlameBoostRemaining = 0;
        this.player?.setRewardedFlameBoostActive?.(false);
        this.rewardedSpeedBoostRemaining = 0;
        ...
        nextLevelObjectManager.setDragonTarget(this.player);
        const nextBurnableSceneryManager = new BurnableSceneryManager(this.scene, level, {
            loadingManager: assetLoadingSession.loadingManager,
            audioManager: this.audioManager,
            renderer: this.renderer,
            levelUrl,
            renderOrder: nextLevelRenderer.getDragonRenderOrder() - 0.25,
    ...
    setSequencePresentationActive(isActive) {
        ...
        this.sideFlameBoostButton?.setUiVisible?.(!this.sequencePresentationActive);
    ...
    collectPickup(type, amount, worldPosition) {
        const soundKey = { health: 'pickupHealth', coin: 'pickupCoin', energy: 'pickupEnergy', flame: 'pickupFire' }[type];
    ...
    _applyPickupAmount(type, amount) {
        ...
        switch (type) {
            ...
            case 'flame':
                if (player) {
                    const max = player.maxFlameValue ?? 0;
                    player.currentFlameValue = THREE.MathUtils.clamp(
                        (player.currentFlameValue ?? 0) + amount, 0, max
                    );
                }
                break;
            case 'energy':
    ...
    updatePickupDropButtonState() {
        ...
        if (this.player.isAutoInteractionActive?.()) {
            // While auto alignment is moving the dragon, disable the button visually and functionally
        ...
        // Use skipMouthSideCheck so the drag icon shows even when the dragon needs to walk back
    ...
    updateFireButtonState() {
        if (!this.player || !this.joystick?.setFireEnabled) {
            return;
        }

        this.joystick.setFireEnabled(
            !this.isMissionButtonDisabled('fire') &&
            this.player.canUseFireButton?.() !== false
        );
    }
    ...
    async activateRewardedFlameBoost() {
        if (
            this.flameBoostRewardInProgress ||
            this.player?.isDead?.()
        ) {
            return;
        }

        this.flameBoostRewardInProgress = true;
        this.updateRewardedFlameBoostUI();

        const rewardGranted = await this.runPokiRewardedBreak('flame boost');
        this.flameBoostRewardInProgress = false;
        ...
        this.rewardedFlameBoostRemaining += this.getRewardedFlameBoostDuration();
        this.player?.setRewardedFlameBoostActive?.(this.rewardedFlameBoostRemaining > 0);
        this.updateRewardedFlameBoostUI();
    }

    updateRewardedFlameBoost(dt = 0) {
        const duration = this.getRewardedFlameBoostDuration();
        const flameDepletedCount = this.player?.getFlameDepletedCount?.() ?? 0;
        const flameUnlockCount = CONFIG.REWARDED_FLAME_BOOST?.unlockDrainCount ?? 3;
        if (!this.sideFlameBoostUnlocked && flameDepletedCount >= flameUnlockCount) {
            this.sideFlameBoostUnlocked = true;
        }

        if (this.player?.isDead?.()) {
            this.rewardedFlameBoostRemaining = 0;
            this.player?.setRewardedFlameBoostActive?.(false);
            this.sideFlameBoostButton?.update({
                isActive: false,
                isPending: this.flameBoostRewardInProgress,
                remainingSeconds: duration,
                durationSeconds: duration,
                isVisible: false,
                isUnlocked: this.sideFlameBoostUnlocked,
                dt
            });
            return;
        }

        if (this.rewardedFlameBoostRemaining > 0 && Number.isFinite(dt) && dt > 0) {
            this.rewardedFlameBoostRemaining = Math.max(0, this.rewardedFlameBoostRemaining - dt);
        }

        const isActive = this.rewardedFlameBoostRemaining > 0;
        this.player?.setRewardedFlameBoostActive?.(isActive);
        this.sideFlameBoostButton?.update({
            isActive,
            isPending: this.flameBoostRewardInProgress,
            remainingSeconds: this.rewardedFlameBoostRemaining,
            durationSeconds: duration,
            isVisible: !this.gameOverDialogShown,
            isUnlocked: this.sideFlameBoostUnlocked,
            highlight: this.sideFlameBoostUnlocked && !isActive && (this.player?.isTryingFlameWhileEmpty?.() ?? false),
            dt
        });
    }

    updateRewardedFlameBoostUI() {
        this.updateRewardedFlameBoost(0);
    }
    ...
    updateRewardedSpeedBoost(dt = 0) {
        ...
        this.sideSpeedBoostButton?.update({
            ...
            isVisible: !this.gameOverDialogShown,
            isUnlocked: this.sideSpeedBoostUnlocked,
            highlight: this.sideSpeedBoostUnlocked && !isActive && (this.player?.isTryingSpeedWhileEmpty?.() ?? false),
            dt
        });
    }
    ...
    // Fires the Dragon Fury ultimate if the player has a full charge.
    tryTriggerFury() {
    ...
    renderFrame(dt = null) {
        ...
        if (typeof dt === 'number') {
            ...
            this.updateRewardedFlameBoost(clampedDt);
            this.updateRewardedSpeedBoost(clampedDt);
            this.levelObjectManager?.applyDragonFireDamage?.(this.player, clampedDt);
            this.burnableSceneryManager?.applyDragonFireDamage?.(this.player, clampedDt);
            if (this.levelObjectManager) {
                const objs = this.levelObjectManager.objects;
                for (let i = 0, n = objs.length; i < n; i++) {
                    const obj = objs[i];
                    if (obj?.type === 'bossdragon' && !obj.isDestroyed && !obj.markedForRemoval) {
                        this.levelObjectManager.applyBossFireDamage(obj, this.player, clampedDt);
                    }
                }
            }
        ...
        if (this.topBarUI && this.player?.getFlameProgress) {
            this.topBarUI.setFlameProgress(this.player.getFlameProgress());
        }
        ...
        if (this.topBarUI && this.player && this.level) {
            ...
            const showFighterJets = CONFIG.minimapShowFighterJets;
            ...
            const fighterJetObjects = this._minimapFighterJetObjects || (this._minimapFighterJetObjects = []);
            planeObjects.length = 0;
            zeppelinObjects.length = 0;
            sharkObjects.length = 0;
            fighterJetObjects.length = 0;
            const allObjs = this.levelObjectManager?.objects;
            if (allObjs && (showPlanes || showZeppelins || showSharks || showFighterJets)) {
                for (let i = 0, n = allObjs.length; i < n; i++) {
                    ...
                    else if (showFighterJets && t === 'fighterjet') fighterJetObjects.push(o);
                }
            }
            ...
            this.topBarUI.updateMinimapFighterJetDots(fighterJetObjects, this.level);
        ...
        if (CONFIG.disableDragonRendering && this.player?.mesh) {
        ...
        // Transient screen shake (e.g. Dragon Fury detonation). Offset only for this render so
...
//        this.renderBelowLevelColorOverlay();
        ...
        if (!CONFIG.disableUI) {
            ...
            this.sideFlameBoostButton?.render(this.renderer);
    ...
    animate() {
        ...
        // Dragon Fury ultimate: trigger on key/button press, then advance its slow-mo + shake
