...
export class PhysicsWorld {
    constructor(scene, options = {}) {
        ...
        this.debugDragonAnchorLines = new Map();
    ...
    getDragConstraintStiffness() {
        return Number.isFinite(CONFIG.DRAGON_DRAG?.matterConstraintStiffness)
            ? THREE.MathUtils.clamp(CONFIG.DRAGON_DRAG.matterConstraintStiffness, 0, 1)
            : 0.85;
    }

    getDragConstraintDamping() {
        return Number.isFinite(CONFIG.DRAGON_DRAG?.matterConstraintDamping)
            ? Math.max(CONFIG.DRAGON_DRAG.matterConstraintDamping, 0)
            : 0.18;
    }

    getDragConstraintLength() {
        return Number.isFinite(CONFIG.DRAGON_DRAG?.matterConstraintLength)
            ? Math.max(CONFIG.DRAGON_DRAG.matterConstraintLength, 0)
            : 0.15;
    }

    getDragAnchorTerrainStretchMargin() {
        return Number.isFinite(CONFIG.DRAGON_DRAG?.matterDragAnchorTerrainStretchMargin)
            ? Math.max(CONFIG.DRAGON_DRAG.matterDragAnchorTerrainStretchMargin, 0)
            : 0.55;
    }

    getDragAngularDamping() {
        return Number.isFinite(CONFIG.DRAGON_DRAG?.matterDragAngularDamping)
            ? THREE.MathUtils.clamp(CONFIG.DRAGON_DRAG.matterDragAngularDamping, 0, 1)
            : 0.82;
    }

    getCarryAngularDamping() {
        return Number.isFinite(CONFIG.DRAGON_DRAG?.matterCarryAngularDamping)
            ? THREE.MathUtils.clamp(CONFIG.DRAGON_DRAG.matterCarryAngularDamping, 0, 1)
            : 0.6;
    }
    ...
    setStartupSettlingMode(enabled) {
        ...
        // active so fallOnLoad stacks settle exactly as if the dragon had been nearby all along.
    ...
    _updateTerrainActivation() {
        ...
        // Build list of focal points: dragon + all unsettled falling object bodies.
    ...
    shouldUseMatterForLevelObject(levelObject) {
        ...
        // Choppers and other air actors keep their existing custom movement. The dragon also
    ...
    // or the object has no usable body. Used by the Dragon Fury inferno blast.
    ...
    isWorldPointBlockedForDragAnchor(point) {
        ...
        // Start stretching slightly before the dragon anchor is visibly buried. A zero-length
    ...
    getConstraintLengthForTightDrag(pointA, bodyAnchor) {
        const desiredLength = Math.max(
            Number.isFinite(CONFIG.DRAGON_DRAG?.matterGroundDragRopeLength)
                ? CONFIG.DRAGON_DRAG.matterGroundDragRopeLength
                : 0,
            0
        );
        if (this.isWorldPointBlockedForDragAnchor(pointA)) {
            // If the dragon anchor is pushed inside terrain, a zero-length constraint would
    ...
    forceBodyOutsideTerrain(body, levelObject) {
        ...
        // surface rather than from inside it. This matters when the dragon is pressing down
    ...
    syncObjectFromBody(levelObject) {
        ...
        // Only for objects that opt in to dragon/object collision physics (collideWithDragon).
        if (
            levelObject.state === LEVEL_OBJECT_STATES.IDLE &&
            !levelObject.gravityEnabled &&
            !body.isSleeping &&
            ...
            levelObject.config?.collideWithDragon === true
        ) {
            levelObject.gravityEnabled = true;
            levelObject.state = LEVEL_OBJECT_STATES.FALLING;
    ...
    setDragonState(pos, vel) {
        this._dragonPos = pos || null;
        this._dragonVel = vel || null;
    }
    ...
    update(deltaSeconds = 0) {
        ...
        while (this.accumulatorMs >= fixedStepMs) {
            ...
            // Dragon push runs every substep so it fires reliably regardless of framerate.
            if (this._dragonPos) {
                this.pushLevelObjectsFromDragon(this._dragonPos, this._dragonVel);
            }
    ...
    // Wake all sleeping collideWithDragon blocks within wakeRadius of the given body,
    ...
    _wakeNeighbourBlocks(body, wakeRadius = 6) {
        ...
        for (const [lo, nb] of this.objectBodies.entries()) {
            if (nb === body) continue;
            if (!lo.config?.collideWithDragon) continue;
    ...
    pushIdleBodiesFromMovingLevelObjects() {
        ...
        for (const [idleLO, idleBody] of this.objectBodies.entries()) {
            if (!idleLO.config?.collideWithDragon) continue;
    ...
    handleCollisionStart(event) {
        for (const pair of event.pairs || []) {
            ...
            if (otherObjectBodies.length === 2) {
                ...
                // Cascade-wake any IDLE collideWithDragon block hit by a moving one.
...
applyLaunchVelocities() {
    ...
    applyCarryAngleSpring() {
        const carryStrength = Number.isFinite(CONFIG.DRAGON_DRAG?.matterCarryAngleSpringStrength)
            ? CONFIG.DRAGON_DRAG.matterCarryAngleSpringStrength : 0.02;
        const carryDamping = Number.isFinite(CONFIG.DRAGON_DRAG?.matterCarryAngleSpringDamping)
            ? CONFIG.DRAGON_DRAG.matterCarryAngleSpringDamping : 0.6;
        const carryTargetRad = THREE.MathUtils.degToRad(
            Number.isFinite(CONFIG.DRAGON_DRAG?.matterCarryAngleSpringTargetDeg)
                ? CONFIG.DRAGON_DRAG.matterCarryAngleSpringTargetDeg : 0
        );

        const dragStrength = Number.isFinite(CONFIG.DRAGON_DRAG?.matterDragAngleSpringStrength)
            ? CONFIG.DRAGON_DRAG.matterDragAngleSpringStrength : 0.02;
        const dragDamping = Number.isFinite(CONFIG.DRAGON_DRAG?.matterDragAngleSpringDamping)
            ? CONFIG.DRAGON_DRAG.matterDragAngleSpringDamping : 0.6;
        const dragTargetRad = THREE.MathUtils.degToRad(
            Number.isFinite(CONFIG.DRAGON_DRAG?.matterDragAngleSpringTargetDeg)
                ? CONFIG.DRAGON_DRAG.matterDragAngleSpringTargetDeg : 45
        );
    ...
    // Wake and impulse all sleeping physics bodies whose AABB overlaps the dragon's
    // collision circle. Call this once per frame from LevelObjectManager after the
    // physics update so blocks topple when the dragon flies into them.
    pushLevelObjectsFromDragon(dragonPos, dragonVelocity) {
        if (!this.Matter || !dragonPos) return;

        const velX = dragonVelocity?.x ?? 0;
        const velY = dragonVelocity?.y ?? 0;
        // Skip the O(n) body scan entirely when the dragon is barely moving —
        // it can't meaningfully push anything at low speed.
        if (velX * velX + velY * velY < 4) return;

        const dragonRadius = 1.1;
        ...
        // the dragon is hovering slowly into an object or the device is running slowly.
        const minPushSpeed = 8.0;

        for (const [levelObject, body] of this.objectBodies.entries()) {
            if (body.isStatic) continue;
            if (!levelObject.config?.collideWithDragon) continue;
            ...
            if (distSq > dragonRadius * dragonRadius) continue;

            // Derive the push direction. When the dragon is already inside the AABB
            ...
            if (sepLen > 0.0001) {
                ...
                // Dragon exactly at body centre — push upward as safe default.
                toNX = 0;
                toNY = 1;
            }

            // Skip if dragon is clearly moving away from this object already.
            ...
            // Transfer the dragon's velocity component toward the block, with a floor
            // so slow devices still generate a visible push.
            const dragonDot = velX * toNX * fixedStepSec + velY * toNY * fixedStepSec;
            const blockDot = body.velocity.x * toNX + body.velocity.y * toNY;
            const minDot = minPushSpeed * fixedStepSec;
            const pushDot = Math.max(dragonDot, minDot);
    ...
    updateDebug() {
        ...
        for (const [levelObject, lines] of [...this.debugDragonAnchorLines.entries()]) {
            if (!this.objectBodies.has(levelObject)) {
                for (const line of Object.values(lines)) {
                    line?.geometry?.dispose?.();
                    line?.material?.dispose?.();
                ...
                this.debugDragonAnchorLines.delete(levelObject);
            }
        }
        ...
        for (const [levelObject, body] of this.objectBodies.entries()) {
            ...
            const constraint = this.dragConstraints.get(levelObject);
            const dragonAnchorLines = this.debugDragonAnchorLines.get(levelObject) || {};
            const dragonMarkerSpecs = [];
            if (constraint) {
            ...
            const activeDragonMarkerKeys = new Set();
            for (const marker of dragonMarkerSpecs) {
                activeDragonMarkerKeys.add(marker.key);
                if (!marker.point || !Number.isFinite(marker.point.x) || !Number.isFinite(marker.point.y)) {
                    continue;
                }

                const markerGeometry = makeCrossGeometry(marker.point, 0.34, DEBUG_Z + 0.65);
                let markerLine = dragonAnchorLines[marker.key];
                if (!markerLine) {
                    markerLine = new THREE.LineSegments(
                        markerGeometry,
                        new THREE.LineBasicMaterial({
                    ...
                    markerLine.frustumCulled = false;
                    group.add(markerLine);
                    dragonAnchorLines[marker.key] = markerLine;
                } else {
                    markerLine.geometry?.dispose?.();
                    markerLine.geometry = markerGeometry;
                    markerLine.material.color.setHex(marker.color);
            ...
            for (const [key, line] of Object.entries(dragonAnchorLines)) {
                if (activeDragonMarkerKeys.has(key)) {
                    continue;
                }
                ...
                line?.material?.dispose?.();
                line?.removeFromParent?.();
                delete dragonAnchorLines[key];
            }
            this.debugDragonAnchorLines.set(levelObject, dragonAnchorLines);
            ...
            if (this.debugUpdateCounter % 60 === 0) {
                if (this.shouldDebugDropDiagnostics())
                {
                    console.log('[PhysicsWorld] Matter debug overlay status', {
                        ...
                        dragonAnchorOverlayCount: [...this.debugDragonAnchorLines.values()]
                            .reduce((sum, lines) => sum + Object.keys(lines || {}).length, 0),
    ...
    removeObjectDebug(levelObject) {
        ...
        const dragonAnchorLines = this.debugDragonAnchorLines.get(levelObject);
        for (const dragonAnchorLine of Object.values(dragonAnchorLines || {})) {
            dragonAnchorLine?.geometry?.dispose?.();
            dragonAnchorLine?.material?.dispose?.();
            dragonAnchorLine?.removeFromParent?.();
        }
        this.debugDragonAnchorLines.delete(levelObject);
    ...
    disposeDebug() {
        ...
        for (const lines of this.debugDragonAnchorLines.values()) {
            for (const line of Object.values(lines || {})) {
                line?.geometry?.dispose?.();
                line?.material?.dispose?.();
            }
        }
        this.debugDragonAnchorLines.clear();
