function injectSettingsDialogStyles() {
    if (document.getElementById('dragon-settings-dialog-styles')) {
        return;
    }

    const style = document.createElement('style');
    style.id = 'dragon-settings-dialog-styles';
    style.textContent = `
        .dragon-settings-screen {
            position: fixed;
            inset: 0;
            z-index: 1200;
            display: flex;
        ...
        .dragon-settings-screen.is-visible {
            opacity: 1;
            visibility: visible;
            pointer-events: auto;
        }

        .dragon-settings-card {
        ...
        .dragon-settings-card::-webkit-scrollbar {
            width: 10px;
        }

        .dragon-settings-card::-webkit-scrollbar-track {
            background: rgba(255, 255, 255, 0.06);
            border-radius: 999px;
        }

        .dragon-settings-card::-webkit-scrollbar-thumb {
            background: rgba(160, 200, 255, 0.45);
            border-radius: 999px;
        }

        .dragon-settings-card::-webkit-scrollbar-thumb:hover {
            background: rgba(160, 200, 255, 0.68);
        }

        .dragon-settings-header {
            display: flex;
            align-items: center;
        ...
        .dragon-settings-header > :first-child {
            min-width: 0;
            flex: 1 1 auto;
        }

        .dragon-settings-kicker {
            margin: 0 0 8px;
        ...
        .dragon-settings-row:last-of-type {
        ...
        .dragon-settings-toggle[aria-checked="true"] {
            background:
                linear-gradient(180deg, #fff4b8 0%, #ffb949 100%);
        }

        .dragon-settings-toggle[aria-checked="true"]::before {
            transform: translateX(38px);
            background: #14253a;
        }

        .dragon-settings-close:hover,
        .dragon-settings-close:focus-visible,
        .dragon-settings-toggle:hover,
        .dragon-settings-toggle:focus-visible {
            outline: none;
            filter: brightness(1.05);
        }

        .dragon-settings-select {
            flex: 0 0 auto;
            padding: 8px 12px;
            border: 1px solid rgba(255, 255, 255, 0.18);
            border-radius: 8px;
        ...
        .dragon-settings-select:focus {
            outline: none;
            border-color: rgba(255, 255, 255, 0.4);
        }

        .dragon-settings-restart-btn {
            flex: 0 0 auto;
            padding: 9px 18px;
        ...
        .dragon-settings-restart-btn:hover,
        .dragon-settings-restart-btn:focus-visible {
            outline: none;
        ...
        .dragon-settings-restart-btn.confirm {
            background: rgba(255, 60, 40, 0.5);
            border-color: #ff6050;
            color: #fff;
        }

        .dragon-settings-select option {
            background: #0c2457;
            color: #f7fbff;
        }
...
export class SettingsDialog {
    constructor({
        ...
        this.root.className = 'dragon-settings-screen';

        this.card = document.createElement('div');
        this.card.className = 'dragon-settings-card';
        this.card.setAttribute('role', 'dialog');
        this.card.setAttribute('aria-modal', 'true');
        this.card.setAttribute('aria-labelledby', 'dragon-settings-title');

        const header = document.createElement('div');
        header.className = 'dragon-settings-header';

        const headingGroup = document.createElement('div');
        this.kicker = document.createElement('p');
        this.kicker.className = 'dragon-settings-kicker';
        this.kicker.textContent = 'DRAGON THE DRAGON';

        this.title = document.createElement('h2');
        this.title.id = 'dragon-settings-title';
        this.title.className = 'dragon-settings-title';
        this.title.textContent = t('settings_title');
        headingGroup.append(this.kicker, this.title);

        this.closeButton = document.createElement('button');
        this.closeButton.className = 'dragon-settings-close';
        ...
        this._nameLabel = nameRow.querySelector('.dragon-settings-label');
        const qualityRow = this.qualitySystemEnabled ? this.createQualityRow() : null;
        this._qualityLabel = qualityRow?.querySelector('.dragon-settings-label') || null;
        ...
        this._languageLabel = languageRow.querySelector('.dragon-settings-label');
        const quitRow = this.createDesktopQuitRow();
        this._quitLabel = quitRow?.querySelector('.dragon-settings-label') || null;
        const restartRow = this.createRestartRow();
        this._restartLabel = restartRow.querySelector('.dragon-settings-label');
        const resetAllRow = this.createResetAllRow();
        this._resetAllLabel = resetAllRow.querySelector('.dragon-settings-label');
    ...
    createNameRow() {
        ...
        row.className = 'dragon-settings-row';

        const labelGroup = document.createElement('div');
        labelGroup.style.cssText = 'min-width: 0; flex: 1 1 auto;';

        const label = document.createElement('div');
        label.className = 'dragon-settings-label';
    ...
    createDesktopFullscreenRow() {
        ...
        toggle.className = 'dragon-settings-toggle';
        toggle.type = 'button';
    ...
    createDesktopQuitRow() {
        ...
        this._quitBtn.className = 'dragon-settings-restart-btn';
    ...
    createLanguageRow() {
        ...
        select.className = 'dragon-settings-select';
    ...
    createRestartRow() {
        ...
        this._restartBtn.className = 'dragon-settings-restart-btn';
    ...
    createResetAllRow() {
        ...
        this._resetAllBtn.className = 'dragon-settings-restart-btn';
    ...
    saveSettings() {
        try {
            ...
            void saveJsonWithPlatformMirrors('dragonSettings', settings);
        } catch (error) {
    ...
    loadSettings() {
        const settings = loadLocalJson('dragonSettings', {});
