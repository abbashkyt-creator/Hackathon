...
export class VehicleObject extends LevelObject {
    ...
    keepDraggedFreeCornerOnGround(level, stepDelta) {
        if (
            !this.isBeingDragged() ||
            !CONFIG.DRAGON_DRAG?.keepFreeCornerGrounded ||
        ...
        const slop = Number.isFinite(CONFIG.DRAGON_DRAG?.freeCornerGroundSlop)
            ? CONFIG.DRAGON_DRAG.freeCornerGroundSlop
            : 0.03;
        if (gap <= slop) {
            return false;
        }
    ...
    updatePhysicsStep(stepDelta, level) {
        ...
        const linearDamping = dragging && Number.isFinite(CONFIG.DRAGON_DRAG?.linearDamping)
            ? CONFIG.DRAGON_DRAG.linearDamping
            : this.linearDamping;
        const angularDamping = dragging && Number.isFinite(CONFIG.DRAGON_DRAG?.angularDamping)
            ? CONFIG.DRAGON_DRAG.angularDamping
            : this.angularDamping;
    ...
    getWalkableTopEdge() {
        ...
        // edges — the dragon walks across the gap without leaving the grounded surface.
