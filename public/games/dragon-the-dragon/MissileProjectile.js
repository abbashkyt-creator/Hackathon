...
export class MissileProjectile {
    ...
    launch(position, direction, config = {}) {
        ...
        this.age = 0;
        this.active = true;
        this.damageToDragon = Math.max(0, Number.isFinite(config.damageToDragon) ? config.damageToDragon : 25);
    ...
    update(delta, target) {
        ...
        if (this.hitDragon(target)) {
            const damage = Math.max(0, Number.isFinite(this.damageToDragon) ? this.damageToDragon : 0);
    ...
    updateHoming(delta, target) {
        ...
        // Homing direction is the vector from missile to the current dragon target point.
...
export class MissileLauncher {
    ...
    handleMissileEvent(reason, position, direction) {
        ...
        if (reason === 'hitDragon' || reason === 'timeout' || reason === 'groundCull' || reason === 'hitByDragonFire') {
