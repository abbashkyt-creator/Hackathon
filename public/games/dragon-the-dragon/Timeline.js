 ...
 *   timeline.registerActors({ dragon, tank_1, chopper_1 });
...
// Start or stop flame on any actor that supports it.
// Goes directly to dragonFire to bypass input-lock and resource guards that
// would block flame during timeline playback (when input is always locked).
...
function setActorFlame(actor, active, fireAngleDeg, fireOffset) {
    if (!actor) return;
    const fire = actor.dragonFire;
    if (!fire) return;

    if (typeof fireAngleDeg === 'number') {
        const facingDir = fire.facingDirection >= 0 ? 1 : -1;
        const rad = fireAngleDeg * (Math.PI / 180);
        fire.setAimDirectionOverride({
            x: Math.cos(rad) * facingDir,
            y: Math.sin(rad),
            z: 0
        });
        actor._timelineFlameDirectionOverride = true;
    } else {
        // No fireAngle — clear the override so the actor uses its own facing logic.
        fire.setAimDirectionOverride(null);
        actor._timelineFlameDirectionOverride = false;
    }

    if (fireOffset && (typeof fireOffset.x === 'number' || typeof fireOffset.y === 'number')) {
        fire.anchorPositionOverride = { x: fireOffset.x ?? 0, y: fireOffset.y ?? 0 };
    } else {
        fire.anchorPositionOverride = null;
    }

    if (active) {
        fire.startContinuousFire?.();
    } else {
        fire.stopFire?.();
    }
}
 ...
 *   timeline.registerActors({ dragon, tank_1 });
 ...
 *
 * For the dragon actor track: only hideNodes/showNodes/nodeVisibility keyframes
 * are applied (in order, so the last one wins), and the dragon is forced visible.
 * Position, rotation, and animation tracks on the dragon are skipped — the game
...
export function applySequenceInstantly(sequenceConfig, context = {}) {
    ...
    const dragonActor = trackContext.actors?.dragon ?? null;

    for (const track of timeline.tracks) {
        if (!track || track instanceof EventTrack || track instanceof SfxTrack || track instanceof CameraTrack) continue;

        const isDragonActorTrack = track instanceof ActorTrack && track.actor === 'dragon';

        if (isDragonActorTrack) {
            // Dragon: apply only node-visibility keyframes in order
            for (const kf of track.keyframes) {
                applyActorNodeVisibility(dragonActor, kf);
            }
            // Ensure the dragon is always visible after a skip, flame always off.
            if (dragonActor) {
                setActorVisible(dragonActor, true);
                setActorFlame(dragonActor, false, null);
                dragonActor.onTimelineTransformUpdated?.();
            }
        } else {
 ...
 * 
 * USAGE:
 *   const actors = collectActors(game, ['dragon', 'tank_1', 'chopper_2']);
 */
