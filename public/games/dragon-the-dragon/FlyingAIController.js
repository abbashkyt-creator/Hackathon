...
export class FlyingAIController {
    constructor(owner, options = {}) {
        ...
        // Multiplier applied to all three speeds when in attack mode (move/up/down).
        this.attackSpeedMultiplier = Math.max(0.001, Number(r('attackSpeedMultiplier', 1)));
        ...
        // Attack-mode target override. When set by FighterJetObject, _updatePlane steers
        // toward this position at attackSpeed instead of using the normal patrol picker.
        // Cleared each frame after use so the owner must set it every update cycle.
        this._attackTarget    = null;
        this._useAttackSpeed  = false;
    ...
    update(delta, level, dragonTarget) {
        ...
        if (this.movementType === 'plane') {
            this._updatePlane(delta, level, dragonTarget);
        } else {
            ...
            this._updateDrone(delta, level, dragonTarget);
    ...
    // Called by FighterJetObject each frame when in attack mode.
    // Overrides the patrol picker: the plane steers toward `pos` at attackSpeed
    // instead of wandering. Pass null to resume normal patrol.
    setAttackTarget(pos, useAttackSpeed = true) {
        this._attackTarget   = pos ? { x: pos.x, y: pos.y } : null;
        this._useAttackSpeed = Boolean(useAttackSpeed);
    }

    // ── plane movement ────────────────────────────────────────────────────────

    _updatePlane(delta, level, dragonTarget) {
        const obj = this.owner;
        const pos = obj.container.position;
        ...
        if (this.behaviorMode === 'flee') {
            this._updatePlaneFleeTarget(dragonTarget, level);
        }

        // Attack mode: FighterJetObject calls setAttackTarget() each frame with a position
        // to steer toward. This bypasses the patrol picker while an attack target is set.
        if (this._attackTarget) {
            this.patrolTarget.set(this._attackTarget.x, this._attackTarget.y, pos.z);
            this._hasPatrolTarget = true;
            this._attackTarget = null; // consumed; owner must set it again next frame
        }
        ...
        // Attack mode scales all three speeds by attackSpeedMultiplier.
        const speedMul = this._useAttackSpeed ? this.attackSpeedMultiplier : 1;
        this._useAttackSpeed = false; // consumed; owner must re-enable each frame
        const cruise  = this.moveSpeed    * speedMul;
        const upSpeed = this.moveUpSpeed  * speedMul;
        const dnSpeed = this.moveDownSpeed * speedMul;
        const baseSpeed = sinAngle >= 0
            ? cruise + (upSpeed - cruise) * sinAngle
            : cruise + (cruise  - dnSpeed) * sinAngle;
    ...
    _updatePlaneFleeTarget(dragonTarget, level) {
        const targetPos = this._getDragonXY(dragonTarget);
        if (!targetPos) return;
        ...
        for (let attempt = 0; attempt < FLEE_PATH_RETRIES && fleeX === null; attempt++) {
            // Random jink angle biased away from dragon.
    ...
    _updateDrone(delta, level, dragonTarget) {
        if (this.behaviorMode === 'flee') {
            this._updateDroneFlee(delta, level, dragonTarget);
        } else {
    ...
    _updateDroneFlee(delta, level, dragonTarget) {
        const pos = this.owner.container.position;
        const targetPos = this._getDragonXY(dragonTarget);
