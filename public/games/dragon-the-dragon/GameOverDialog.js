...
function injectGameOverDialogStyles() {
    if (document.getElementById('dragon-game-over-dialog-styles')) {
        return;
    }

    const style = document.createElement('style');
    style.id = 'dragon-game-over-dialog-styles';
    style.textContent = `
        .dragon-game-over-screen {
            position: fixed;
            inset: 0;
            z-index: 1100;
            display: flex;
        ...
        .dragon-game-over-screen.is-visible {
            opacity: 1;
            visibility: visible;
            pointer-events: auto;
        }

        .dragon-game-over-card {
            width: min(100%, 478px);
        ...
        .dragon-game-over-preview::after {
            content: "";
            position: absolute;
            inset: auto 9% 10px;
            height: 20px;
        ...
        .dragon-game-over-preview canvas {
            position: absolute;
            inset: 0;
            width: 100%;
            height: 100%;
        ...
        .dragon-game-over-button:hover,
        .dragon-game-over-button:focus-visible {
        ...
        .dragon-game-over-button:active {
            transform: translateY(1px);
        }

        .dragon-game-over-button:disabled {
            cursor: default;
...
export class GameOverDialog {
    constructor({ onRetry, onRevive } = {}) {
        ...
        this.root.className = 'dragon-game-over-screen';

        this.card = document.createElement('div');
        this.card.className = 'dragon-game-over-card';

        this.kicker = document.createElement('p');
        this.kicker.className = 'dragon-game-over-kicker';
        this.kicker.textContent = 'DRAGON THE DRAGON';

        this.previewContainer = document.createElement('div');
        this.previewContainer.className = 'dragon-game-over-preview';

        this.title = document.createElement('h1');
        this.title.className = 'dragon-game-over-title';
        this.title.textContent = t('game_over_title');

        this.actions = document.createElement('div');
        this.actions.className = 'dragon-game-over-actions';

        this.retryButton = document.createElement('button');
        this.retryButton.className = 'dragon-game-over-button';
        this.retryButton.type = 'button';
        this.retryButton.textContent = t('game_over_retry');

        this.reviveButton = document.createElement('button');
        this.reviveButton.className = 'dragon-game-over-button';
        ...
        this.reviveAdIcon.className = 'dragon-game-over-button-ad';
    ...
    initPreview() {
        try {
            ...
            console.warn('[GameOverDialog] Dragon preview unavailable.', error);
    ...
    async loadPreviewModel() {
        try {
            ...
            const [gltf, texture] = await Promise.all([
                ...
                textureLoader.loadAsync(DRAGON_MODEL_SETTINGS.texturePath).catch(() => null)
            ]);

            const dragonModel = gltf.scene;
            this.preparePreviewModel(dragonModel, texture);
            this.preview.root.add(dragonModel);
            this.fitPreviewToStage(dragonModel);
            this.applyDeadPose(dragonModel, gltf.animations);
            if (this.visible) {
                this.startPreview();
            }
        } catch (error) {
            console.warn('[GameOverDialog] Failed to load dragon preview.', error);
            this.previewContainer.style.display = 'none';
        }
    }

    preparePreviewModel(dragonModel, texture) {
        if (texture) {
    ...
    fitPreviewToStage(dragonModel) {
        ...
        dragonModel.position.y -= PREVIEW_MODEL_TARGET_SIZE * 0.12;
        this.preview.root.rotation.z = 0;
    }

    applyDeadPose(dragonModel, clips = []) {
        ...
        this.preview.mixer = new THREE.AnimationMixer(dragonModel);
