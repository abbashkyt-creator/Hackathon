...
export class ChopperObject extends LevelObject {
    ...
    getDragonTargetWorld(dragonTarget, out = new THREE.Vector3()) {
    ...
    updateFacingTarget(dragonTarget) {
        const target = this.getDragonTargetWorld(dragonTarget, TMP_TARGET_DELTA);
        if (!target) {
            return;
        }
    ...
    update(delta, level, dragonTarget = null) {
        ...
        this.state = LEVEL_OBJECT_STATES.IDLE;
        this.gravityEnabled = false;
        this.updateFacingTarget(dragonTarget);
