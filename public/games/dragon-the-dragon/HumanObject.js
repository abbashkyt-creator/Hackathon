...
export class HumanObject extends VehicleObject {
    constructor(options) {
        ...
        // Dragon reaction — fleeRange maps to dragonReactDistance.
        // reactToDragon: false disables it entirely.
        const reactToDragon = wb.reactToDragon !== false;
        this.fleeRange = reactToDragon
            ? Math.max(0, getFiniteNumber(wb.dragonReactDistance ?? this.config.fleeRange, 40))
            : 0;
        this.fleeOnDragonProximity = wb.fleeOnDragonProximity === true || this.config.fleeOnDragonProximity === true;
        ...
        this.moveAwayFromDragon = wb.moveAwayFromDragon !== false;
        // For male (no walkingBehavior), moveAwayFromDragon defaults true (existing flee).
        if (!this.config.walkingBehavior) {
            this.moveAwayFromDragon = true;
        }
    ...
    advanceWalkingStateMachine(delta) {
        if (this.walkState === 'runAway') {
            return; // Dragon reaction overrides normal cycle; released in updateFleeState.
        }
    ...
    update(delta, level, dragonTarget = null, airTargets = []) {
        if (!this.loaded) {
            return;
        }
        ...
        if (this.state !== LEVEL_OBJECT_STATES.IDLE || this.gravityEnabled) {
            ...
            super.update(delta, level, dragonTarget, airTargets);
        ...
        if (!this.isDragonWithinAiActivationRange(dragonTarget)) {
            this._lastDragonTarget = null;
            this.isFleeing = false;
            this.fleeDirectionLock = 0;
            this.fleeCorrectionTimer = 0;
            if (this.walkState === 'runAway') {
            ...
            // Only advance state machine, no movement or animation updates when dragon is far away
        ...
        this._lastDragonTarget = dragonTarget;
        this.updateFleeState(delta, level, dragonTarget);
    ...
    // ── Flee / dragon reaction ────────────────────────────────────────────────

    isDragonWithinAiActivationRange(dragonTarget) {
        if (this.aiActivationRange <= 0) return false;
        const dragonPosition = this.getDragonPosition(dragonTarget);
        if (!dragonPosition) return false;
        const dx = dragonPosition.x - this.container.position.x;
        const dy = dragonPosition.y - this.container.position.y;
        if (!Number.isFinite(dx) || !Number.isFinite(dy)) return false;
        return (dx * dx + dy * dy) <= (this.aiActivationRange * this.aiActivationRange);
    }

    isDragonNearForWalkActivation(dragonTarget) {
    ...
    updateFleeState(delta, level, dragonTarget) {
        const dragonPosition = this.getDragonPosition(dragonTarget);
        ...
        if (!dragonPosition || this.fleeRange <= 0) {
            stopFleeing();
            return;
        }
        ...
        // Condition 3: dragon must be on the ground (not airborne).
        ...
        // Condition 2: dragon facing toward the cow (dragonFacing * dx < 0 means facing our way).
        const dragonFacing = dragonTarget?.lastFacingDirection >= 0 ? 1 : -1;
        const dragonFacingCow = dragonFacing * dx < 0;

        // Condition 4: cow facing the dragon (walkDirection toward dragon).
        const cowFacingDragon = this.walkDirection * dx > 0;

        const shouldFleeFromFacingThreat = dragonFacingCow && cowFacingDragon;
        const shouldFleeFromProximity = this.fleeOnDragonProximity;

        if (!shouldFleeFromFacingThreat && !shouldFleeFromProximity) {
            // Dragon not threatening — stop fleeing but don't turn back if already fleeing away.
            stopFleeing();
            return;
        }
        ...
        if (this.moveAwayFromDragon) {
            ...
            // Turn away from the dragon (preserve new direction so we never face it again
            // while it stays in range and facing us).
            this.turnAround(level, { delayFleeCorrection: true, preserveDirection: false });
        }
    }

    getDragonPosition(dragonTarget) {
        const position = dragonTarget?.position || dragonTarget?.mesh?.position || null;
    ...
    pickUp(dragon, socket, options = {}) {
        const didPickUp = super.pickUp(dragon, socket, options);
        if (didPickUp) {
    ...
    grab(dragon, socket) {
        const didGrab = super.grab(dragon, socket);
        if (didGrab) {
            this.snapTurnToFacing();
            this.playPickupStartSound();
        }
    ...
    startDrag(dragon, grabPointName) {
        const didStartDrag = super.startDrag(dragon, grabPointName);
        if (didStartDrag) {
            this.playPickupStartSound();
        }
        return didStartDrag;
    ...
    turnAround(level, options = {}) {
        if (!options.preserveDirection) {
            ...
            // Don't turn if doing so would face the cow toward a threatening grounded dragon.
            if (this._lastDragonTarget) {
                const dragonPos = this.getDragonPosition(this._lastDragonTarget);
                if (dragonPos) {
                    const dx = dragonPos.x - this.container.position.x;
                    const dy = dragonPos.y - this.container.position.y;
                    const groundThreshold = (level?.tileHeight ?? 2) * 0.75;
                    const dragonGrounded = dy <= groundThreshold;
                    const dragonFacing = this._lastDragonTarget?.lastFacingDirection >= 0 ? 1 : -1;
                    const dragonFacingCow = dragonFacing * dx < 0;
                    const inRange = (dx * dx + dy * dy) <= (this.fleeRange * this.fleeRange);
                    const wouldFaceDragon = newDirection * dx > 0;

                    if (dragonGrounded && dragonFacingCow && inRange && wouldFaceDragon) {
                        // Turning would face the dragon — go idle instead of looping.
                        this.velocity.set(0, 0, 0);
                        this.angularVelocity = 0;
                        if (this.availableAnimations.idle) {
                            this.walkState = 'idle';
