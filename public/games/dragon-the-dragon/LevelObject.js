...
// Cache parsed GLTF results by model path so objects sharing the same GLB
// (e.g. 10 cars) only parse it once. Each instance clones gltf.scene so
// mutations (name, scale, materials) are fully isolated per instance.
// Animation clips are shared by reference — THREE's AnimationMixer handles this correctly.
const SHARED_GLTF_PROMISES = new Map();

// Stores { scene, animations } keyed by modelPath after the first instance has fully
// prepared the scene (computeBoundingBox called on all geometries). Subsequent instances
// clone from this pre-prepared template so bounding data is always correct.
const SHARED_GLTF_TEMPLATES = new Map();

// Clone a scene that may contain SkinnedMesh nodes. SkeletonUtils.clone correctly
// remaps bone references in the clone to the clone's own bone hierarchy, whereas a
// plain Object3D.clone(true) leaves SkinnedMesh.skeleton pointing at the source bones.
...
export class LevelObject {
    ...
    async load() {
        ...
        // If a fully-prepared template already exists for this model, clone it directly.
        // The template has already had computeBoundingBox run on all geometries by the
        // first instance, so bounds are correct on clones without needing a live scene.
        if (SHARED_GLTF_TEMPLATES.has(modelPath)) {
            const template = SHARED_GLTF_TEMPLATES.get(modelPath);
            const texture = await this.loadConfiguredTexture();
            this.sceneObject = cloneScene(template.scene);
            ...
            this.setupAnimationClips(template.animations);
            ...
            this.baseGroundOffset = Number.isFinite(bounds.min.y) ? (containerWorldPosition.y - bounds.min.y) : 0;
            this.buildCollisionShapeData({
                width: boundsSize.x,
                height: boundsSize.y,
                localMinX: bounds.min.x - containerWorldPosition.x,
                localMaxX: bounds.max.x - containerWorldPosition.x,
                localMinY: bounds.min.y - containerWorldPosition.y,
                localMaxY: bounds.max.y - containerWorldPosition.y
            });
        ...
        // First load (or parallel batch siblings): fetch+parse once, each instance gets
        // its own scene. The first instance uses gltf.scene directly; parallel siblings
        // within the same batch clone it (gltf.scene is still unparented at this point
        // so clone(true) is safe and gives correct geometry bounds after updateMatrixWorld).
        const isFirstRequester = !SHARED_GLTF_PROMISES.has(modelPath);
        if (isFirstRequester) {
            SHARED_GLTF_PROMISES.set(modelPath, loaderLoadAsyncWithRetry(this.loader, modelPath));
        }

        const [gltf, texture] = await Promise.all([
            SHARED_GLTF_PROMISES.get(modelPath),
            this.loadConfiguredTexture()
        ]);

        // First requester uses gltf.scene directly (no clone overhead, identical to
        // pre-cache behaviour). Parallel siblings clone the still-unparented gltf.scene.
        this.sceneObject = isFirstRequester ? gltf.scene : cloneScene(gltf.scene);
        this.sceneObject.name = `${this.type}:${this.id}:scene`;
        this.sceneObject.scale.setScalar(this.scale);
        this.sceneObject.visible = this.initialVisible;
        this.container.add(this.sceneObject);
        ...
        // Store a prepared template now that computeBoundingBox has run on all geometries.
        // Future instances for this model path clone from here instead of re-parsing.
        if (!SHARED_GLTF_TEMPLATES.has(modelPath)) {
            SHARED_GLTF_TEMPLATES.set(modelPath, {
                scene: cloneScene(this.sceneObject),
    ...
    applyCarryAlignment() {
        ...
        // pose under the dragon, then the dragon's carry socket provides the world-facing turn.
        // That keeps lifting consistent regardless of how the object was lying on the ground or
        // which way the dragon is facing when it picks the object up.
    ...
    restoreGroundLayerZ() {
        ...
        // While held by the dragon, keep the object at dragon Z rather than snapping
    ...
    getDraggedLayerZ() {
        const dragon = this.draggedBy ?? this.carriedBy;
        if (!dragon) {
            return null;
        }

        const fallbackDragonZ = Number.isFinite(dragon.position?.z)
            ? dragon.position.z
            : (Number.isFinite(dragon.mesh?.position?.z) ? dragon.mesh.position.z : null);
        const dragonBackMostZ = typeof dragon.getBackMostVisualZ === 'function'
            ? dragon.getBackMostVisualZ()
            : fallbackDragonZ;
        if (!Number.isFinite(dragonBackMostZ)) {
            return null;
        }

        const behindDragonOffset = Number.isFinite(CONFIG.DRAGON_DRAG?.draggedBehindDragonZOffset)
            ? CONFIG.DRAGON_DRAG.draggedBehindDragonZOffset
            : 0.02;
        const inFrontOfObjectsOffset = Number.isFinite(CONFIG.DRAGON_DRAG?.draggedInFrontOfObjectsZOffset)
            ? CONFIG.DRAGON_DRAG.draggedInFrontOfObjectsZOffset
            : 0.01;
        const targetBehindDragon = dragonBackMostZ - behindDragonOffset;
        const minInFrontOfObjects = this.groundLayerZ + inFrontOfObjectsOffset;
        return Math.max(minInFrontOfObjects, targetBehindDragon);
    }
    ...
    canBePickedUpBy(dragon) {
        if (!dragon || !this.loaded || this.isDestroyed || !this.container.visible) {
            return false;
        }
        ...
        if (typeof dragon.canPickupObject === 'function' && !dragon.canPickupObject(this)) {
            return false;
        }
    ...
    pickUp(dragon, socket, options = {}) {
        if (!this.canBePickedUpBy(dragon) || !socket) {
            return false;
        }
        ...
        this.physicsWorld?.beginLevelObjectDrag?.(
            ...
            null,
            {
                stiffness: Number.isFinite(CONFIG.DRAGON_DRAG?.matterCarryConstraintStiffness)
                    ? CONFIG.DRAGON_DRAG.matterCarryConstraintStiffness
                    : 1,
                damping: Number.isFinite(CONFIG.DRAGON_DRAG?.matterCarryConstraintDamping)
                    ? CONFIG.DRAGON_DRAG.matterCarryConstraintDamping
                    : 0.5,
                length: 0,
                usePickupRoot: true
            }
    ...
    attachToCarrySocketPreservingWorldVisual(socket) {
        ...
        // Auto-pickup has already moved the dragon so the foot socket reaches the object's
        ...
        // offset under the scaled dragon rig, which made carried objects drift during turns.
    ...
    grab(dragon, socket) {
        ...
        const pickupRootName = this.getSelectedPickupRootName();
        // Too-heavy objects keep their exact world transform. The dragon enters a grabbed/struggle
        // state, but the object itself stays in Matter as a normal sleeping body. We do not
        // disable collision here; the dragon simply cannot move the object because it is too heavy.
        this.velocity.set(0, 0, 0);
        this.angularVelocity = 0;
        this.gravityEnabled = false;
        this.fallStartY = null;
    ...
    startDrag(dragon, grabPointName) {
        if (!this.canBeDraggedBy(dragon) || !grabPointName) {
            return false;
        }

        if (this.mouthDragEnabled) {
            return this.startMouthDrag(dragon, grabPointName);
        }

        this.draggedBy = dragon;
        ...
        this.physicsWorld?.beginLevelObjectDrag?.(
            ...
            this.getGrabPointWorldPosition(grabPointName, new THREE.Vector3()),
            {
                ...
                stiffness: Number.isFinite(CONFIG.DRAGON_DRAG?.matterGroundDragConstraintStiffness)
                    ? CONFIG.DRAGON_DRAG.matterGroundDragConstraintStiffness
                    : undefined,
                damping: Number.isFinite(CONFIG.DRAGON_DRAG?.matterGroundDragConstraintDamping)
                    ? CONFIG.DRAGON_DRAG.matterGroundDragConstraintDamping
                    : undefined,
                length: Number.isFinite(CONFIG.DRAGON_DRAG?.matterGroundDragRopeLength)
                    ? Math.max(CONFIG.DRAGON_DRAG.matterGroundDragRopeLength, 0)
                    : 0
            }
        );
    ...
    startMouthDrag(dragon, grabPointName) {
        const mouthSocket = dragon?.getMouthAttachmentObject?.() || dragon?.mouthObject || null;
        if (!mouthSocket) {
            return false;
        }
    ...
    drop(initialVelocity = new THREE.Vector3(), options = {}) {
        ...
        if (this.isPhysicsCarried()) {
            ...
            this.stopInteractionAnimation();
            // Inherit dragon velocity so the object carries forward momentum on release.
    ...
    hasTakenDamage() {
        ...
        // Global flag: skip the damage requirement and treat all enemies as already aggroed.
        if (CONFIG.requireDamageBeforeAttack === false) {
            return true;
        }
