...
export class BurnableSceneryManager {
    ...
    applyDragonFireDamage(player) {
        if (!player) {
            return;
        }

        this.applyFlameDamage(player);
        this.applyFireballDamage(player);
    }
    ...
    applyFlameDamage(player) {
        ...
        if (flameRect) {
            const radius = Math.hypot(flameRect.halfWidth, flameRect.halfHeight);
            this.queryPoint(flameRect.centerX, flameRect.centerY, radius, (object) => {
                if (object.isIgnitable() && rectIntersectsBounds(flameRect, object.bounds)) {
                    const hitPoint = boundsHitPointFromRect(flameRect, object.bounds);
                    this.igniteObject(object, { source: 'flame', ...hitPoint });
                }
            });
        }

        if (!player.dragonFire?.forEachDamagePoint) {
            return;
        }

        let checked = 0;
        const maxChecks = Math.max(0, CONFIG.BURNABLE_SCENERY?.maxFlamePointChecks ?? 28);
        player.dragonFire.forEachDamagePoint((x, y) => {
            if (checked >= maxChecks || !Number.isFinite(x) || !Number.isFinite(y)) {
                return;
            }
            checked += 1;
            this.queryPoint(x, y, 0.25, (object) => {
                if (object.isIgnitable() && object.overlapsPoint(x, y, 0.2)) {
                    this.igniteObject(object, { source: 'flamePoint', x, y });
                }
            });
        });
    }
    ...
    applyFireballDamage(player) {
        ...
        const activeIds = this._activeFireballIds;
        activeIds.clear();
        for (let i = 0; i < fireballs.length; i++) {
            const id = fireballs[i]?.id;
            if (Number.isFinite(id)) activeIds.add(id);
        }
        for (const id of this.fireballHitRegistry.keys()) {
            if (!activeIds.has(id)) {
                this.fireballHitRegistry.delete(id);
            }
        }

        for (const fireball of fireballs) {
            if (!Number.isFinite(fireball?.id) || !Number.isFinite(fireball?.x) || !Number.isFinite(fireball?.y)) {
                continue;
            }
            if (this.fireballHitRegistry.has(fireball.id)) {
                continue;
            }

            const radius = Math.max(0, Number.isFinite(fireball.radius) ? fireball.radius : 0.9);
            let hitObject = null;
            this.queryPoint(fireball.x, fireball.y, radius, (object) => {
                if (!hitObject && object.isIgnitable() && object.overlapsCircle(fireball.x, fireball.y, radius)) {
                    hitObject = object;
                }
            });
            ...
            this.fireballHitRegistry.set(fireball.id, hitObject.id);
            this.igniteObject(hitObject, { source: 'fireball', x: fireball.x, y: fireball.y });
            if (CONFIG.BURNABLE_SCENERY?.consumeFireballOnHit !== false) {
                player.consumeFireballById?.(fireball.id, {
                    impactPosition: { x: fireball.x, y: fireball.y, z: Number.isFinite(fireball.z) ? fireball.z : 0 },
                    impactNormal: null
                });
            }
        }
