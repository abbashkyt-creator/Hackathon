var Utils = {
    version: 'v2',
    dataVersion: 'v2',
    gameName: 'build_league',
    shortName: 'build_league',
    ...
    isLocalStorageSupported: function () {
        ...
        Utils.isLocalStorageSupported = true;
...
class NURBS {
    static createClampedKnots(count, degree) {
        const knots = [];
    ...
    static createPeriodicKnots(count, degree) {
        const knots = [];
        const totalKnots = count + degree + 1;
    ...
    static prepareData(controlPoints, weights, degree, isCyclic) {
        ...
        if (isCyclic) {
            for (let i = 0; i < degree; i++) {
                pts.push(controlPoints[i]);
                wts.push(weights[i]);
            }
            knots = this.createPeriodicKnots(pts.length, degree);
        } else {
            knots = this.createClampedKnots(pts.length, degree);
        }

        return { pts, wts, knots };
    }

    static getValidU(t, degree, knots) {
        const minU = knots[degree];
        const maxU = knots[knots.length - 1 - degree];
        return minU + t * (maxU - minU);
    }

    static evaluate(u, degree, controlPoints, weights, knots) {
        ...
        for (let i = 0; i <= degree; i++) {
            ...
            const weight = N[i] * weights[index];
        ...
        if (den === 0) return num;
        return { x: num.x / den, y: num.y / den, z: num.z / den };
    }
}