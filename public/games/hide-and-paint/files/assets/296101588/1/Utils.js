...
Utils.mapsCDN = {
    ...
    // One localStorage entry per custom map. The value is its replay count.
    replayStoragePrefix: 'HideAndPaint:ReplayCounts:',

    _ownerId: null,

    // Thumbnail impressions are collected across one Custom Games popup visit
    // and sent together when that popup closes or this limit is reached.
    thumbnailImpressionBatchSize: 50,
    _thumbnailImpressionQueue: [],
    _thumbnailImpressionFlushPromise: null
};
...
Utils.mapsCDN._getReplayStorageKey = function (id) {
    return (this.replayStoragePrefix || 'HideAndPaint:ReplayCounts:') + id;
};

Utils.mapsCDN.getMapReloadState = function (id) {
    var stored = null;
    try { stored = localStorage.getItem(this._getReplayStorageKey(id)); } catch (e) { stored = null; }

    var mapReload = Number(stored);
    return {
        played: stored !== null,
        map_reload: Number.isFinite(mapReload) && mapReload >= 0 ? Math.floor(mapReload) : 0
    };
};

// Mark a successful custom-map start locally. The first start only records that
// the map was played; every later start increments the local replay count and
// reports one aggregate replay to the API.
Utils.mapsCDN.trackMapPlay = function (id) {
    if (!id) return Promise.reject(new Error('Cannot track a map play without a map id'));

    var previous = this.getMapReloadState(id);
    var mapReload = previous.played ? previous.map_reload + 1 : 0;

    try {
        localStorage.setItem(this._getReplayStorageKey(id), String(mapReload));
    } catch (e) {
        console.error('[Utils.mapsCDN] could not persist replay count', e);
    }

    var result = {
        id: id,
        isReload: previous.played,
        map_reload: mapReload
    };

    if (!previous.played) return Promise.resolve(result);

    return this.recordMapReload(id).then(function () {
        return result;
    });
};
...
// may not allow).
Utils.mapsCDN.getApiVersion = function () {
    var version = Number(Utils.mapsApiVersion);
...
Utils.mapsCDN._toRow = function (m) {
    var versionPlays = Number(m.plays);
    var lifetimePlays = Number(m.lifetimeStats && m.lifetimeStats.plays);

    if (!Number.isFinite(versionPlays) || versionPlays < 0) versionPlays = 0;
    if (!Number.isFinite(lifetimePlays) || lifetimePlays < 0) lifetimePlays = versionPlays;
...
// Lightweight discovery analytics. The single-event endpoints accept no body;
// the impression batch sends mapIds. Both return 202 when queued by the API.
Utils.mapsCDN.queueThumbnailImpression = function (id) {
    if (!id) return false;
    if (!Array.isArray(this._thumbnailImpressionQueue)) {
        this._thumbnailImpressionQueue = [];
    }

    this._thumbnailImpressionQueue.push(String(id));
    if (this._thumbnailImpressionQueue.length >= this.thumbnailImpressionBatchSize) {
        this.flushThumbnailImpressions().catch(function (err) {
            console.warn('[Utils.mapsCDN] thumbnail impression batch failed', err);
        });
    }
    return true;
};

Utils.mapsCDN.flushThumbnailImpressions = function () {
    var self = this;
    if (this._thumbnailImpressionFlushPromise) {
        return this._thumbnailImpressionFlushPromise;
    }
    if (!Array.isArray(this._thumbnailImpressionQueue) ||
        this._thumbnailImpressionQueue.length === 0) {
        return Promise.resolve({ status: 'empty', recorded: 0, ignored: 0 });
    }

    var batchSize = Math.max(1, Number(this.thumbnailImpressionBatchSize) || 50);
    var flushNext = function () {
        if (self._thumbnailImpressionQueue.length === 0) {
            return Promise.resolve({ status: 'recorded', recorded: 0, ignored: 0 });
        }

        var mapIds = self._thumbnailImpressionQueue.splice(0, batchSize);
        var request = self._request(
            'POST',
            '/maps/impressions/batch',
            { mapIds: mapIds }
        ).then(null, function (err) {
            // Keep the client safe during an API-first rollout. Until the
            // additive batch route is deployed, use the existing endpoint.
            if (err && err.status === 404) {
                return Promise.all(mapIds.map(function (id) {
                    return self.recordThumbnailImpression(id);
                })).then(function () {
                    return {
                        status: 'legacy-fallback',
                        recorded: mapIds.length,
                        ignored: 0
                    };
                });
            }
            ...
            // Keep delivery best-effort but retryable. Entries that arrived
            // during this request remain after the restored failed batch.
            self._thumbnailImpressionQueue =
                mapIds.concat(self._thumbnailImpressionQueue);
            throw err;
        });
    ...
    this._thumbnailImpressionFlushPromise = flushNext().then(
        function (result) {
            self._thumbnailImpressionFlushPromise = null;
            return result;
        },
        function (err) {
            self._thumbnailImpressionFlushPromise = null;
...
// Kept for older callers and older published clients. New Custom Games builds
// use queueThumbnailImpression + flushThumbnailImpressions instead.
Utils.mapsCDN.recordThumbnailImpression = function (id) {
    if (!id) return Promise.reject(new Error('Cannot record a thumbnail impression without a map id'));
    return this._request(
        'POST',
        '/maps/' + encodeURIComponent(id) + '/impressions'
    );
};

Utils.mapsCDN.recordThumbnailClick = function (id) {
    if (!id) return Promise.reject(new Error('Cannot record a thumbnail click without a map id'));
    return this._request(
        'POST',
        '/maps/' + encodeURIComponent(id) + '/thumbnail-clicks'
    );
};

Utils.mapsCDN.recordMapReload = function (id) {
    if (!id) return Promise.reject(new Error('Cannot record a map reload without a map id'));
    return this._request(
        'POST',
        '/maps/' + encodeURIComponent(id) + '/map-reloads'
    );
};
...
// Establish the anonymous client identity during game startup instead of waiting
// until the player first opens a maps screen.
Utils.mapsCDN.getOwnerId();
