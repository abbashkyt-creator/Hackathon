...
const GAME_SOURCE = "tiptap-wheel-master";
const PARENT_SOURCE = "tiptap-parent";

export function WheelMasterGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/wheel-master/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena wheel-master-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena wheel-master-game"
      src={gameUrl}
      title="Play wheel-master"
