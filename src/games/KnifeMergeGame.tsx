...
const GAME_SOURCE = "tiptap-knife-merge";
const PARENT_SOURCE = "tiptap-parent";

export function KnifeMergeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/knife-merge/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena knife-merge-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena knife-merge-game"
      src={gameUrl}
      title="Play knife-merge"
