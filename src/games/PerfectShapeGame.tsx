...
const GAME_SOURCE = "tiptap-perfect-shape";
const PARENT_SOURCE = "tiptap-parent";

export function PerfectShapeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/perfect-shape/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena perfect-shape-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena perfect-shape-game"
      src={gameUrl}
      title="Play perfect-shape"
