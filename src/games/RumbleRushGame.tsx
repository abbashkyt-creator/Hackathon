...
const GAME_SOURCE = "tiptap-rumble-rush";
const PARENT_SOURCE = "tiptap-parent";

export function RumbleRushGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/rumble-rush/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena rumble-rush-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena rumble-rush-game"
      src={gameUrl}
      title="Play rumble-rush"
