...
const GAME_SOURCE = "tiptap-swole-simulator";
const PARENT_SOURCE = "tiptap-parent";

export function SwoleSimulatorGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/swole-simulator/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena swole-simulator-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena swole-simulator-game"
      src={gameUrl}
      title="Play swole-simulator"
