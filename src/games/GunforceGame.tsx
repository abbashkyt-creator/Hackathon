...
const GAME_SOURCE = "tiptap-gunforce";
const PARENT_SOURCE = "tiptap-parent";

export function GunforceGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/gunforce/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena gunforce-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena gunforce-game"
      src={gameUrl}
      title="Play gunforce"
