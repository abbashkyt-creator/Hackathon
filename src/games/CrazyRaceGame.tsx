...
const GAME_SOURCE = "tiptap-crazy-race";
const PARENT_SOURCE = "tiptap-parent";

export function CrazyRaceGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/crazy-race/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena crazy-race-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena crazy-race-game"
      src={gameUrl}
      title="Play crazy-race"
