...
const GAME_SOURCE = "tiptap-go-battle-2";
const PARENT_SOURCE = "tiptap-parent";

export function GoBattle2Game({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/go-battle-2/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) {
    return <div className="game-arena go-battle-2-placeholder" aria-hidden="true" />;
  }

  return (
    <iframe
      ref={frameRef}
      className="game-arena go-battle-2-game"
      src={gameUrl}
      title="Play Go Battle 2"
