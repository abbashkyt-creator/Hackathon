...
const GAME_SOURCE = "tiptap-merge-zoo";
const PARENT_SOURCE = "tiptap-parent";

export function MergeZooGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/merge-zoo/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena merge-zoo-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena merge-zoo-game"
      src={gameUrl}
      title="Play merge-zoo"
