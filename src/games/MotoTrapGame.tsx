...
const GAME_SOURCE = "tiptap-moto-trap";
const PARENT_SOURCE = "tiptap-parent";

export function MotoTrapGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/moto-trap/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena moto-trap-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena moto-trap-game"
      src={gameUrl}
      title="Play biker"
