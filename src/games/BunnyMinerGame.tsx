...
const GAME_SOURCE = "tiptap-bunny-miner";
const PARENT_SOURCE = "tiptap-parent";

export function BunnyMinerGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/bunny-miner/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena bunny-miner-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena bunny-miner-game"
      src={gameUrl}
      title="Play bunny-miner"
