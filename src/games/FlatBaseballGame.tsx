...
const GAME_SOURCE = "tiptap-flat-baseball";
const PARENT_SOURCE = "tiptap-parent";

export function FlatBaseballGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/flat-baseball/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena flat-baseball-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena flat-baseball-game"
      src={gameUrl}
      title="Play flat-baseball"
