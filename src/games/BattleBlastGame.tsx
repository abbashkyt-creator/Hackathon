...
const GAME_SOURCE = "tiptap-battle-blast";
const PARENT_SOURCE = "tiptap-parent";

export function BattleBlastGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/battle-blast/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena battle-blast-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena battle-blast-game"
      src={gameUrl}
      title="Play battle-blast-1.1.0-rc1"
