...
const GAME_SOURCE = "tiptap-punchy-guy";
const PARENT_SOURCE = "tiptap-parent";

export function PunchyGuyGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/punchy-guy/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena punchy-guy-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena punchy-guy-game"
      src={gameUrl}
      title="Play punchy-guy"
