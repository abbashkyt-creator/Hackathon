...
const GAME_SOURCE = "tiptap-goods-master";
const PARENT_SOURCE = "tiptap-parent";

export function GoodsMasterGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/goods-master/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena goods-master-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena goods-master-game"
      src={gameUrl}
      title="Play goods-master"
