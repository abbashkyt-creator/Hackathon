...
const GAME_SOURCE = "tiptap-monkey-tag-io";
const PARENT_SOURCE = "tiptap-parent";

export function MonkeyTagIoGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/monkey-tag-io/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena monkey-tag-io-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena monkey-tag-io-game"
      src={gameUrl}
      title="Play Monkey Tag Arena"
