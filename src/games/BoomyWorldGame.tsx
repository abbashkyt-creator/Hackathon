...
const GAME_SOURCE = "tiptap-boomy-world";
const PARENT_SOURCE = "tiptap-parent";

export function BoomyWorldGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/boomy-world/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena boomy-world-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena boomy-world-game"
      src={gameUrl}
      title="Play boomy-world"
