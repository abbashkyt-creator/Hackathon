...
const GAME_SOURCE = "tiptap-brainrot-puzzle";
const PARENT_SOURCE = "tiptap-parent";

export function BrainrotPuzzleGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/brainrot-puzzle/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena brainrot-puzzle-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena brainrot-puzzle-game"
      src={gameUrl}
      title="Play brainrot-puzzle"
