...
const GAME_SOURCE = "tiptap-spacebar-clicker";
const PARENT_SOURCE = "tiptap-parent";

export function SpacebarClickerGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/spacebar-clicker/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena spacebar-clicker-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena spacebar-clicker-game"
      src={gameUrl}
      title="Play spacebar-clicker"
