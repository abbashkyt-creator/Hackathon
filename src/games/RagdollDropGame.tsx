...
const GAME_SOURCE = "tiptap-ragdoll-drop";
const PARENT_SOURCE = "tiptap-parent";

export function RagdollDropGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/ragdoll-drop/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena ragdoll-drop-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena ragdoll-drop-game"
      src={gameUrl}
      title="Play ragdoll-drop"
