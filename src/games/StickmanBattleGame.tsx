...
const GAME_SOURCE = "tiptap-stickman-battle";
const PARENT_SOURCE = "tiptap-parent";

export function StickmanBattleGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/stickman-battle/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) {
    return <div className="game-arena stickman-battle-placeholder" aria-hidden="true" />;
  }

  return (
    <iframe
      ref={frameRef}
      className="game-arena stickman-battle-game"
      src={gameUrl}
      title="Play Stickman Battle"
