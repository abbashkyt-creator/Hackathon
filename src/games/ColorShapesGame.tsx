...
const GAME_SOURCE = "tiptap-color-shapes";
const PARENT_SOURCE = "tiptap-parent";

export function ColorShapesGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/color-shapes/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena color-shapes-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena color-shapes-game"
      src={gameUrl}
      title="Play Color Shapes"
