...
const GAME_SOURCE = "tiptap-crazy-descent";
const PARENT_SOURCE = "tiptap-parent";

export function CrazyDescentGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/crazy-descent/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena crazy-descent-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena crazy-descent-game"
      src={gameUrl}
      title="Play crazy-descent"
