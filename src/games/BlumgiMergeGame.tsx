...
const GAME_SOURCE = "tiptap-blumgi-merge";
const PARENT_SOURCE = "tiptap-parent";

export function BlumgiMergeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/blumgi-merge/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena blumgi-merge-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena blumgi-merge-game"
      src={gameUrl}
      title="Play blumgi-merge"
