...
const GAME_SOURCE = "tiptap-eggy-car";
const PARENT_SOURCE = "tiptap-parent";

export function EggyCarGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/eggy-car/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena eggy-car-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena eggy-car-game"
      src={gameUrl}
      title="Play Eggy Car"
