...
const GAME_SOURCE = "tiptap-tank-stars";
const PARENT_SOURCE = "tiptap-parent";

export function TankStarsGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/tank-stars/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena tank-stars-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena tank-stars-game"
      src={gameUrl}
      title="Play tank-stars"
