...
const GAME_SOURCE = "tiptap-elf-backpack";
const PARENT_SOURCE = "tiptap-parent";

export function ElfBackpackGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/elf-backpack/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena elf-backpack-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena elf-backpack-game"
      src={gameUrl}
      title="Play elf-backpack"
