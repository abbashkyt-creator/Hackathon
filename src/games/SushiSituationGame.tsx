...
const GAME_SOURCE = "tiptap-sushi-situation";
const PARENT_SOURCE = "tiptap-parent";

export function SushiSituationGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/sushi-situation/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena sushi-situation-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena sushi-situation-game"
      src={gameUrl}
      title="Play sushi-situation"
