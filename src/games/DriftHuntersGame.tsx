...
const GAME_SOURCE = "tiptap-drift-hunters";
const PARENT_SOURCE = "tiptap-parent";

export function DriftHuntersGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/drift-hunters/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena drift-hunters-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena drift-hunters-game"
      src={gameUrl}
      title="Play drift-hunters"
