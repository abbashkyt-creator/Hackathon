...
const GAME_SOURCE = "tiptap-stickers-merge";
const PARENT_SOURCE = "tiptap-parent";

export function StickersMergeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/stickers-merge/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena stickers-merge-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena stickers-merge-game"
      src={gameUrl}
      title="Play HTML5 Game"
