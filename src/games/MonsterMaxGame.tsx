...
const GAME_SOURCE = "tiptap-monster-max";
const PARENT_SOURCE = "tiptap-parent";

export function MonsterMaxGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/monster-max/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena monster-max-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena monster-max-game"
      src={gameUrl}
      title="Play monster-max"
