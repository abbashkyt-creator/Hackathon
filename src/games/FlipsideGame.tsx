...
const GAME_SOURCE = "tiptap-flipside";
const PARENT_SOURCE = "tiptap-parent";

export function FlipsideGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/flipside/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena flipside-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena flipside-game"
      src={gameUrl}
      title="Play flipside"
