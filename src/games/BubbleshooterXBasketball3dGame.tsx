...
const GAME_SOURCE = "tiptap-bubbleshooter-x-basketball-3d";
const PARENT_SOURCE = "tiptap-parent";

export function BubbleshooterXBasketball3dGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/bubbleshooter-x-basketball-3d/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena bubbleshooter-x-basketball-3d-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena bubbleshooter-x-basketball-3d-game"
      src={gameUrl}
      title="Play bubbleshooter-x-basketball-3d"
