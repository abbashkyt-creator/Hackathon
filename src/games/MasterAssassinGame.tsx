...
const GAME_SOURCE = "tiptap-master-assassin";
const PARENT_SOURCE = "tiptap-parent";

export function MasterAssassinGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/master-assassin/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena master-assassin-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena master-assassin-game"
      src={gameUrl}
      title="Play master-assassin"
