...
const GAME_SOURCE = "tiptap-numbers-match";
const PARENT_SOURCE = "tiptap-parent";

export function NumbersMatchGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/numbers-match/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena numbers-match-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena numbers-match-game"
      src={gameUrl}
      title="Play numbers-match"
