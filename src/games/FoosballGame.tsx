...
const GAME_SOURCE = "tiptap-foosball";
const PARENT_SOURCE = "tiptap-parent";

export function FoosballGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/foosball/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena foosball-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena foosball-game"
      src={gameUrl}
      title="Play foosball"
