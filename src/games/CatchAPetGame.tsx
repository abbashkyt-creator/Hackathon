...
const GAME_SOURCE = "tiptap-catch-a-pet";
const PARENT_SOURCE = "tiptap-parent";

export function CatchAPetGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/catch-a-pet/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena catch-a-pet-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena catch-a-pet-game"
      src={gameUrl}
      title="Play Catch a Pet v1.0.3"
