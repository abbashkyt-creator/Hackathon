...
const GAME_SOURCE = "tiptap-cook-house";
const PARENT_SOURCE = "tiptap-parent";

export function CookHouseGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/cook-house/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena cook-house-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena cook-house-game"
      src={gameUrl}
      title="Play cook-house"
