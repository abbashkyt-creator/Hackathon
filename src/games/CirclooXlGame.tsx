...
const GAME_SOURCE = "tiptap-circloo-xl";
const PARENT_SOURCE = "tiptap-parent";

export function CirclooXlGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/circloo-xl/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena circloo-xl-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena circloo-xl-game"
      src={gameUrl}
      title="Play circloo-xl"
