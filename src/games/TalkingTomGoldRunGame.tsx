...
const GAME_SOURCE = "tiptap-talking-tom-gold-run";
const PARENT_SOURCE = "tiptap-parent";

export function TalkingTomGoldRunGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/talking-tom-gold-run/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena talking-tom-gold-run-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena talking-tom-gold-run-game"
      src={gameUrl}
      title="Play TTGR_Live_1.0.0"
