...
const GAME_SOURCE = "tiptap-grass-knight";
const PARENT_SOURCE = "tiptap-parent";

export function GrassKnightGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/grass-knight/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena grass-knight-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena grass-knight-game"
      src={gameUrl}
      title="Play grass-knight"
