...
const GAME_SOURCE = "tiptap-papa-louie-3";
const PARENT_SOURCE = "tiptap-parent";

export function PapaLouie3Game({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/papa-louie-3/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena papa-louie-3-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena papa-louie-3-game"
      src={gameUrl}
      title="Play papa-louie-3"
