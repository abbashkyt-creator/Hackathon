...
const GAME_SOURCE = "tiptap-flick-goal";
const PARENT_SOURCE = "tiptap-parent";

export function FlickGoalGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/flick-goal/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena flick-goal-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena flick-goal-game"
      src={gameUrl}
      title="Play flick-goal"
