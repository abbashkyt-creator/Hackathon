...
const GAME_SOURCE = "tiptap-keyboard-warrior";
const PARENT_SOURCE = "tiptap-parent";

export function KeyboardWarriorGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/keyboard-warrior/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena keyboard-warrior-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena keyboard-warrior-game"
      src={gameUrl}
      title="Play keyboard-warrior"
