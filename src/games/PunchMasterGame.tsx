...
const GAME_SOURCE = "tiptap-punch-master";
const PARENT_SOURCE = "tiptap-parent";

export function PunchMasterGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/punch-master/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena punch-master-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena punch-master-game"
      src={gameUrl}
      title="Play punch-master"
