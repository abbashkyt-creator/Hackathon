...
const GAME_SOURCE = "tiptap-sword-road";
const PARENT_SOURCE = "tiptap-parent";

export function SwordRoadGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/sword-road/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena sword-road-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena sword-road-game"
      src={gameUrl}
      title="Play sword-road"
