...
const GAME_SOURCE = "tiptap-thung-wars";
const PARENT_SOURCE = "tiptap-parent";

export function ThungWarsGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/thung-wars/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena thung-wars-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena thung-wars-game"
      src={gameUrl}
      title="Play thung-wars"
