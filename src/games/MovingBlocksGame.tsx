...
const GAME_SOURCE = "tiptap-moving-blocks";
const PARENT_SOURCE = "tiptap-parent";

export function MovingBlocksGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/moving-blocks/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena moving-blocks-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena moving-blocks-game"
      src={gameUrl}
      title="Play moving-blocks"
