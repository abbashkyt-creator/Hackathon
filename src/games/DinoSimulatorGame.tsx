...
const GAME_SOURCE = "tiptap-dino-simulator";
const PARENT_SOURCE = "tiptap-parent";

export function DinoSimulatorGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/dino-simulator/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena dino-simulator-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena dino-simulator-game"
      src={gameUrl}
      title="Play dino-simulator"
