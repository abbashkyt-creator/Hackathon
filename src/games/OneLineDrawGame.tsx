...
const GAME_SOURCE = "tiptap-one-line-draw";
const PARENT_SOURCE = "tiptap-parent";

export function OneLineDrawGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/one-line-draw/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena one-line-draw-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena one-line-draw-game"
      src={gameUrl}
      title="Play one-line-draw"
