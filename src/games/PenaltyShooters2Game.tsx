...
const GAME_SOURCE = "tiptap-penalty-shooters-2";
const PARENT_SOURCE = "tiptap-parent";

export function PenaltyShooters2Game({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/penalty-shooters-2/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) {
    return <div className="game-arena penalty-shooters-2-placeholder" aria-hidden="true" />;
  }

  return (
    <iframe
      ref={frameRef}
      className="game-arena penalty-shooters-2-game"
      src={gameUrl}
      title="Play Penalty Shooters 2"
