...
const GAME_SOURCE = "tiptap-trapped-in-the-dollhouse";
const PARENT_SOURCE = "tiptap-parent";

export function TrappedInTheDollhouseGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/trapped-in-the-dollhouse/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena trapped-in-the-dollhouse-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena trapped-in-the-dollhouse-game"
      src={gameUrl}
      title="Play trapped-in-the-dollhouse"
