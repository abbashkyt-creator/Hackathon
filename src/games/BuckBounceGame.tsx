...
const GAME_SOURCE = "tiptap-buck-bounce";
const PARENT_SOURCE = "tiptap-parent";

export function BuckBounceGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/buck-bounce/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena buck-bounce-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena buck-bounce-game"
      src={gameUrl}
      title="Play buck-bounce"
