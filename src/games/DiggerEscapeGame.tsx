...
const GAME_SOURCE = "tiptap-digger-escape";
const PARENT_SOURCE = "tiptap-parent";

export function DiggerEscapeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/digger-escape/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena digger-escape-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena digger-escape-game"
      src={gameUrl}
      title="Play digger-escape"
