...
const GAME_SOURCE = "tiptap-snek-left";
const PARENT_SOURCE = "tiptap-parent";

export function SnekLeftGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/snek-left/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena snek-left-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena snek-left-game"
      src={gameUrl}
      title="Play snek-left"
