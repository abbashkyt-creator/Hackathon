...
const GAME_SOURCE = "tiptap-jelly-sokoban";
const PARENT_SOURCE = "tiptap-parent";

export function JellySokobanGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/jelly-sokoban/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena jelly-sokoban-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena jelly-sokoban-game"
      src={gameUrl}
      title="Play jelly-sokoban"
