...
const GAME_SOURCE = "tiptap-jelly-well";
const PARENT_SOURCE = "tiptap-parent";

export function JellyWellGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/jelly-well/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena jelly-well-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena jelly-well-game"
      src={gameUrl}
      title="Play jelly-well"
