...
const GAME_SOURCE = "tiptap-free-skate";
const PARENT_SOURCE = "tiptap-parent";

export function FreeSkateGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/free-skate/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena free-skate-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena free-skate-game"
      src={gameUrl}
      title="Play free-skate"
