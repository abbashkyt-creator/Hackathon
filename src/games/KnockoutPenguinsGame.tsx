...
const GAME_SOURCE = "tiptap-knockout-penguins";
const PARENT_SOURCE = "tiptap-parent";

export function KnockoutPenguinsGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/knockout-penguins/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena knockout-penguins-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena knockout-penguins-game"
      src={gameUrl}
      title="Play knockout-penguins"
