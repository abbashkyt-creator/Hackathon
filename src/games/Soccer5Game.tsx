...
const GAME_SOURCE = "tiptap-soccer-5";
const PARENT_SOURCE = "tiptap-parent";

export function Soccer5Game({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/soccer-5/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena soccer-5-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena soccer-5-game"
      src={gameUrl}
      title="Play soccer-5"
