...
const GAME_SOURCE = "tiptap-morph-balls";
const PARENT_SOURCE = "tiptap-parent";

export function MorphBallsGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/morph-balls/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena morph-balls-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena morph-balls-game"
      src={gameUrl}
      title="Play morph-balls"
