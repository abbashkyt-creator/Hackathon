...
const GAME_SOURCE = "tiptap-snow-plow";
const PARENT_SOURCE = "tiptap-parent";

export function SnowPlowGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/snow-plow/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena snow-plow-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena snow-plow-game"
      src={gameUrl}
      title="Play snow-plow"
