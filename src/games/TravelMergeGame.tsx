...
const GAME_SOURCE = "tiptap-travel-merge";
const PARENT_SOURCE = "tiptap-parent";

export function TravelMergeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/travel-merge/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena travel-merge-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena travel-merge-game"
      src={gameUrl}
      title="Play Travel Merge 1.11.00"
