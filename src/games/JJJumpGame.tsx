...
const GAME_SOURCE = "tiptap-j-j-jump";
const PARENT_SOURCE = "tiptap-parent";

export function JJJumpGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/j-j-jump/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena j-j-jump-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena j-j-jump-game"
      src={gameUrl}
      title="Play j-j-jump"
