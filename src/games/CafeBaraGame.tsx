...
const GAME_SOURCE = "tiptap-cafe-bara";
const PARENT_SOURCE = "tiptap-parent";

export function CafeBaraGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/cafe-bara/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena cafe-bara-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena cafe-bara-game"
      src={gameUrl}
      title="Play cafe-bara"
