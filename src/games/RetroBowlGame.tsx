...
const GAME_SOURCE = "tiptap-retro-bowl";
const PARENT_SOURCE = "tiptap-parent";

export function RetroBowlGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/retro-bowl/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) {
    return <div className="game-arena retro-bowl-placeholder" aria-hidden="true" />;
  }

  return (
    <iframe
      ref={frameRef}
      className="game-arena retro-bowl-game"
      src={gameUrl}
      title="Play Retro Bowl"
