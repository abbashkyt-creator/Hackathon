...
const GAME_SOURCE = "tiptap-merge-rot";
const PARENT_SOURCE = "tiptap-parent";

export function MergeRotGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/merge-rot/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena merge-rot-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena merge-rot-game"
      src={gameUrl}
      title="Play merge-rot"
