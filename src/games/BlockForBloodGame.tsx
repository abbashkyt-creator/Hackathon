...
const GAME_SOURCE = "tiptap-block-for-blood";
const PARENT_SOURCE = "tiptap-parent";

export function BlockForBloodGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/block-for-blood/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena block-for-blood-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena block-for-blood-game"
      src={gameUrl}
      title="Play Block for Blood"
