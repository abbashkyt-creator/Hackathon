...
const GAME_SOURCE = "tiptap-jetpack-fury";
const PARENT_SOURCE = "tiptap-parent";

export function JetpackFuryGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/jetpack-fury/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena jetpack-fury-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena jetpack-fury-game"
      src={gameUrl}
      title="Play jetpack-fury"
