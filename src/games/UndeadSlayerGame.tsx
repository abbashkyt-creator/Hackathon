...
const GAME_SOURCE = "tiptap-undead-slayer";
const PARENT_SOURCE = "tiptap-parent";

export function UndeadSlayerGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/undead-slayer/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena undead-slayer-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena undead-slayer-game"
      src={gameUrl}
      title="Play undead-slayer"
