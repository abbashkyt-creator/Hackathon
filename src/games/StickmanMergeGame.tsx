...
const GAME_SOURCE = "tiptap-stickman-merge";
const PARENT_SOURCE = "tiptap-parent";

export function StickmanMergeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/stickman-merge/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena stickman-merge-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena stickman-merge-game"
      src={gameUrl}
      title="Play stickman-merge"
