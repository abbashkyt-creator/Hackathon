...
const GAME_SOURCE = "tiptap-mirror-image";
const PARENT_SOURCE = "tiptap-parent";

export function MirrorImageGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/mirror-image/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena mirror-image-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena mirror-image-game"
      src={gameUrl}
      title="Play mirror-image"
