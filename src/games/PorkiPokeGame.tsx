...
const GAME_SOURCE = "tiptap-porki-poke";
const PARENT_SOURCE = "tiptap-parent";

export function PorkiPokeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/porki-poke/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena porki-poke-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena porki-poke-game"
      src={gameUrl}
      title="Play porki-poke"
