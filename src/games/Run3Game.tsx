...
const GAME_SOURCE = "tiptap-run-3";
const PARENT_SOURCE = "tiptap-parent";

export function Run3Game({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/run-3/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena run-3-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena run-3-game"
      src={gameUrl}
      title="Play run-3"
