...
const GAME_SOURCE = "tiptap-magic-battleground";
const PARENT_SOURCE = "tiptap-parent";

export function MagicBattlegroundGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/magic-battleground/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena magic-battleground-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena magic-battleground-game"
      src={gameUrl}
      title="Play magic-battleground"
