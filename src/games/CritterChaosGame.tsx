...
const GAME_SOURCE = "tiptap-critter-chaos";
const PARENT_SOURCE = "tiptap-parent";

export function CritterChaosGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/critter-chaos/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena critter-chaos-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena critter-chaos-game"
      src={gameUrl}
      title="Play Squad CTF Prototype"
