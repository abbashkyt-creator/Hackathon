...
const GAME_SOURCE = "tiptap-moto-x3m";
const PARENT_SOURCE = "tiptap-parent";

export function MotoX3mGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/moto-x3m/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena moto-x3m-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena moto-x3m-game"
      src={gameUrl}
      title="Play moto-x3m"
