...
const GAME_SOURCE = "tiptap-eggsplosion";
const PARENT_SOURCE = "tiptap-parent";

export function EggsplosionGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/eggsplosion/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena eggsplosion-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena eggsplosion-game"
      src={gameUrl}
      title="Play eggsplosion"
