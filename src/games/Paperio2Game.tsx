...
const GAME_SOURCE = "tiptap-paperio-2";
const PARENT_SOURCE = "tiptap-parent";

export function Paperio2Game({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/paperio-2/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena paperio-2-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena paperio-2-game"
      src={gameUrl}
      title="Play Paper.io 2"
