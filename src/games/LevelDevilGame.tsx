...
const GAME_SOURCE = "tiptap-level-devil";
const PARENT_SOURCE = "tiptap-parent";

export function LevelDevilGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/level-devil/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) {
    return <div className="game-arena level-devil-placeholder" aria-hidden="true" />;
  }

  return (
    <iframe
      ref={frameRef}
      className="game-arena level-devil-game"
      src={gameUrl}
      title="Play Level Devil"
