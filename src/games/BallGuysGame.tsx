...
const GAME_SOURCE = "tiptap-ball-guys";
const PARENT_SOURCE = "tiptap-parent";

export function BallGuysGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/ball-guys/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena ball-guys-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena ball-guys-game"
      src={gameUrl}
      title="Play ball-guys"
