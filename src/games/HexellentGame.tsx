...
const GAME_SOURCE = "tiptap-hexellent";
const PARENT_SOURCE = "tiptap-parent";

export function HexellentGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/hexellent/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena hexellent-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena hexellent-game"
      src={gameUrl}
      title="Play hexellent"
