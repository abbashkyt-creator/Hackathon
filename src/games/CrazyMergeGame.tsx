...
const GAME_SOURCE = "tiptap-crazy-merge";
const PARENT_SOURCE = "tiptap-parent";

export function CrazyMergeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/crazy-merge/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena crazy-merge-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena crazy-merge-game"
      src={gameUrl}
      title="Play crazy-merge"
