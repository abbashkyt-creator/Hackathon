...
const GAME_SOURCE = "tiptap-drive-mad";
const PARENT_SOURCE = "tiptap-parent";

export function DriveMadGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/drive-mad/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) {
    return <div className="game-arena drive-mad-placeholder" aria-hidden="true" />;
  }

  return (
    <iframe
      ref={frameRef}
      className="game-arena drive-mad-game"
      src={gameUrl}
      title="Play Drive Mad"
