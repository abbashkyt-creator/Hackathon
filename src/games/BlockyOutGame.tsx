...
const GAME_SOURCE = "tiptap-blocky-out";
const PARENT_SOURCE = "tiptap-parent";

export function BlockyOutGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/blocky-out/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena blocky-out-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena blocky-out-game"
      src={gameUrl}
      title="Play blocky-out"
