...
const GAME_SOURCE = "tiptap-dummies-fight";
const PARENT_SOURCE = "tiptap-parent";

export function DummiesFightGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/dummies-fight/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena dummies-fight-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena dummies-fight-game"
      src={gameUrl}
      title="Play dummies-fight"
