...
const GAME_SOURCE = "tiptap-bacon-survivor";
const PARENT_SOURCE = "tiptap-parent";

export function BaconSurvivorGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/bacon-survivor/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena bacon-survivor-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena bacon-survivor-game"
      src={gameUrl}
      title="Play bacon-survivor"
