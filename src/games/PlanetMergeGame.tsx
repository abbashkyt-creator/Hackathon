...
const GAME_SOURCE = "tiptap-planet-merge";
const PARENT_SOURCE = "tiptap-parent";

export function PlanetMergeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/planet-merge/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena planet-merge-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena planet-merge-game"
      src={gameUrl}
      title="Play planet-merge"
