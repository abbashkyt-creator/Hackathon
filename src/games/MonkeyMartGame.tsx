...
const GAME_SOURCE = "tiptap-monkey-mart";
const PARENT_SOURCE = "tiptap-parent";

export function MonkeyMartGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/monkey-mart/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena monkey-mart-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena monkey-mart-game"
      src={gameUrl}
      title="Play MonkeyMart"
