...
const GAME_SOURCE = "tiptap-bloku";
const PARENT_SOURCE = "tiptap-parent";

export function BlokuGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/bloku/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena bloku-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena bloku-game"
      src={gameUrl}
      title="Play bloku"
