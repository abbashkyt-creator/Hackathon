...
const GAME_SOURCE = "tiptap-carnado-bike-stunt";
const PARENT_SOURCE = "tiptap-parent";

export function CarnadoBikeStuntGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/carnado-bike-stunt/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena carnado-bike-stunt-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena carnado-bike-stunt-game"
      src={gameUrl}
      title="Play CARNADO BIKE STUNT"
