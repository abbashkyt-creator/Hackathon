...
const GAME_SOURCE = "tiptap-turnament";
const PARENT_SOURCE = "tiptap-parent";

export function TurnamentGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/turnament/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena turnament-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena turnament-game"
      src={gameUrl}
      title="Play turnament"
