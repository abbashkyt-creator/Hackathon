...
const GAME_SOURCE = "tiptap-pizza-planet";
const PARENT_SOURCE = "tiptap-parent";

export function PizzaPlanetGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/pizza-planet/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena pizza-planet-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena pizza-planet-game"
      src={gameUrl}
      title="Play pizza-planet"
