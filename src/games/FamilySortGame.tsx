...
const GAME_SOURCE = "tiptap-family-sort";
const PARENT_SOURCE = "tiptap-parent";

export function FamilySortGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/family-sort/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena family-sort-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena family-sort-game"
      src={gameUrl}
      title="Play family-sort"
