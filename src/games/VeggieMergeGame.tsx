...
const GAME_SOURCE = "tiptap-veggie-merge";
const PARENT_SOURCE = "tiptap-parent";

export function VeggieMergeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/veggie-merge/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena veggie-merge-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena veggie-merge-game"
      src={gameUrl}
      title="Play veggie-merge"
