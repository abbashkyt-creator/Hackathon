...
const GAME_SOURCE = "tiptap-bubble-heroes";
const PARENT_SOURCE = "tiptap-parent";

export function BubbleHeroesGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/bubble-heroes/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena bubble-heroes-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena bubble-heroes-game"
      src={gameUrl}
      title="Play bubble-heroes"
