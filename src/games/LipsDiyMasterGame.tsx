...
const GAME_SOURCE = "tiptap-lips-diy-master";
const PARENT_SOURCE = "tiptap-parent";

export function LipsDiyMasterGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/lips-diy-master/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena lips-diy-master-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena lips-diy-master-game"
      src={gameUrl}
      title="Play Lips"
