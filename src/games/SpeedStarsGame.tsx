...
const GAME_SOURCE = "tiptap-speed-stars";
const PARENT_SOURCE = "tiptap-parent";

export function SpeedStarsGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/speed-stars/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena speed-stars-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena speed-stars-game"
      src={gameUrl}
      title="Play speed-stars"
