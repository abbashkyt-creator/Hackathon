...
const GAME_SOURCE = "tiptap-hero-vs-criminal";
const PARENT_SOURCE = "tiptap-parent";

export function HeroVsCriminalGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/hero-vs-criminal/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena hero-vs-criminal-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena hero-vs-criminal-game"
      src={gameUrl}
      title="Play hero-vs-criminal"
