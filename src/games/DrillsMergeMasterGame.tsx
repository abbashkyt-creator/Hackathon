...
const GAME_SOURCE = "tiptap-drills-merge-master";
const PARENT_SOURCE = "tiptap-parent";

export function DrillsMergeMasterGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/drills-merge-master/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena drills-merge-master-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena drills-merge-master-game"
      src={gameUrl}
      title="Play drills-merge-master"
