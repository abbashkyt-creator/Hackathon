...
const GAME_SOURCE = "tiptap-ball-vs-block";
const PARENT_SOURCE = "tiptap-parent";

export function BallVsBlockGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/ball-vs-block/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena ball-vs-block-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena ball-vs-block-game"
      src={gameUrl}
      title="Play ball-vs-block"
