...
const GAME_SOURCE = "tiptap-stickman-maze-run";
const PARENT_SOURCE = "tiptap-parent";

export function StickmanMazeRunGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/stickman-maze-run/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena stickman-maze-run-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena stickman-maze-run-game"
      src={gameUrl}
      title="Play stickman-maze-run"
