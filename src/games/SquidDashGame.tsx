...
const GAME_SOURCE = "tiptap-squid-dash";
const PARENT_SOURCE = "tiptap-parent";

export function SquidDashGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/squid-dash/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena squid-dash-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena squid-dash-game"
      src={gameUrl}
      title="Play squid-dash"
