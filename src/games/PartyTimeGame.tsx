...
const GAME_SOURCE = "tiptap-party-time";
const PARENT_SOURCE = "tiptap-parent";

export function PartyTimeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/party-time/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena party-time-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena party-time-game"
      src={gameUrl}
      title="Play party-time"
