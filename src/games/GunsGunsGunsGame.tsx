...
const GAME_SOURCE = "tiptap-guns-guns-guns";
const PARENT_SOURCE = "tiptap-parent";

export function GunsGunsGunsGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/guns-guns-guns/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena guns-guns-guns-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena guns-guns-guns-game"
      src={gameUrl}
      title="Play guns-guns-guns"
