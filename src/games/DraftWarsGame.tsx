...
const GAME_SOURCE = "tiptap-draft-wars";
const PARENT_SOURCE = "tiptap-parent";

export function DraftWarsGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/draft-wars/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena draft-wars-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena draft-wars-game"
      src={gameUrl}
      title="Play draft-wars"
