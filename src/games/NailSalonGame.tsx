...
const GAME_SOURCE = "tiptap-nail-salon";
const PARENT_SOURCE = "tiptap-parent";

export function NailSalonGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/nail-salon/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena nail-salon-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena nail-salon-game"
      src={gameUrl}
      title="Play nail-salon"
