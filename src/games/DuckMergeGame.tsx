...
const GAME_SOURCE = "tiptap-duck-merge";
const PARENT_SOURCE = "tiptap-parent";

export function DuckMergeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/duck-merge/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena duck-merge-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena duck-merge-game"
      src={gameUrl}
      title="Play duck-merge"
