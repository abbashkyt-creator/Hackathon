...
const GAME_SOURCE = "tiptap-critters-quest";
const PARENT_SOURCE = "tiptap-parent";

export function CrittersQuestGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/critters-quest/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena critters-quest-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena critters-quest-game"
      src={gameUrl}
      title="Play critters-quest"
