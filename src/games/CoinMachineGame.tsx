...
const GAME_SOURCE = "tiptap-coin-machine";
const PARENT_SOURCE = "tiptap-parent";

export function CoinMachineGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/coin-machine/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena coin-machine-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena coin-machine-game"
      src={gameUrl}
      title="Play coin-machine"
