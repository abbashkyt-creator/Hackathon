...
const GAME_SOURCE = "tiptap-chess-multiplayer";
const PARENT_SOURCE = "tiptap-parent";

export function ChessMultiplayerGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/chess-multiplayer/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena chess-multiplayer-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena chess-multiplayer-game"
      src={gameUrl}
