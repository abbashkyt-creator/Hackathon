...
const GAME_SOURCE = "tiptap-obby-roads";
const PARENT_SOURCE = "tiptap-parent";

export function ObbyRoadsGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/obby-roads/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena obby-roads-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena obby-roads-game"
      src={gameUrl}
      title="Play obby-roads"
