...
const GAME_SOURCE = "tiptap-goal-gang";
const PARENT_SOURCE = "tiptap-parent";

export function GoalGangGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/goal-gang/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena goal-gang-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena goal-gang-game"
      src={gameUrl}
      title="Play goal-gang"
