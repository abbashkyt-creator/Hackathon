...
const GAME_SOURCE = "tiptap-fashion-legends";
const PARENT_SOURCE = "tiptap-parent";

export function FashionLegendsGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/fashion-legends/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena fashion-legends-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena fashion-legends-game"
      src={gameUrl}
      title="Play fashion-legends"
