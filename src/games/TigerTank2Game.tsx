...
const GAME_SOURCE = "tiptap-tiger-tank-2";
const PARENT_SOURCE = "tiptap-parent";

export function TigerTank2Game({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/tiger-tank-2/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena tiger-tank-2-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena tiger-tank-2-game"
      src={gameUrl}
      title="Play Tiger Tank 2"
