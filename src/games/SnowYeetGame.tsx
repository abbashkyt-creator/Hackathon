...
const GAME_SOURCE = "tiptap-snow-yeet";
const PARENT_SOURCE = "tiptap-parent";

export function SnowYeetGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/snow-yeet/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena snow-yeet-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena snow-yeet-game"
      src={gameUrl}
      title="Play Snow Yeet"
