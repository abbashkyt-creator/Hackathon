...
const GAME_SOURCE = "tiptap-subway-surfers";
const PARENT_SOURCE = "tiptap-parent";

export function SubwaySurfersGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/subway-surfers/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) {
    return <div className="game-arena subway-surfers-placeholder" aria-hidden="true" />;
  }

  return (
    <iframe
      ref={frameRef}
      className="game-arena subway-surfers-game"
      src={gameUrl}
      title="Play Subway Surfers"
