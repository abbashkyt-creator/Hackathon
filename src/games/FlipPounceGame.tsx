...
const GAME_SOURCE = "tiptap-flip-pounce";
const PARENT_SOURCE = "tiptap-parent";

export function FlipPounceGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/flip-pounce/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena flip-pounce-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena flip-pounce-game"
      src={gameUrl}
      title="Play flip-pounce"
