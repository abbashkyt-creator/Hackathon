...
const GAME_SOURCE = "tiptap-basketball-stars";
const PARENT_SOURCE = "tiptap-parent";

export function BasketballStarsGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/basketball-stars/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) {
    return <div className="game-arena basketball-stars-placeholder" aria-hidden="true" />;
  }

  return (
    <iframe
      ref={frameRef}
      className="game-arena basketball-stars-game"
      src={gameUrl}
      title="Play Basketball Stars"
