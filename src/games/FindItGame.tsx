...
const GAME_SOURCE = "tiptap-find-it";
const PARENT_SOURCE = "tiptap-parent";

export function FindItGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/find-it/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena find-it-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena find-it-game"
      src={gameUrl}
      title="Play find-it"
