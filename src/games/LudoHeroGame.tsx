...
const GAME_SOURCE = "tiptap-ludo-hero";
const PARENT_SOURCE = "tiptap-parent";

export function LudoHeroGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/ludo-hero/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena ludo-hero-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena ludo-hero-game"
      src={gameUrl}
      title="Play ludo-hero"
