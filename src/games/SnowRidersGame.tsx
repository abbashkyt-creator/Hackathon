...
const GAME_SOURCE = "tiptap-snow-riders";
const PARENT_SOURCE = "tiptap-parent";

export function SnowRidersGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/snow-riders/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena snow-riders-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena snow-riders-game"
      src={gameUrl}
      title="Play snow-riders"
