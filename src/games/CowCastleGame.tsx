...
const GAME_SOURCE = "tiptap-cow-castle";
const PARENT_SOURCE = "tiptap-parent";

export function CowCastleGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/cow-castle/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena cow-castle-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena cow-castle-game"
      src={gameUrl}
      title="Play cow-castle"
