...
const GAME_SOURCE = "tiptap-whip-flip";
const PARENT_SOURCE = "tiptap-parent";

export function WhipFlipGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/whip-flip/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena whip-flip-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena whip-flip-game"
      src={gameUrl}
      title="Play whip-flip"
