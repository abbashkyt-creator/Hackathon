...
const GAME_SOURCE = "tiptap-clean-house";
const PARENT_SOURCE = "tiptap-parent";

export function CleanHouseGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/clean-house/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena clean-house-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena clean-house-game"
      src={gameUrl}
      title="Play Clean House - Poki"
