...
const GAME_SOURCE = "tiptap-zombit";
const PARENT_SOURCE = "tiptap-parent";

export function ZombitGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/zombit/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena zombit-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena zombit-game"
      src={gameUrl}
      title="Play zombit"
