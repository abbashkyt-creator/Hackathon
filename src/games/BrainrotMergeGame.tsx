...
const GAME_SOURCE = "tiptap-brainrot-merge";
const PARENT_SOURCE = "tiptap-parent";

export function BrainrotMergeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/brainrot-merge/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena brainrot-merge-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena brainrot-merge-game"
      src={gameUrl}
      title="Play brainrot-merge"
