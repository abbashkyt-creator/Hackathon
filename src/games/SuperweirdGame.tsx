...
const GAME_SOURCE = "tiptap-superweird";
const PARENT_SOURCE = "tiptap-parent";

export function SuperweirdGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/superweird/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena superweird-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena superweird-game"
      src={gameUrl}
      title="Play superweird"
