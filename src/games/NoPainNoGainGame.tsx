...
const GAME_SOURCE = "tiptap-no-pain-no-gain";
const PARENT_SOURCE = "tiptap-parent";

export function NoPainNoGainGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/no-pain-no-gain/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena no-pain-no-gain-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena no-pain-no-gain-game"
      src={gameUrl}
      title="Play no-pain-no-gain"
