...
const GAME_SOURCE = "tiptap-doodle-race";
const PARENT_SOURCE = "tiptap-parent";

export function DoodleRaceGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/doodle-race/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena doodle-race-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena doodle-race-game"
      src={gameUrl}
      title="Play doodle-race"
