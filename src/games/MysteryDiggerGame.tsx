...
const GAME_SOURCE = "tiptap-mystery-digger";
const PARENT_SOURCE = "tiptap-parent";

export function MysteryDiggerGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/mystery-digger/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena mystery-digger-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena mystery-digger-game"
      src={gameUrl}
      title="Play mystery-digger"
