...
const GAME_SOURCE = "tiptap-bee-sort-by-sam";
const PARENT_SOURCE = "tiptap-parent";

export function BeeSortBySamGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/bee-sort-by-sam/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena bee-sort-by-sam-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena bee-sort-by-sam-game"
      src={gameUrl}
      title="Play bee-sort-by-sam"
