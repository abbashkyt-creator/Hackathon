...
const GAME_SOURCE = "tiptap-subway-surfers-match";
const PARENT_SOURCE = "tiptap-parent";

export function SubwaySurfersMatchGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/subway-surfers-match/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena subway-surfers-match-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena subway-surfers-match-game"
      src={gameUrl}
      title="Play subway-surfers-match"
