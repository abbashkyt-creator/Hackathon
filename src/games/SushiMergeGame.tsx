...
const GAME_SOURCE = "tiptap-sushi-merge";
const PARENT_SOURCE = "tiptap-parent";

export function SushiMergeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/sushi-merge/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena sushi-merge-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena sushi-merge-game"
      src={gameUrl}
      title="Play sushi-merge"
