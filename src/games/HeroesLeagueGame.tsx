...
const GAME_SOURCE = "tiptap-heroes-league";
const PARENT_SOURCE = "tiptap-parent";

export function HeroesLeagueGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/heroes-league/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena heroes-league-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena heroes-league-game"
      src={gameUrl}
      title="Play heroes-league"
