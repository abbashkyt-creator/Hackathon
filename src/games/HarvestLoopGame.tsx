...
const GAME_SOURCE = "tiptap-harvest-loop";
const PARENT_SOURCE = "tiptap-parent";

export function HarvestLoopGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/harvest-loop/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena harvest-loop-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena harvest-loop-game"
      src={gameUrl}
      title="Play harvest-loop"
