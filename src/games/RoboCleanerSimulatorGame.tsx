...
const GAME_SOURCE = "tiptap-robo-cleaner-simulator";
const PARENT_SOURCE = "tiptap-parent";

export function RoboCleanerSimulatorGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/robo-cleaner-simulator/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena robo-cleaner-simulator-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena robo-cleaner-simulator-game"
      src={gameUrl}
      title="Play robo cleaner"
