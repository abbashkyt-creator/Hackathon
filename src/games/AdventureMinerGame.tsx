...
const GAME_SOURCE = "tiptap-adventure-miner";
const PARENT_SOURCE = "tiptap-parent";

export function AdventureMinerGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/adventure-miner/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena adventure-miner-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena adventure-miner-game"
      src={gameUrl}
      title="Play adventure-miner"
