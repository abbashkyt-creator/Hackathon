...
const GAME_SOURCE = "tiptap-car-circle";
const PARENT_SOURCE = "tiptap-parent";

export function CarCircleGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/car-circle/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena car-circle-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena car-circle-game"
      src={gameUrl}
      title="Play car-circle"
