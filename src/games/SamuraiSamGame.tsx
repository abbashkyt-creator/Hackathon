...
const GAME_SOURCE = "tiptap-samurai-sam";
const PARENT_SOURCE = "tiptap-parent";

export function SamuraiSamGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/samurai-sam/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena samurai-sam-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena samurai-sam-game"
      src={gameUrl}
      title="Play samurai-sam"
