...
const GAME_SOURCE = "tiptap-cleanup-crew";
const PARENT_SOURCE = "tiptap-parent";

export function CleanupCrewGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/cleanup-crew/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena cleanup-crew-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena cleanup-crew-game"
      src={gameUrl}
      title="Play cleanup-crew"
