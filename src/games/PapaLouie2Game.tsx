...
const GAME_SOURCE = "tiptap-papa-louie-2";
const PARENT_SOURCE = "tiptap-parent";

export function PapaLouie2Game({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/papa-louie-2/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena papa-louie-2-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena papa-louie-2-game"
      src={gameUrl}
      title="Play papa-louie-2"
