...
const GAME_SOURCE = "tiptap-slime-dunk";
const PARENT_SOURCE = "tiptap-parent";

export function SlimeDunkGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/slime-dunk/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena slime-dunk-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena slime-dunk-game"
      src={gameUrl}
      title="Play slime-dunk"
