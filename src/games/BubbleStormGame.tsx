...
const GAME_SOURCE = "tiptap-bubble-storm";
const PARENT_SOURCE = "tiptap-parent";

export function BubbleStormGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/bubble-storm/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena bubble-storm-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena bubble-storm-game"
      src={gameUrl}
      title="Play bubble-storm"
