...
const GAME_SOURCE = "tiptap-block-sandbox";
const PARENT_SOURCE = "tiptap-parent";

export function BlockSandboxGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/block-sandbox/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena block-sandbox-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena block-sandbox-game"
      src={gameUrl}
      title="Play block-sandbox"
