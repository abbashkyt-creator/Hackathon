...
const GAME_SOURCE = "tiptap-calamari";
const PARENT_SOURCE = "tiptap-parent";

export function CalamariGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/calamari/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena calamari-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena calamari-game"
      src={gameUrl}
      title="Play calamari"
