...
const GAME_SOURCE = "tiptap-pets-park";
const PARENT_SOURCE = "tiptap-parent";

export function PetsParkGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/pets-park/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena pets-park-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena pets-park-game"
      src={gameUrl}
      title="Play pets-park"
