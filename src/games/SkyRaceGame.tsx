...
const GAME_SOURCE = "tiptap-sky-race";
const PARENT_SOURCE = "tiptap-parent";

export function SkyRaceGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/sky-race/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena sky-race-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena sky-race-game"
      src={gameUrl}
      title="Play sky-race"
