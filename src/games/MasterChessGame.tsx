...
const GAME_SOURCE = "tiptap-master-chess";
const PARENT_SOURCE = "tiptap-parent";

export function MasterChessGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/master-chess/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) {
    return <div className="game-arena master-chess-placeholder" aria-hidden="true" />;
  }

  return (
    <iframe
      ref={frameRef}
      className="game-arena master-chess-game"
      src={gameUrl}
      title="Play Master Chess"
