...
const GAME_SOURCE = "tiptap-ludo-online";
const PARENT_SOURCE = "tiptap-parent";

export function LudoOnlineGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/ludo-online/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena ludo-online-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena ludo-online-game"
      src={gameUrl}
