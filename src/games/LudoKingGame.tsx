...
const GAME_SOURCE = "tiptap-ludo-king";
const PARENT_SOURCE = "tiptap-parent";

export function LudoKingGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/ludo-king/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena ludo-king-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena ludo-king-game"
      src={gameUrl}
      title="Play ludo-king"
