...
const GAME_SOURCE = "tiptap-bos-bedroom";
const PARENT_SOURCE = "tiptap-parent";

export function BosBedroomGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/bos-bedroom/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena bos-bedroom-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena bos-bedroom-game"
      src={gameUrl}
      title="Play bos-bedroom"
