...
const GAME_SOURCE = "tiptap-blast-buddies";
const PARENT_SOURCE = "tiptap-parent";

export function BlastBuddiesGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/blast-buddies/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena blast-buddies-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena blast-buddies-game"
      src={gameUrl}
      title="Play blast-buddies"
