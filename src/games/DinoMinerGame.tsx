...
const GAME_SOURCE = "tiptap-dino-miner";
const PARENT_SOURCE = "tiptap-parent";

export function DinoMinerGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/dino-miner/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena dino-miner-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena dino-miner-game"
      src={gameUrl}
      title="Play dino-miner"
