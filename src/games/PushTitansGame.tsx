...
const GAME_SOURCE = "tiptap-push-titans";
const PARENT_SOURCE = "tiptap-parent";

export function PushTitansGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/push-titans/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena push-titans-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena push-titans-game"
      src={gameUrl}
      title="Play push-titans"
