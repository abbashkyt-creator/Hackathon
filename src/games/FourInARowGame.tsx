...
const GAME_SOURCE = "tiptap-four-in-a-row";
const PARENT_SOURCE = "tiptap-parent";

export function FourInARowGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/four-in-a-row/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena four-in-a-row-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena four-in-a-row-game"
      src={gameUrl}
