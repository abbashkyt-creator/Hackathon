...
const GAME_SOURCE = "tiptap-skill-knight";
const PARENT_SOURCE = "tiptap-parent";

export function SkillKnightGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/skill-knight/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena skill-knight-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena skill-knight-game"
      src={gameUrl}
      title="Play skill-knight"
