...
const GAME_SOURCE = "tiptap-eyes-drop";
const PARENT_SOURCE = "tiptap-parent";

export function EyesDropGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/eyes-drop/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena eyes-drop-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena eyes-drop-game"
      src={gameUrl}
      title="Play eyes-drop"
