...
const GAME_SOURCE = "tiptap-elemental-master";
const PARENT_SOURCE = "tiptap-parent";

export function ElementalMasterGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/elemental-master/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena elemental-master-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena elemental-master-game"
      src={gameUrl}
      title="Play elemental-master"
