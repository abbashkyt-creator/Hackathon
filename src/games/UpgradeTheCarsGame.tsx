...
const GAME_SOURCE = "tiptap-upgrade-the-cars";
const PARENT_SOURCE = "tiptap-parent";

export function UpgradeTheCarsGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/upgrade-the-cars/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena upgrade-the-cars-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena upgrade-the-cars-game"
      src={gameUrl}
      title="Play upgrade-the-cars"
