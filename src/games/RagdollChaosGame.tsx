...
const GAME_SOURCE = "tiptap-ragdoll-chaos";
const PARENT_SOURCE = "tiptap-parent";

export function RagdollChaosGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/ragdoll-chaos/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena ragdoll-chaos-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena ragdoll-chaos-game"
      src={gameUrl}
      title="Play ragdoll-chaos"
