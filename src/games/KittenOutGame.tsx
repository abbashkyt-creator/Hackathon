...
const GAME_SOURCE = "tiptap-kitten-out";
const PARENT_SOURCE = "tiptap-parent";

export function KittenOutGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/kitten-out/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena kitten-out-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena kitten-out-game"
      src={gameUrl}
      title="Play kitten-out"
