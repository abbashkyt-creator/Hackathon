...
const GAME_SOURCE = "tiptap-beauty-salon";
const PARENT_SOURCE = "tiptap-parent";

export function BeautySalonGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/beauty-salon/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena beauty-salon-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena beauty-salon-game"
      src={gameUrl}
      title="Play BeautySalon"
