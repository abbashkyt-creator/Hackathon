...
const GAME_SOURCE = "tiptap-bubble-tower";
const PARENT_SOURCE = "tiptap-parent";

export function BubbleTowerGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/bubble-tower/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena bubble-tower-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena bubble-tower-game"
      src={gameUrl}
      title="Play bubble-tower"
