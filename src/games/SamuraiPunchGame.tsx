...
const GAME_SOURCE = "tiptap-samurai-punch";
const PARENT_SOURCE = "tiptap-parent";

export function SamuraiPunchGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/samurai-punch/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena samurai-punch-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena samurai-punch-game"
      src={gameUrl}
      title="Play samurai-punch"
