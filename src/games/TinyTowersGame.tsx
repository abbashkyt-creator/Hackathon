...
const GAME_SOURCE = "tiptap-tiny-towers";
const PARENT_SOURCE = "tiptap-parent";

export function TinyTowersGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/tiny-towers/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena tiny-towers-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena tiny-towers-game"
      src={gameUrl}
      title="Play tiny-towers"
