...
const GAME_SOURCE = "tiptap-basketball-superstars";
const PARENT_SOURCE = "tiptap-parent";

export function BasketballSuperstarsGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/basketball-superstars/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena basketball-superstars-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena basketball-superstars-game"
      src={gameUrl}
      title="Play basketball-superstars"
