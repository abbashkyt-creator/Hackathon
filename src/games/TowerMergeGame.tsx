...
const GAME_SOURCE = "tiptap-tower-merge";
const PARENT_SOURCE = "tiptap-parent";

export function TowerMergeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/tower-merge/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena tower-merge-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena tower-merge-game"
      src={gameUrl}
      title="Play tower-merge"
