...
const GAME_SOURCE = "tiptap-chick-flix";
const PARENT_SOURCE = "tiptap-parent";

export function ChickFlixGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/chick-flix/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena chick-flix-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena chick-flix-game"
      src={gameUrl}
      title="Play chick-flix"
