...
const GAME_SOURCE = "tiptap-metamon";
const PARENT_SOURCE = "tiptap-parent";

export function MetamonGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/metamon/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena metamon-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena metamon-game"
      src={gameUrl}
      title="Play metamon"
