...
const GAME_SOURCE = "tiptap-master-checkers";
const PARENT_SOURCE = "tiptap-parent";

export function MasterCheckersGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/master-checkers/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena master-checkers-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena master-checkers-game"
      src={gameUrl}
      title="Play master-checkers"
