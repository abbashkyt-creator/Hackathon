...
const GAME_SOURCE = "tiptap-happy-tidy-time";
const PARENT_SOURCE = "tiptap-parent";

export function HappyTidyTimeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/happy-tidy-time/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena happy-tidy-time-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena happy-tidy-time-game"
      src={gameUrl}
      title="Play Happy Tidy Time"
