...
const GAME_SOURCE = "tiptap-racing-rampage";
const PARENT_SOURCE = "tiptap-parent";

export function RacingRampageGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/racing-rampage/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena racing-rampage-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena racing-rampage-game"
      src={gameUrl}
      title="Play racing-rampage"
