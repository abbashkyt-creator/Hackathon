...
const GAME_SOURCE = "tiptap-need-for-madness";
const PARENT_SOURCE = "tiptap-parent";

export function NeedForMadnessGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/need-for-madness/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena need-for-madness-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena need-for-madness-game"
      src={gameUrl}
      title="Play need-for-madness"
