...
const GAME_SOURCE = "tiptap-meme-madness";
const PARENT_SOURCE = "tiptap-parent";

export function MemeMadnessGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/meme-madness/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena meme-madness-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena meme-madness-game"
      src={gameUrl}
      title="Play meme-madness"
