...
const GAME_SOURCE = "tiptap-right-jump";
const PARENT_SOURCE = "tiptap-parent";

export function RightJumpGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/right-jump/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena right-jump-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena right-jump-game"
      src={gameUrl}
      title="Play right-jump"
