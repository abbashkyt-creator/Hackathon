...
const GAME_SOURCE = "tiptap-hockey-taka";
const PARENT_SOURCE = "tiptap-parent";

export function HockeyTakaGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/hockey-taka/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena hockey-taka-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena hockey-taka-game"
      src={gameUrl}
      title="Play hockey-taka"
