...
const GAME_SOURCE = "tiptap-blockpost";
const PARENT_SOURCE = "tiptap-parent";

export function BlockpostGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/blockpost/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena blockpost-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena blockpost-game"
      src={gameUrl}
      title="Play blockpost"
