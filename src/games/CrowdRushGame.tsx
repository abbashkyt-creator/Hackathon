...
const GAME_SOURCE = "tiptap-crowd-rush";
const PARENT_SOURCE = "tiptap-parent";

export function CrowdRushGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/crowd-rush/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena crowd-rush-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena crowd-rush-game"
      src={gameUrl}
      title="Play crowd-rush"
