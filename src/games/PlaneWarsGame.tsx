...
const GAME_SOURCE = "tiptap-plane-wars";
const PARENT_SOURCE = "tiptap-parent";

export function PlaneWarsGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/plane-wars/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena plane-wars-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena plane-wars-game"
      src={gameUrl}
      title="Play plane-wars"
