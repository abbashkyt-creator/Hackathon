...
const GAME_SOURCE = "tiptap-vortex-racer";
const PARENT_SOURCE = "tiptap-parent";

export function VortexRacerGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/vortex-racer/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena vortex-racer-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena vortex-racer-game"
      src={gameUrl}
      title="Play vortex-racer"
