...
const GAME_SOURCE = "tiptap-dino-quake";
const PARENT_SOURCE = "tiptap-parent";

export function DinoQuakeGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/dino-quake/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) return <div className="game-arena dino-quake-placeholder" aria-hidden="true" />;

  return (
    <iframe
      ref={frameRef}
      className="game-arena dino-quake-game"
      src={gameUrl}
      title="Play dino-quake"
