...
const GAME_SOURCE = "tiptap-drift-boss";
const PARENT_SOURCE = "tiptap-parent";

export function DriftBossGame({
  active,
  runKey,
  soundEnabled,
  hapticsEnabled,
  ...
  const gameUrl = useMemo(() => {
    ...
    return `/games/drift-boss/index.html?${params}`;
  }, [active, runKey]);
  ...
  if (!active) {
    return <div className="game-arena drift-boss-placeholder" aria-hidden="true" />;
  }

  return (
    <iframe
      ref={frameRef}
      className="game-arena drift-boss-game"
      src={gameUrl}
      title="Play Drift Boss"
