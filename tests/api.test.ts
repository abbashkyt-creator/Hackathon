...
describe("Tip Tap API", () => {
  ...
  it("creates a persistent guest and returns every active game", async () => {
    ...
    expect(first.body.games).toHaveLength(107);
    ...
    expect(first.body.games).toEqual(
      expect.arrayContaining([expect.objectContaining({ slug: "game-2048" })]),
    );
    expect(first.body.games).toEqual(
      expect.arrayContaining([expect.objectContaining({ slug: "level-devil" })]),
    );
    expect(first.body.games).toEqual(
      expect.arrayContaining([expect.objectContaining({ slug: "slice-master" })]),
    );
    expect(first.body.games).toEqual(
      expect.arrayContaining([expect.objectContaining({ slug: "drive-mad" })]),
    );
    expect(first.body.games).toEqual(
    ...
    expect(first.body.games).toEqual(
      expect.arrayContaining([expect.objectContaining({ slug: "basketball-stars" })]),
    );
    expect(first.body.games).toEqual(
      expect.arrayContaining([expect.objectContaining({ slug: "master-chess" })]),
    );
    expect(first.body.games).toEqual(
      expect.arrayContaining([expect.objectContaining({ slug: "soccer-real" })]),
    );
    expect(first.body.games).toEqual(
      expect.arrayContaining([expect.objectContaining({ slug: "penalty-shooters-2" })]),
    );
    expect(first.body.games).toEqual(
      expect.arrayContaining([expect.objectContaining({ slug: "tic-tac-toe" })]),
    );
    expect(first.body.games).toEqual(
      expect.arrayContaining([expect.objectContaining({ slug: "go-battle-2" })]),
    );
    expect(first.body.games).toEqual(
      expect.arrayContaining([expect.objectContaining({ slug: "stickman-battle" })]),
    );
    expect(first.body.games).toEqual(
      expect.arrayContaining([expect.objectContaining({ slug: "retro-bowl" })]),
    );
    expect(first.body.games).toEqual(
